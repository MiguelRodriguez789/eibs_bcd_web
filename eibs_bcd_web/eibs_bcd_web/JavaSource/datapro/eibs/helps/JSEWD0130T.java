 ...
 */
public class JSEWD0130T extends JSEIBSServlet {
		...
		try {
			...
			if (!mp.hasError(newmessage)) {
				...
				while (!(msgHelp.getBNKPVI().equals("9"))) {
					String cityAndState = (msgHelp.getBNKCTY().trim() + (msgHelp.getBNKSTA().trim().equals("")?"":", ") + msgHelp.getBNKSTA().trim());
					...
					myRow = new StringBuffer("<TR>");
						myRow.append(
							"<td nowrap><a href=\"javascript:enter('"
								...
								+ msgHelp.getBNKAD1()
								+ "','"
								+ (cityAndState.length() > 35 ? msgHelp.getBNKCTY() : cityAndState)
								+ "','"
								+ msgHelp.getBNKCTR()
								+ "')\">"
								+ msgHelp.getBNKID()
			...
			forward("EWD0130T_bankid_help_helpmessage.jsp", req, res);
