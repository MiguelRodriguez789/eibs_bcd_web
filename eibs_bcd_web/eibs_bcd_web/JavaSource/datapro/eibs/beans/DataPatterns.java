...
public class DataPatterns {
	...
	private void addCell(int column, Row row, CharSequence chars) {
		Cell cell = row.createCell(column);
		
		RichTextString dataWithoutComas = null;
		if (row instanceof HSSFRow) {
			dataWithoutComas = new HSSFRichTextString(
					removeComas(chars.toString()));
		} else {
			dataWithoutComas = new XSSFRichTextString(
					removeComas(chars.toString()));
		}
		cell.setCellValue(dataWithoutComas);
	}
	
	public CharSequence[] newCells() {
		cells = new CharSequence[pattern.size()];
		for (int i = 0; i < cells.length; i++) {
			int myXY[] = (int[]) pattern.get(COLUMN + i);
			if(myXY[1] > lineData.length()){
				cells[i] = lineData.subSequence(myXY[0], lineData.length());
				break;
			} else {
				cells[i] = lineData.subSequence(myXY[0], myXY[1]);
			}
		}
		return cells;
	}
	 ...
	 *  
	 */
	public void formatLine(Row row) {
		newCells();
		for (int i = 0; i < cells.length; i++) {
			addCell(i, row, cells[i]);
		}
	}
	...
	public String formatLine(StringBuffer resultLine) {
		...
		newCells();
		for (int i = 0; i < cells.length; i++) {
			resultLine.append(removeComas(cells[i].toString()));
			if (i == pattern.size() - 1) {
				resultLine.append("\n");
			} else {
				resultLine.append(",");
	...
	private void whenDifferentBegginingAndEqualEnding(int[] xy, int[] myXY,
		...
		if ( xy[0] == lastIndex ) {
			...
			if (xy[1] > lastIndex) {
				...
				i = i + 1;
				j = j + 1;
				//DEPRONTO ME FALTA UNA CONDICION
				test = 0;
			} else {
				test = test + 1;
				j = j + 1;
