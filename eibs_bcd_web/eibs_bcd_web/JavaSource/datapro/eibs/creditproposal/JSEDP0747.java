...
public class JSEDP0747 extends datapro.eibs.master.SuperServlet {
	...
	// Send Initial data
	try {
		msgList = (EDP074701Message) mc.getMessageRecord("EDP074701");
		msgList.setH01USERID(user.getH01USR());
		msgList.setH01PROGRM("EDP074701");
		msgList.setH01TIMSYS(getTimeStamp());
		msgList.setH01SCRCOD("01");
		msgList.setH01OPECOD("0015");
		msgList.send();
		msgList.destroy();
		flexLog("EDP074701 Message Sent");
	} catch (Exception e) {
		e.printStackTrace();
		flexLog("Error: " + e);
	...
	try {
		...
		if (newmessage.getFormatName().equals("EDP074701")) {
			...
			while (ct++ < datapro.eibs.master.JSEIBSProp.getMaxIterations()) {
				...
				msgList = (EDP074701Message) newmessage;
			...
			flexLog("Putting java beans into the session");
			ses.setAttribute("EDP074701Help", beanList);
			ses.setAttribute("userPO", userPO);
			...
			try {
					...
					if (msgError.getERRNUM().equals("0")) {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EDP0747_parametros_list1.jsp");
						callPage(
							LangPath + "EDP0747_parametros_list1.jsp",
							req,
							res);
					} else {
						flexLog(
	...
	//	manejo formato a trabajar first screen
	 ...
	 switch (inptOPT) {
		 ...
		 default :
			 res.sendRedirect(
				 super.srctx
					 + "/servlet/datapro.eibs.creditproposal.JSEDP0747?SCREEN=800") ;
			 break;
		 }
	 }
	...
	// Send Initial data
	try {
		msgList = (EDP074701Message) mc.getMessageRecord("EDP070201");
		msgList.setH01USERID(user.getH01USR());
...
//	ELEERRMessage msgError = null;
	...
	switch (inptOPT) {
		case 1 : //new
			...
			//res.sendRedirect(super.srctx + "/servlet/datapro.eibs.creditproposer.JSEDP0747?SCREEN=300");
		...
		default :
			...
			res.sendRedirect(
				super.srctx
					+ "/servlet/datapro.eibs.creditproposer.JSEDP0747?SCREEN=500") ;
			break;
	}
}
   ...
   protected void procReqNew(
	...
	msgRT = new datapro.eibs.beans.EDP074701Message();
...
//	msgRT.setE01DPXGLN(userPO.getHeader12());
	...
	try {
		flexLog(
			"About to call Page: "
				+ LangPath
				+ "EDP0747_parametros_maintenance.jsp");
		callPage(LangPath + "EDP0747_parametros_maintenance.jsp", req, res);
	} catch (Exception e) {
		flexLog("Exception calling page " + e);
	}
...
protected void procActionDelete(
		...
		try {
			...
			msgRT = (EDP074701Message) ses.getAttribute("brnDetails");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDP074701")) {
				try {
					msgRT = new datapro.eibs.beans.EDP074701Message();
					flexLog("EDP074701 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgRT = (EDP074701Message) newmessage;
				...
				//
					if (IsNotError) { // There are no errors
						res.sendRedirect(
							super.srctx
								+ "/servlet/datapro.eibs.creditproposer.JSEDP0747?SCREEN=200");
						...
						try {
							flexLog(
								"About to call Page: "
									...
									+ "EDP0747_parametros_maintenance.jsp");
							callPage(
								LangPath + "EDP0747_parametros_maintenance.jsp",
								req,
								res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
...
protected void procActionInsert(
			...
			if (newmessage.getFormatName().equals("EDP074701")) {
				...
				//
					if (IsNotError) { // There are no errors
						res.sendRedirect(
							super.srctx
								+ "/servlet/datapro.eibs.creditproposal.JSEDP0747?SCREEN=200");
					} else { // There are errors
						try {
							flexLog(
								"About to call Page: "
	...
	// Receive Data
	try {
		JBObjList bl = (JBObjList) ses.getAttribute("EDP074701Help");
		int idx = Integer.parseInt(req.getParameter("CURRCODE2"));
		bl.setCurrentRow(idx);

		msgDoc = (EDP074701Message) bl.getRecord();
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDP0747", req));
