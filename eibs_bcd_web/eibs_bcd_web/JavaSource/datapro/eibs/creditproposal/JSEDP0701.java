...
public class JSEDP0701 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqFilter(
		...
		if (req.getParameter("GRT") != null) {
			userPO.setHeader1(req.getParameter("GRT"));
		}
...
//		BEGIN TABLE PG
		/*---------------------------------------------------------------*/ 
		// Carga dropdownlist para la consulta de la tabla cnOFC PG
		/*---------------------------------------------------------------*/
		  EDP073601Message msgCnofc = null;

		JBList beanCN05 = null;
		String myFlagCN05 = "";
		String myOptionCN05 = "";
		String myFlag = "";

   		try {
			beanCN05 = new datapro.eibs.beans.JBList();
		...
		//habilitar cuando la primera seleccion esta en blanco
			 myOptionCN05 = "<option value=\"\"></option>";
			 beanCN05.addRow(myFlag, myOptionCN05);
					...
					if (markerCN.equals("*")) {
						...
						myOptionCN05 =
							"<option value=\""
								...
								+ "</option>";
						beanCN05.addRow(myFlagCN05, myOptionCN05);
					}
					newmessage = mc.receiveMessage();
				}
		...
		//		 ses.setAttribute("optList", beanList);
		 ...
		 ses.setAttribute("optCN05", beanCN05);
...
//END TABLE CNPG
		...
		try {
			...
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EDP0701_recaudosGAR_filter.jsp");
			callPage(LangPath + "EDP0701_recaudosGAR_filter.jsp", req, res);
		} catch (Exception ex) {
...
//take out
	  ...
	  if(req.getParameter("E01XXXGRT")!=null){
					 userPO.setHeader1(req.getParameter("E01XXXGRT"));
		 }
					
		// Send Initial data
		try {
			msgList = (EDP070101Message) mc.getMessageRecord("EDP070101");
			msgList.setH01USERID(user.getH01USR());
			msgList.setH01PROGRM("ELD010003");
			msgList.setH01TIMSYS(getTimeStamp());
			msgList.setH01SCRCOD("01");
			msgList.setH01OPECOD("0015");
//			msgList.setE01DPWGRT(req.getParameter("GRT"));
			msgList.setE01DPWGRT(userPO.getHeader1());
			msgList.setH01FLGWK2(userPO.getOption());
			msgList.send();
			msgList.destroy();
			flexLog("EDP070101 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
			throw new RuntimeException("Socket Communication Error");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDP070101")) {
...
//				String myFlag = "";
			   ...
			   beanList.setNoResult(true);
				...
				while (ct++ < datapro.eibs.master.JSEIBSProp.getMaxIterations()) {
					...
					msgList = (EDP070101Message) newmessage;
				...
				flexLog("Putting java beans into the session");
				ses.setAttribute("EDP070101Help", beanList);
				ses.setAttribute("userPO", userPO);
...
//		 String myFlag = "";
		 ...
		 try {
			 beanCNPG = new datapro.eibs.beans.JBList();
			 msgCnofc = new datapro.eibs.beans.EDP073601Message();
		 } catch (Exception ex) {
		 flexLog(mensaje_error+ ex);	
		 }
		 ...
		 try {
			 newmessage = mc.receiveMessage();
		 //habilitar cuando la primera seleccion esta en blanco
		//	  myOptionCNPG = "<option value=\"\"></option>";//
			  beanCNPG.addRow(myFlag, myOptionCNPG);
					 ...
					 if (markerCN.equals("*")) {
						 ...
						 myOptionCNPG =
							 "<option value=\""
								 ...
								 + msgCnofc.getE01CNODSC()
								 + "</option>";
						 beanCNPG.addRow(myFlagCNPG, myOptionCNPG);
					 }
					 newmessage = mc.receiveMessage();
				 }
...
//		  ses.setAttribute("optList", beanList);
		  ses.setAttribute("error", msgError);
		  ses.setAttribute("optCNPG", beanCNPG);
				...
				try {
						...
						if (msgError.getERRNUM().equals("0")) {
							flexLog(
								"About to call Page: "
									+ LangPath
									+ "EDP0701_recaudosGAR_list1.jsp");
							callPage(
								LangPath + "EDP0701_recaudosGAR_list1.jsp",
								req,
								res);
						} else {
							flexLog(
...
//		}
	...
	//	manejo formato a trabajar first screen
	 ...
	 switch (inptOPT) {
		 ...
		 default :
			 res.sendRedirect(
				 super.srctx
					 + "/servlet/datapro.eibs.creditproposal.JSEDP0701?SCREEN=800") ;
			 break;
		 }
	 }
	...
	// Send Initial data
	try {
		msgList = (EDP070101Message) mc.getMessageRecord("EDP070201");
		msgList.setH01USERID(user.getH01USR());
		msgList.setH01PROGRM("ELD010003");
		msgList.setH01TIMSYS(getTimeStamp());
		msgList.setH01SCRCOD("01");
...
//	ELEERRMessage msgError = null;
	...
	switch (inptOPT) {
		case 1 : //new
			...
			res.sendRedirect(
			super.srctx
		+ "/servlet/datapro.eibs.creditproposal.JSEDP0701?SCREEN=300&PRD="
...
//			procReqNew(mc, user, req, res, ses);
		...
		default :
			...
			res.sendRedirect(
				super.srctx
			+ "/servlet/datapro.eibs.creditproposal.JSEDP0701?SCREEN=150&PRD="
			+ req.getParameter("PRD"));
			break;
	}
}
   ...
   protected void procReqNew(
	...
	msg0701 = new datapro.eibs.beans.EDP070101Message();
//	msg0701.setE01DPWGRT(req.getParameter("PRD"));

	ses.setAttribute("msg0701", msg0701);
	try {
		flexLog(
			"About to call Page: "
				+ LangPath
				+ "EDP0701_recaudosGAR_maintenance.jsp");
		callPage(LangPath + "EDP0701_recaudosGAR_maintenance.jsp", req, res);
	} catch (Exception e) {
		flexLog("Exception calling page " + e);
	}
...
protected void procActionDelete(
		...
		try {
			flexLog("Send Initial Data");
			msg0701 = (EDP070101Message) ses.getAttribute("msg0701");
			msg0701.setH01USERID(user.getH01USR());
			msg0701.setH01PROGRM("EGL036001");
			msg0701.setH01TIMSYS(getTimeStamp());
			msg0701.setH01SCRCOD("01");
			msg0701.setH01OPECOD("0003");
			msg0701.setH01FLGWK1("N");

			// all the fields here
			java.util.Enumeration enu = msg0701.fieldEnumeration();
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDP070101")) {
				try {
					msg0701 = new datapro.eibs.beans.EDP070101Message();
					flexLog("EDP070101 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msg0701 = (EDP070101Message) newmessage;
						...
						try {
							flexLog(
								"About to call Page: "
									...
									+ "EDP0701_recaudosGAR_maintenance.jsp");
							callPage(
								LangPath + "EDP0701_recaudosGAR_maintenance.jsp",
								req,
								res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
...
protected void procActionInsert(
	...
	protected void procActionMaintenance(
		...
		try {
			...
			msg0701 = (EDP070101Message) ses.getAttribute("msg0701");
			msg0701.setH01USERID(user.getH01USR());
			msg0701.setH01PROGRM("EDP070101");
			msg0701.setH01TIMSYS(getTimeStamp());
			msg0701.setH01SCRCOD("01");
			msg0701.setH01OPECOD("0005");
			msg0701.setH01FLGWK1("N");
			msg0701.setH01FLGWK3(req.getParameter("opt"));
			...
			if (newmessage.getFormatName().equals("EDP070101")) {
				...
				//
					if (IsNotError) { // There are no errors
						res.sendRedirect(
							super.srctx
								+ "/servlet/datapro.eibs.creditproposal.JSEDP0701?SCREEN=150&GRT="
								+ req.getParameter("GRT"));
					} else { // There are errors
						try {
							flexLog(
								"About to call Page: "
	...
	// Receive Data
	try {
		JBObjList bl = (JBObjList) ses.getAttribute("EDP070101Help");
		int idx = Integer.parseInt(req.getParameter("CURRCODE2"));
		bl.setCurrentRow(idx);

		msgDoc = (EDP070101Message) bl.getRecord();

		flexLog("Putting java beans into the session");
		ses.setAttribute("userPO", userPO);
		ses.setAttribute("msg0701", msgDoc);
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDP0701", req));
