...
public class JSEIE0130 extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE013001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130_inv_trade_tickets.jsp");
						callPage(LangPath + "EIE0130_inv_trade_tickets.jsp", req, res);	
					}
					catch (Exception e) {
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					...
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=900" + "&CODE=" + ORDNUM);
				
					}
					else {
					try {
						    flexLog("About to call Page: " + LangPath + "EIE0130_inv_trade_tickets_confirm.jsp");
							callPage(LangPath + "EIE0130_inv_trade_tickets_confirm.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected  void procActionCalculateMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					...
					try {
							flexLog("About to call Page: " + LangPath + "EIE0130_inv_trade_tickets_mp.jsp");
							callPage(LangPath + "EIE0130_inv_trade_tickets_mp.jsp", req, res);	
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
	...
	protected  void procActionListMultiple(
		...
		switch (inptOPT) {
			case 1 : //New
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=1200" + "&ORDMAST=" + ORDMAST);		    
				break;
			case 2 : //Maintenance
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=1400" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM);
				break;
				
			case 3 : //Delete
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=1600" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM );
				break;
			case 10 : //Ticket Confirm
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=1800");
				break;	
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=1200" + "&ORDMAST=" + ORDMAST);
...
protected  void procActionMaintenanceMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0130?SCREEN=900"+ "&CODE=" + CODE);
				}
				else {  // There are errors
