				...
				if (newmessage.getFormatName().equals("EIE032002")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=RIGHT><img src=\"../images/pledge.gif\" title=\"Pledge\" align=\"absmiddle\" border=\"0\" ><input type=\"radio\" name=\"index\" value=\""
								+ msgList.getE02MSTCUN()
								+ "\" "
								+ chk
								+ " onclick=\"getParams('"
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EIE0320", req));
