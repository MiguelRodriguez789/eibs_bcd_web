...
public class JSEIE0120 extends datapro.eibs.master.SuperServlet {
	...
	protected static final  int A_CALCULATE_MP		= 17;
...
protected  void procActionEnterNew(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgInst = (EIE012001Message)mc.getMessageRecord("EIE012001");
	 	...
	 	try{
	 	 msgInst.setE01ORDCUN(req.getParameter("CUSTOMER"));
	 	}
	 	catch (Exception e)	{
	 	}
	 	
	 	try{
	 	 msgInst.setE01ORDIIC(req.getParameter("INSTRUMENT"));
	 	}
	 	catch (Exception e)	{
	 	}
	 	
	 	try{
	 	 msgInst.setE01ORDPRF(req.getParameter("PORTFOLIO"));
	 	}
	 	catch (Exception e)	{
	 	}
	 	
	 	try{
	 	 msgInst.setE01ORDVNO(req.getParameter("NOMINAL"));
	 	}
	 	catch (Exception e)	{
	 	}	
	 	
	 	try{
	 	 msgInst.setE01ORDQTY(req.getParameter("QUANTITY"));
	 	}
	 	catch (Exception e)	{
	 	}
	 	
	 	try{
	 	 msgInst.setE01ORDPTY(req.getParameter("INSTTYPE"));
	 	}
	 	catch (Exception e)	{
	 	}			
		
		try{
	 	 msgInst.setE01ORDCUC(req.getParameter("CUSTODIAN"));
	 	}
	 	catch (Exception e)	{
	 	}
	 	
	 	try{
	 	 msgInst.setE01ORDTRN(req.getParameter("TRANSCODE"));
	 	}
	 	catch (Exception e)	{
	 	}
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE012001")) {
			try {
				msgInst = new EIE012001Message();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex);
		  	}

			msgInst = (EIE012001Message)newmessage;
			...
			if (IsNotError) {  // There are no errors
                    /*if(msgInst.getE01ORDTRN().equals("5") || msgInst.getE01ORDTRN().equals("6") ){
                    	try {
						flexLog("About to call Page: " + LangPath + "EIE0120_inv_mult_operations.jsp");
						callPage(LangPath + "EIE0120_inv_mult_operations.jsp", req, res);
					}
					catch (Exception e) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0120_inv_trade_tickets.jsp");
						callPage(LangPath + "EIE0120_inv_trade_tickets.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				...
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEWD0309?SCREEN=300");			}
		}
		else
			flexLog("Message " + newmessage.getFormatName() + " received.");
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE012001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0120_inv_inq_trade_tickets.jsp");
						callPage(LangPath + "EIE0120_inv_inq_trade_tickets.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE012001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0120_inv_trade_tickets_del.jsp");
						callPage(LangPath + "EIE0120_inv_trade_tickets_del.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected  void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		flexLog("Send Initial Data");
	 	msgInst = (EIE012001Message)ses.getAttribute("invTrade");
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					...
					|| msgInst.getE01ORDTRN().equals("6")){
					   res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=900" + "&CODE=" + ORDNUM);

					}
					else {
					try {
						    flexLog("About to call Page: " + LangPath + "EIE0120_inv_trade_tickets_confirm.jsp");
							callPage(LangPath + "EIE0120_inv_trade_tickets_confirm.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected  void procActionCalculateMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
			throws ServletException, IOException {


	MessageRecord newmessage = null;
	EIE012002Message msgInst = null;
	ELEERRMessage msgError = null;
	UserPos	userPO = null;
	boolean IsNotError = false;
	...
	{
		flexLog("Send Initial Data");
	 	msgInst = (EIE012002Message)ses.getAttribute("invMult");
		//msgInst = (EFE0120DSMessage)mc.getMessageRecord("EFE0120DS");
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				msgInst = (EIE012002Message)newmessage;
				// showESD008004(msgInst);
				...
				String ORDNUM = msgInst.getE02ORDNUM();


				if (IsNotError) {  // There are no errors

					try {
							flexLog("About to call Page: " + LangPath + "EIE0120_inv_trade_tickets_mp.jsp");
							callPage(LangPath + "EIE0120_inv_trade_tickets_mp.jsp", req, res);
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
	...
	try
	{
		msgInst = (EIE012002Message)mc.getMessageRecord("EIE012002");
	 	msgInst.setH02USERID(user.getH01USR());
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("EIE012002")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0120_sel_list_purchase.jsp");
						callPage(LangPath + "EIE0120_sel_list_purchase.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected  void procActionListMultiple(
		...
		switch (inptOPT) {
			case 1 : //New
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=1200" + "&ORDMAST=" + ORDMAST);
				break;
			case 2 : //Maintenance
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=1400" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM);
				break;

			case 3 : //Delete
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=1600" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM );
				break;
			case 10 : //Ticket Confirm
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=1800");
				break;
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0110?SCREEN=1200" + "&ORDMAST=" + ORDMAST);
		}
	}
...
protected  void procActionEnterNewMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	MessageRecord newmessage = null;
	//mod emat
	EIE012001Message msgMaster = null;
	EIE012002Message msgInst = null;
	ELEERRMessage msgError = null;
	UserPos	userPO = null;
	boolean IsNotError = false;
	...
	userPO = (datapro.eibs.beans.UserPos)ses.getAttribute("userPO");
	//mod emat
	msgMaster = (datapro.eibs.beans.EIE012001Message)ses.getAttribute("invTrade");
	...
	try
	{
		msgInst = (EIE012002Message)mc.getMessageRecord("EIE012002");
	 	...
	 	msgInst.setH02TIMSYS(getTimeStamp());
	 	msgInst.setH02SCRCOD("01");
	 	msgInst.setH02OPECOD("0001");
	 	//mod emat - mapeo de campos necesarios
	 	try{
	 	msgInst.setE02ORDMNU(msgMaster.getE01ORDNUM());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDMCU(msgMaster.getE01ORDCUN());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDMCN(msgMaster.getE01ORDCTN());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDMIC(msgMaster.getE01ORDIIC());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDMCY(msgMaster.getE01ORDSCY());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDIN1(msgMaster.getE01ORDIN1());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDIN2(msgMaster.getE01ORDIN2());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDIN3(msgMaster.getE01ORDIN3());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDINT(msgMaster.getE01ORDINT());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDINU(msgMaster.getE01ORDINU());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDST1(msgMaster.getE01ORDST1());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDST2(msgMaster.getE01ORDST2());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDST3(msgMaster.getE01ORDST3());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDTRN(msgMaster.getE01ORDTRN());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDPTY(msgMaster.getE01ORDPTY());
	 	}
		catch (Exception e)	{

		}
		try{
	 	msgInst.setE02ORDCDE(msgMaster.getE01ORDCDE());
	 	msgInst.setE02ORDPNM(msgMaster.getE01ORDPNM());
	 	msgInst.setE02ORDPBK(msgMaster.getE01ORDPBK());
	 	msgInst.setE02ORDPBR(msgMaster.getE01ORDPBR());
	 	msgInst.setE02ORDPCY(msgMaster.getE01ORDPCY());
	 	msgInst.setE02ORDPGL(msgMaster.getE01ORDPGL());
	 	msgInst.setE02ORDPCC(msgMaster.getE01ORDPCC());
	 	msgInst.setE02ORDPCC(msgMaster.getE01ORDPCC());
	 	msgInst.setE02ORDCSP(msgMaster.getE01ORDCSP());
	 	msgInst.setE02ORDSYM(msgMaster.getE01ORDSYM());
	 	msgInst.setE02ORDISI(msgMaster.getE01ORDISI());
	 	msgInst.setE02ORDISR(msgMaster.getE01ORDISR());
	 	msgInst.setE02ORDEMI(msgMaster.getE01ORDEMI());
	 	msgInst.setE02ORDPRB(msgMaster.getE01ORDPRB());
	 	msgInst.setE02ORDBRK(msgMaster.getE01ORDBRK());
	 	msgInst.setE02ORDBKN(msgMaster.getE01ORDBKN());
	 	msgInst.setE02ORDCUC(msgMaster.getE01ORDCUC());
	 	msgInst.setE02ORDCCN(msgMaster.getE01ORDCCN());
	 	msgInst.setE02ORDRTE(msgMaster.getE01ORDRTE());
	 	msgInst.setE02ORDIPL(msgMaster.getE01ORDIPL());
	 	//msgInst.setE02ORDLPC(msgMaster.getE01ORDLPC());
	 	msgInst.setE02ORDLP1(msgMaster.getE01ORDLP1());
	 	msgInst.setE02ORDLP2(msgMaster.getE01ORDLP2());
	 	msgInst.setE02ORDLP3(msgMaster.getE01ORDLP3());
		}
		catch (Exception e)	{
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE012002")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0120_inv_trade_tickets_mp_del.jsp");
						callPage(LangPath + "EIE0120_inv_trade_tickets_mp_del.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected  void procActionMaintenanceMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0120?SCREEN=900"+ "&CODE=" + CODE);
				}
				else {  // There are errors
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EIE0120", req));
			...
			switch (screen) {
				...
				case A_CALCULATE :
					...
					break;
				case A_CALCULATE_MP :
					procActionCalculateMP(mc, msgUser, req, res, session);
					break;
				case A_ENTER_NEW :
