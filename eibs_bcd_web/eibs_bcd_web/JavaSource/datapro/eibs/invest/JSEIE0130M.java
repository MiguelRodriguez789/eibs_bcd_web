...
public class JSEIE0130M extends datapro.eibs.master.SuperServlet {
...
protected void procActionEnterMaint(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE013001")) {
			try {
				msgInst = new datapro.eibs.beans.EIE013001Message();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130M_inv_trade_tickets.jsp");
						callPage(LangPath + "EIE0130M_inv_trade_tickets.jsp", req, res);	
					}
					catch (Exception e) {
				...
				//res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEWD0309?SCREEN=13");			
				try {
...
protected void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					...
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=900" + "&CODE=" + ORDNUM);
				
					}
					else {
					try {
						    flexLog("About to call Page: " + LangPath + "EIE0130M_inv_trade_tickets_confirm.jsp");
							callPage(LangPath + "EIE0130M_inv_trade_tickets_confirm.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	try
	{
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=900");
				...
				if (newmessage.getFormatName().equals("EIE013002")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130M_sel_list_purchase.jsp");
						callPage(LangPath + "EIE0130M_sel_list_purchase.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected void procActionListMultiple(
		...
		switch (inptOPT) {
			case 1 : //New
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=1200" + "&ORDMAST=" + ORDMAST);		    
				break;
			case 2 : //Maintenance
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=1400" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM);
				break;
				
			case 3 : //Delete
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=1600" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM );
				break;
			case 10 : //Ticket Confirm
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=1800");
				break;	
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=1200" + "&ORDMAST=" + ORDMAST);
...
protected void procActionEnterMaintMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE013002")) {
			try {
				msgInst = new datapro.eibs.beans.EIE013002Message();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130M_inv_trade_tickets_mp.jsp");
						callPage(LangPath + "EIE0130M_inv_trade_tickets_mp.jsp", req, res);	
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected void procActionMaintenanceMP(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0130M?SCREEN=900"+ "&CODE=" + CODE);
				}
				else {  // There are errors
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EIE0130", req));
