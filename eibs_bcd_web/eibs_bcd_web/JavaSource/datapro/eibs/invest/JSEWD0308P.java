...
public class JSEWD0308P extends datapro.eibs.master.SuperServlet {
	...
	private synchronized void procReqList(
		...
		try {
			...
			try { // Search By Instrument Type
				msgList.setSWDPTY(req.getParameter("ITYPE").toUpperCase());
			} catch (Exception e) {
			...
			try { // Search By Date to 
				...
				msgList.setSWDTO2(req.getParameter("DATET2").toUpperCase());
				msgList.setSWDTO3(req.getParameter("DATET3").toUpperCase());
			...
			try { // Search By Standard
				
				msgList.setSWDISIRSP(req.getParameter("STANDARDR").toUpperCase());				 
				
			} catch (Exception e) {
				
			}
			
			try { // Search By Moodys
				
				msgList.setSWDISIRMD(req.getParameter("MOODYSR").toUpperCase());				 
				
			} catch (Exception e) {
				
			}

			try { // Search By Preferential Mark (known as complex too)
				
				msgList.setSWDPRE(req.getParameter("PREFINST").toUpperCase());
				userPO.setHeader9(req.getParameter("PREFINST").toUpperCase());				 
			} catch (Exception e) {userPO.setHeader9("&nbsp;");
			}
				...
				if (newmessage.getFormatName().equals("EWD0308DS")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							{
							 myRow.append("<TD NOWRAP  ALIGN=RIGHT><img src=\"" +super.webAppPath+"/images/maintenance.gif\" title=\"Pending\" align=\"middle\" border=\"0\" ><input type=\"radio\" name=\"index\" value=\""
								+ msgList.getSWDIIC()
								+ "\" "
								+ chk
								+ " onclick=\"getParams('"
	...
	private synchronized void procReqListInquiry(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
	...
	private synchronized void procReqEnterAdvSearch(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
		throws ServletException, IOException {
	...
	private synchronized void procReqEnterSearchInq(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
		throws ServletException, IOException {
