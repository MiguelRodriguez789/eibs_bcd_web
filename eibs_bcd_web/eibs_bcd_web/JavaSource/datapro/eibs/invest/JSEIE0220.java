...
public class JSEIE0220 extends datapro.eibs.master.SuperServlet {

	// Action 
	protected static final  int A_MAINTENANCE		= 2;
	
	protected static final  int A_VALIDATE_SPLIT	= 300;
	protected static final  int A_ENTER_SPLIT	 	= 400;
...
protected  void procActionEnterMaint(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		while (enu.hasMoreElements()) {
	 	...
	 	msgInst.send();
	 	msgInst.destroy();
	 	flexLog("EIE022001 Message Sent");
	}		
	catch (Exception e)	{
		e.printStackTrace();
		flexLog("Error: " + e);
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE022001")) {
			try {
				msgInst = new EIE022001Message();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}

			msgInst = (EIE022001Message)newmessage;
			...
			ses.setAttribute("invSplit", msgInst);
			ses.setAttribute("userPO", userPO);
 
			if (IsNotError) {  // There are no errors
					try {
						flexLog("About to call Page: " + LangPath + "EIE0220_inv_split_confirm.jsp");
						callPage(LangPath + "EIE0220_inv_split_confirm.jsp", req, res);	
					}
					catch (Exception e) {
...
protected  void procActionValidateSplit(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
			throws ServletException, IOException {
	MessageRecord newmessage = null;
	EIE022001Message msgInst = null;
	ELEERRMessage msgError = null;
	...
	try
	{
		msgInst = (EIE022001Message)mc.getMessageRecord("EIE022001");
		msgInst.setH01USERID(user.getH01USR());
		msgInst.setH01PROGRM("EIE0220");
		msgInst.setH01TIMSYS(getTimeStamp());
		msgInst.setH01SCRCOD("01");
		msgInst.setH01OPECOD("0002");

		try {
			if (req.getParameter("E01ISIIIC") != null)
				msgInst.setE01ISIIIC(req.getParameter("E01ISIIIC"));
		}
		catch (Exception e)
		{
		...
		try {
			if (req.getParameter("E01ISIDSC") != null)
				msgInst.setE01ISIDSC(req.getParameter("E01ISIDSC"));
		}
		catch (Exception e)
		{
			msgInst.setE01ISIDSC("");
			flexLog("Input data error " + e);
		}		
		try {
			if (req.getParameter("E01ISIPTY") != null)
				msgInst.setE01ISIPTY(req.getParameter("E01ISIPTY"));
		}
		catch (Exception e)
		{
			msgInst.setE01ISIPTY("0");
			flexLog("Input data error " + e);
		}
...
protected  void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		flexLog("Send Initial Data");
	 	msgInst = (EIE022001Message)ses.getAttribute("invSplit");
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EIE0220", req));
			...
			switch (screen) {
				...
				case A_VALIDATE_SPLIT : 
					procActionValidateSplit(mc, msgUser, req, res, session);
					break;									
