...
public class JSERA0081 extends datapro.eibs.master.SuperServlet {
...
protected void procActionApproval(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		msgList = (ERA008102Message)mc.getMessageRecord("ERA008102");
		msgList.setH02USERID(user.getH01USR());
	 	msgList.setH02PROGRM("ERA0081");
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
			...
			if (IsNotError) { // There is no error
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.client.JSERA0081?SCREEN=1");
			}
			else {
				try {
					 flexLog("About to call Page: " + LangPath + "ERA0081_confecamaras_list.jsp");
					 callPage(LangPath + "ERA0081_confecamaras_list.jsp", req, res);
				}
				catch (Exception e) {
...
protected void procReqApproval(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	{
		flexLog("Send Initial Data");
		msgList = (ERA008101Message)mc.getMessageRecord("ERA008101");
		msgList.setH01USERID(user.getH01USR());
	 	msgList.setH01PROGRM("ERA0081");
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
	  	...
	  	else if (newmessage.getFormatName().equals("ERA008101")) {
			...
			while (true) {
				...
				msgList = (ERA008101Message)newmessage;
				...
				if (marker.equals("*")) {
					...
					myRow.append("<TD NOWRAP ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCREF()) + "</A>"+ chkWarn+ "</TD>");;
					myRow.append("<TD NOWRAP><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01CUSNA1()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCCCY()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"RIGHT\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCCY(msgList.getE01ROCAMN()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01REMARK()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCBNK()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCCUN()) + "</A></TD>");
					myRow.append("</TR>");
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ERA0081", req));
...
protected void procReqPassword(HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		...
		userPO = (datapro.eibs.beans.UserPos)ses.getAttribute("userPO");
		userPO.setRedirect("/servlet/datapro.eibs.client.JSERA0081?SCREEN=" + R_APPROVAL);
		ses.setAttribute("userPO", userPO);
