...
protected void procReqApproval(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	ERA008001Message searchParameters = (ERA008001Message) ses.getAttribute("searchParameters");
	if(null != searchParameters){
		setMessageRecord(req, searchParameters);
	} else {
		searchParameters = new ERA008001Message();
	}		
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					myRow.append("<TD NOWRAP width=\"5%\" ALIGN=\"CENTER\"><input type=\"radio\" name=\"REFNUM\" value=\"" + msgList.getE01ROCREF() + "\" " + chk + " onclick=\"showAddInfo("+indexRow+")\"></TD>");
					myRow.append("<TD NOWRAP width=\"15%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCREF()) + "</A>"+ chkWarn+ "</TD>");;
					myRow.append("<TD NOWRAP width=\"10%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCCUN()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"25%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01CUSNA1()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"10%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCCCY()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"15%\" ALIGN=\"RIGHT\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCCY(msgList.getE01ROCAMN()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"10%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01REMARK()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"10%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalColla('" + msgList.getE01ROCACD() + "','" + msgList.getE01ROCREF() + "')\">" + Util.formatCell(msgList.getE01ROCBNK()) + "</A></TD>");
					myRow.append("</TR>");
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ERA0080", req));
