...
public class JSETX0160 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqEnterBranch(
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "ETX0160_spectax_enter.jsp");
			callPage(LangPath + "ETX0160_spectax_enter.jsp", req, res);
		} catch (Exception e) {
	...
	protected void procActionPos(
		...
		switch (inptOPT) {
			...
			default :
				res.sendRedirect(
					super.srctx
						+ "/servlet/datapro.eibs.client.JSETX0160?SCREEN=500");
				break;
		}
	}
	...
	protected void procReqBranchList(
		...
		if(req.getParameter("E01T42CDE")!=null){
			userPO.setHeader10(req.getParameter("E01T42CDE"));
		}
		...
		// Send Initial data
		try {
			msgList = (ETX016001Message) mc.getMessageRecord("ETX016001");
			msgList.setH01USERID(user.getH01USR());
			msgList.setH01PROGRM("ELD010003");
			msgList.setH01TIMSYS(getTimeStamp());
			msgList.setH01SCRCOD("01");
			msgList.setH01OPECOD("0015");
			msgList.setE01T42CDE(userPO.getHeader10());
			msgList.send();
			msgList.destroy();
			flexLog("ETX016001 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
		...
		try {
			...
			if (newmessage.getFormatName().equals("ETX016001")) {
				...
				while (true) {
					...
					msgList = (ETX016001Message) newmessage;
				...
				flexLog("Putting java beans into the session");
				ses.setAttribute("ETX016001Help", beanList);
				ses.setAttribute("userPO", userPO);
				...
				try {
					...
					if (msgError.getERRNUM().equals("0"))
					{
						flexLog(
								"About to call Page: "
								+ LangPath
								+ "ETX0160_spectax_list.jsp");	
						callPage(
						LangPath + "ETX0160_spectax_list.jsp",
						req,
						res);
					}
					else
					{
						...
						callPage(
						LangPath + "ETX0160_spectax_enter.jsp",
						req,
						res);
	...
	protected void procActionMaintenance(
		...
		try {
			...
			msgRT = (ETX016001Message) ses.getAttribute("brnDetails");
		...
		try {
			...
			if (newmessage.getFormatName().equals("ETX016001")) {
				try {
					msgRT = new datapro.eibs.beans.ETX016001Message();
					flexLog("ETX016001 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgRT = (ETX016001Message) newmessage;
				...
				if (IsNotError) { // There are no errors
					res.sendRedirect(
										super.srctx
											+ "/servlet/datapro.eibs.client.JSETX0160?SCREEN=100");
				} else { // There are errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "ETX0160_spectax_maintenance.jsp");
						callPage(
							LangPath + "ETX0160_spectax_maintenance.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
	...
	protected void procReqMaintenance(
		...
		// Receive Data
		try {
			JBObjList bl = (JBObjList) ses.getAttribute("ETX016001Help");
			int idx = Integer.parseInt(req.getParameter("CURRCODE"));
			bl.setCurrentRow(idx);

			msgDoc = (ETX016001Message) bl.getRecord();
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ETX0160", req));
