...
public class JSESB0110 extends SuperServlet {
	...
	protected void procReqNew(
		...
		ESB011001Message msgPart = new ESB011001Message();
		...
		try {
			...
			msgPart = (ESB011001Message)mc.getMessageRecord("ESB011001");
			msgPart.setH01USERID(user.getH01USR());
			msgPart.setH01PROGRM("ESB0110");
			msgPart.setH01TIMSYS(getTimeStamp());
			msgPart.setH01OPECOD("0001");

			msgPart.setE01SBMCOD(req.getParameter("E01SBMCOD"));

			msgPart.send();
			msgPart.destroy();
			flexLog("ESB0011001 Message Sent");
			...
			// Receive Error Y Data
				...
				if (newmessage.getFormatName().equals("ESB011001")) {
					msgPart = new datapro.eibs.beans.ESB011001Message();
					msgPart = (ESB011001Message) newmessage;
					flexLog("ESB011001 Message Received");
					...
					if (IsNotError) { // There are no errors
						userPO.setHeader1(req.getParameter("E01SBMCOD"));
						try {
							flexLog("About to call Page: " + LangPath + "ESB0110_safe_deposit_new.jsp");
							callPage(LangPath + "ESB0110_safe_deposit_new.jsp", req, res);
						} catch (Exception e) {
						...
						try {
							res.sendRedirect(super.srctx + 
								"/servlet/datapro.eibs.safedeposit.JSESB0110?SCREEN=1" );
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
						}		
					}
	...
	protected void procReqList(
		...
		try {
			...
			} else if (newmessage.getFormatName().equals("ESB011001")) {
			...
			try {
				flexLog("About to call Page: " + LangPath + "ESB0110_safe_deposit_list.jsp");
				callPage(LangPath + "ESB0110_safe_deposit_list.jsp", req, res);						
			} catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
	...
	protected void procReqMaint(
		...
		try {
			...
			ESB011001Message msgTemp = null;
			beanList = (JBObjList) ses.getAttribute("msgList");
			beanList.setCurrentRow(Integer.parseInt(req.getParameter("ROW")));
			msgTemp = (ESB011001Message) beanList.getRecord();

			msgPart = (ESB011001Message) mc.getMessageRecord("ESB011001");
			msgPart.setE01SBMCOD(msgTemp.getE01SBMCOD());
			...
			// Receive Data
				...
				if (newmessage.getFormatName().equals("ESB011001")) {
					...
					if (IsNotError) { // There are no errors
						try {
							flexLog("About to call Page: " + LangPath + "ESB0110_safe_deposit_maint.jsp");
							callPage(LangPath + "ESB0110_safe_deposit_maint.jsp", req, res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
					} else { // There are errors
	...
	protected void procActionDelete(
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (IsNotError) {
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.safedeposit.JSESB0110?SCREEN=1");
				} else {
					try {
						flexLog("About to call Page: " + LangPath + "ESB0110_safe_deposit_list.jsp");
						callPage(LangPath + "ESB0110_safe_deposit_list.jsp", req, res);						
				...
				if (IsNotError) { // There are no errors
						...
						try {
							...
							if (userPO.getPurpose().equals("NEW"))
								myPage = "ESB0110_safe_deposit_new.jsp";
							else if (userPO.getPurpose().equals("MAINTENANCE")) 
								myPage = "ESB0110_safe_deposit_maint.jsp";
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ESB0110", req));
