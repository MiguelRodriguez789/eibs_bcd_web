				...
				if (newmessage.getFormatName().equals("EEJ001001")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=CENTER >" + Util.formatDate(msgList.getE01EJLLM1(),msgList.getE01EJLLM2(),msgList.getE01EJLLM3())  + " - " + msgList.getE01EJLLMT() + "</td>");
							myRow.append("<TD NOWRAP  ALIGN=LEFT >" + msgList.getE01EJLLMU() + "</td>");
							myRow.append("</TR>");
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EEJ0010", req));
