...
import datapro.eibs.beans.ESD020401Message;
import datapro.eibs.beans.ESD020401Message;
import datapro.eibs.beans.ESD020401Message;
import datapro.eibs.beans.ESD020401Message;
...
public class JSESD0204 extends JSEIBSServlet {
	...
	protected static final int R_FEES_DELETE = 3;
	...
	private void procReqEnter(ESS0030DSMessage user, HttpServletRequest req,
		...
		throws ServletException, IOException {
			...
			forward("ESD0204_manual_fees_enter.jsp", req, res);
	...
	private void procReqFeesList(ESS0030DSMessage user, HttpServletRequest req,
		...
		try {
			mp = getMessageProcessor("ESD0204", req);
			ESD020401Message msgList = (ESD020401Message) mp.getMessageRecord("ESD020401");
			msgList.setH01USERID(user.getH01USR());
		 	msgList.setH01PROGRM("ESD0204");
			...
			if (mp.hasError(msgError)) {
				...
				ESD020401Message msgHeader = (ESD020401Message) mp.receiveMessageRecord("ESD020401");
				...
				forward("ESD0204_manual_fees_list.jsp", req, res);
			}
	...
	private void procReqFeesNew(ESS0030DSMessage user, HttpServletRequest req,
		...
		try {
			...
			if (index >= 0){
				ESD020401Message listMessage = (ESD020401Message)list.get(index);
			...
			forward("ESD0204_manual_fees_det.jsp", req, res);
	...
	private void procActionFeesMaintenance(ESS0030DSMessage user,
		...
		try {
			...
			ELEERRMessage msgError = (ELEERRMessage) mp.receiveMessageRecord("ELEERR");
			msgList = (ESD020401Message) mp.receiveMessageRecord("ESD020401");
	...
	private void procReqPassword(HttpServletRequest req,
		...
		userPO.setRedirect("/servlet/datapro.eibs.params.JSESD0204?SCREEN="
							+ R_APPROVAL);
		session.setAttribute("userPO", userPO);
	...
	private void procReqApproval(ESS0030DSMessage user, HttpServletRequest req,
		...
		try {
			...
			session.setAttribute("userPO", userPO);
			session.setAttribute("ESD0204Help", list);

			forward("ESD0204_manual_fees_approval.jsp", req, res);
	...
	private void procActionApproval(ESS0030DSMessage user,
		...
		try {
			...
			JBObjList list = (JBObjList)session.getAttribute("ESD0204Help");
	...
	private void procReqInquiryApp(ESS0030DSMessage user,
		...
		try {
			...
			forward("ESD0204_manual_fees_det_inq.jsp", req, res);
