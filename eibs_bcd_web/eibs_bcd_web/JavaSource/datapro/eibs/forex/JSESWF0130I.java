...
public class JSESWF0130I extends datapro.eibs.master.SuperServlet {
		...
		if (newmessage.getFormatName().equals("ESWF10001")) {
			...
			if (IsNotError) {  // There are no errors
					try {
						...
						callPage(LangPath + "ESWF100I_fe_cd_format.jsp", req, res);	
					}
					catch (Exception e) {
...
protected void procReqSwift320CDS(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("ESWF10002")) {
			...
			if (IsNotError) {  // There are no errors
					try {
						flexLog("About to call Page: " + LangPath + "ESWF320I_fe_cd_format.jsp");
						callPage(LangPath + "ESWF320I_fe_cd_format.jsp", req, res);	
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}					
