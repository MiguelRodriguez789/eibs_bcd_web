...
public class JSEDL0120 extends datapro.eibs.master.SuperServlet {
	...
	protected void procActionMaintDeal(
		...
		try {
			...
			setMessageField(msgDeal, "E01DLSTYP", req.getParameter("E01DLSTYP"));
			if (msgDeal.getE01DLSTYP().equals("")) {
				setMessageField(msgDeal, "E01DLSTYP", req.getParameter("E01FESTYP"));
			}
			if (msgDeal.getE01DLSTYP().equals("")) {
				setMessageField(msgDeal, "E01DLSTYP", userPO.getType());
			}			
			setMessageField(msgDeal, "E01DLSREF", req.getParameter("E01DLSREF"));
			setMessageField(msgDeal, "E01DLSCUN", req.getParameter("E01DLSCUN"));
			setMessageField(msgDeal, "E01DLSREF", req.getParameter("E01DLSREF"));
			setMessageField(msgDeal, "E01DLSDID", req.getParameter("E01DLSDID"));
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDL0120DS")) {
				...
				try {
					...
					flexLog("Exception calling page " + page + " " + e);
				}
			} else
				flexLog("Message " + newmessage.getFormatName() + " received.");
			...
			if (newmessage.getFormatName().equals("EDL0120DS")) {
				...
				if (IsNotError) { // There are no errors
					try {
						...
						callPage(
							LangPath + "EDL0120_fe_deposits.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected void procActionEnterCommercial(
		...
		try {
			...
			setMessageField(msgDeal, "E01DLSREF", req.getParameter("E01DLSREF"));
			setMessageField(msgDeal, "E01DLSDID", req.getParameter("E01DLSDID"));
			setMessageField(msgDeal, "H01FLGWK2", req.getParameter("E01DLSPTY"));
				...
				try {
					...
					switch (screen) {
						...
						case A_DEPOSITS :
							...
							break;
						case A_COMM :
							procActionCommercial(mc, msgUser, req, res,	session);
							break;
						case A_BANKERS :
						...
						case A_BLK :
							...
							// Action 
						case A_ENTER_LOAN :
							procActionEnterLoans(mc, msgUser, req, res, session);
							break;
						case A_ENTER_DEPOSIT :
							procActionEnterDeposits(mc, msgUser, req, res, session);
							break;
						case A_ENTER_COMM :
							procActionEnterCommercial(mc, msgUser, req, res, session);
							break;
						case A_ENTER_BANKERS :
							procActionEnterAccept(mc, msgUser, req, res, session);
							break;
						case A_ENTER_CDS :
