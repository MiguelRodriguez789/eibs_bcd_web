...
public class JSESWF0200I extends datapro.eibs.master.SuperServlet {
...
protected void procReqEnterSearch(ESS0030DSMessage user,
		...
		try {
			flexLog("About to call Page: " + LangPath + "ESWF200I_enter_search.jsp");
			callPage(LangPath + "ESWF200I_enter_search.jsp", req, res);
		} catch (Exception e) {
	  	...
	  	newmessage = mc.receiveMessage();
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					String showInqSwift = "showInqSwiftI('" + msgList.getESW1FTM() + "', '" 
														   + msgList.getESW1REF() + "', '" 
														   + msgList.getESW1USR() + "')";
			...
			try {
				 flexLog("About to call Page: " + LangPath + "ESWF200I_sel.jsp");
				 callPage(LangPath + "ESWF200I_sel.jsp", req, res);
			}
			catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
...
protected void procReqSwiftInquiry(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
			...
			try {
				flexLog("About to call Page: " + LangPath + "ESWF200I_swift_free_format_inq.jsp");
				callPage(LangPath + "ESWF200I_swift_free_format_inq.jsp", req, res);	
			}
			catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ESWF200", req));
