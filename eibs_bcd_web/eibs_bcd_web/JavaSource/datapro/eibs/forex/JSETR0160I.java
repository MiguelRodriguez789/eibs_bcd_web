...
public class JSETR0160I extends datapro.eibs.master.SuperServlet {
...
protected void procActionEnterFRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgFRA = (ETR0160DSMessage)mc.getMessageRecord("ETR0160DS");
...
flexLog("inside exception E01WRACC");		
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("ETR0160DS")) {
			try {
				msgFRA = new ETR0160DSMessage();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}

			msgFRA = (ETR0160DSMessage)newmessage;
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ETR0160", req));
