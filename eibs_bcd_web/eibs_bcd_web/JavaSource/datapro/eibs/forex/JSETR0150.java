...
public class JSETR0150 extends datapro.eibs.master.SuperServlet {
	...
	protected static final int A_FRA_TCK			= 65;
	...
	// FRASettlement
	protected static final int R_FRA_STL    	= 70;
	protected static final int A_FRA_STL		= 80;
...
public void destroy() {
	...
	flexLog("free resources used by JSETR0150");
...
protected void procActionEnterFRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	ETR0150DSMessage msgFRA = null;
	ELEERRMessage msgError = null;
	UserPos	userPO = null;	
	boolean IsNotError = false;
flexLog("JSETR0150-->procActionEnterFRA");
	try {
	...
	try
	{
		msgFRA = (ETR0150DSMessage)mc.getMessageRecord("ETR0150DS");
	 	msgFRA.setH01USERID(user.getH01USR());
	 	msgFRA.setH01PROGRM("ETR0150");
		...
		try {
		 	msgFRA.setE01FRAACC(req.getParameter("E01FRAACC"));
		}
		catch (Exception e)	{
	 	  msgFRA.setE01FRAACC("");
		}
	 	...
	 	msgFRA.send();
	 	msgFRA.destroy();
	 	flexLog("ETR0160DS Message Sent");
	}		
	catch (Exception e)	{
		e.printStackTrace();
		flexLog("Error: " + e);
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("ETR0150DS")) {
			try {
				msgFRA = new ETR0150DSMessage();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}

			msgFRA = (ETR0150DSMessage)newmessage;

			userPO.setIdentifier(msgFRA.getE01FRAACC());
			userPO.setCusNum(msgFRA.getE01FRACUN());
			userPO.setCusName(msgFRA.getD01FRACP1());
			userPO.setCurrency(msgFRA.getE01FRACCY());
			userPO.setProdCode(msgFRA.getE01FRAPRO());
			...
			if (IsNotError) {  // There are no errors
					try {
						// TICKET PAGE
						flexLog("About to call Page: " + LangPath + "ETR0150_fra_acc.jsp");
						callPage(LangPath + "ETR0150_fra_acc.jsp", req, res);	
					}
					catch (Exception e) {
...
protected void procActionFRASettlement(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
			throws ServletException, IOException {
	...
	UserPos	userPO = null;	
	boolean IsNotError = false;
flexLog("JSETR0150-->procActionFRASettlement");
	...
	try
	{
		...
		msgFRA = (ETR0150DSMessage)ses.getAttribute("fra");
		msgFRA.setH01USERID(user.getH01USR());
		msgFRA.setH01PROGRM("ETR0150");
		msgFRA.setH01TIMSYS(getTimeStamp());
		msgFRA.setH01SCRCOD("01");
		...
		try {
		     msgFRA.setE01FRANRT(req.getParameter("E01FRANRT"));
		}
		catch (Exception e) {  flexLog("Error: " + e);}	
		
flexLog("E01FRANRT"	+ req.getParameter("E01FRANRT") );		
...
flexLog("msgFRA" + msgFRA );
		//msgFRA.send();
		mc.sendMessage(msgFRA);
		msgFRA.destroy();
		flexLog("ETR0150DS Message Sent");
	}		
	catch (Exception e)	{
		e.printStackTrace();
		flexLog("Error: " + e);
...
flexLog("Data =" + newmessage);
				...
				try {
					msgFRA = new ETR0150DSMessage();
					flexLog("ETR0150DS Message Received");
				} 
				catch (Exception ex) {
					flexLog("Error: " + ex); 
				}
	...
	//			userPO.setHeader3("FRA");
				...
				if (IsNotError) {  // There are no errors
					try {
							flexLog("About to call Page: " + LangPath + "ETR0150_fra_ticket.jsp");
							callPage(LangPath + "ETR0150_fra_ticket.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
						...
						try {
							flexLog("About to call Page: " + LangPath + "ETR0150_fra_stl.jsp");
							callPage(LangPath + "ETR0150_fra_stl.jsp", req, res);	
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);	
						} 
...
protected void procActionFRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	UserPos	userPO = null;	
	boolean IsNotError = false;
	flexLog("JSETR0150-->procActionFRA");
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					try {
						    flexLog("About to call Page: " + LangPath + "ETR0150_fe_fra_mod.jsp");
						...
						try {
							flexLog("About to call Page: " + LangPath + "ETR0150_fe_fra_basic.jsp");
...
protected void procActionFRATck(ESS0030DSMessage user,
	...
	ELEERRMessage msgError = null;
	UserPos userPO = null;
flexLog("JSETR0150-->procActionFRATck");
	try {
	...
	protected void procActionFRAMod(ESS0030DSMessage user,
		...
		ELEERRMessage msgError = null;
		UserPos userPO = null;
flexLog("JSETR0150-->procActionFRAMod");
		try {
...
flexLog("JSETR0150-->procReqFRA");
	...
	protected void procReqFRAMaint(ESS0030DSMessage user,
		...
		ELEERRMessage msgError = null;
		UserPos userPO = null;
flexLog("JSETR0150-->procReqFRAMaint");
		try {
	...
	protected void procReqFRASettlement(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
				throws ServletException, IOException {
		...
		UserPos	userPO = null;	
		boolean IsNotError = false;
flexLog("JSETR0150-->procReqFRASettlement");
		try {
				msgError = new ELEERRMessage();
			} 
		catch (Exception ex) {
	...
	//		msgFRA.setH01FLGWK2("2");
			...
			try {                                     
				...
				try {   
					msgFRA.setE01FRAACC(userPO.getAccOpt());      
				}
            		
				catch (Exception e1)	{ msgFRA.setE01FRAACC(""); }
			}
		...
		try
		{
			...
			if (newmessage.getFormatName().equals("ETR0150DS")) {
				...
				ses.setAttribute("error", msgError);
				ses.setAttribute("fra", msgFRA);
				if (!msgFRA.getE01FRAACC().equals(""))	
					userPO.setAccOpt(msgFRA.getE01FRAACC());
				//	userPO.setE01FRAACC(msgFRA.getE01FRAACC());
				ses.setAttribute("userPO", userPO);			 
				if (IsNotError) {  // There are no errors
						try {					
...
protected void procReqSwift100FRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	UserPos	userPO = null;	
	boolean IsNotError = false;
flexLog("JSETR0150-->procReqSwift100FRA");
...
protected void procActionSwift100FRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	UserPos	userPO = null;	
	boolean IsNotError = false;
flexLog("JSETR0150-->procActionSwift100FRA");	
	try {
		msgError = new ELEERRMessage();
	} 
	catch (Exception ex) {
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ETR0150", req));
			...
			switch (screen) {
				...
				case A_FRA_MOD :
					 ...
					 break;		
				case A_FRA_TCK :
					 procActionFRATck(msgUser, req, res, session);
					 break;	
			...
			// END CD
				...
				case R_ENTER_FRA :
					 ...
					 break;						
				case R_FRA_STL :				 
				     procReqFRASettlement(mc, msgUser, req, res, session);
					 break;	
				case A_FRA_STL :				 
					 procActionFRASettlement(mc, msgUser, req, res, session);
					 break;						 
				case R_S_FRA : 
