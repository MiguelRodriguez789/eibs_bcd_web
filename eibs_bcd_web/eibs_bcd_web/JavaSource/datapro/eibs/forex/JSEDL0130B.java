...
public class JSEDL0130B extends datapro.eibs.master.SuperServlet {
	...
	if (session == null) {
		...
		try {
			...
			try { 
				mc = new MessageContext(super.getMessageHandler(target, req));
				...
				case 1 :
					{
						...
						try {
							...
							while (pmntList.getNextRow()) {
								...
								try {
									msgPmnt.setE04DLPPDD(pmntList.getRecord(1));
									//Date
								} catch (Exception e) {
								}

								try {
									msgPmnt.setE04DLPPDM(pmntList.getRecord(2));
									//Date
								} catch (Exception e) {
								}

								try {
									msgPmnt.setE04DLPPDY(pmntList.getRecord(3));
									//Date
								} catch (Exception e) {
								}
