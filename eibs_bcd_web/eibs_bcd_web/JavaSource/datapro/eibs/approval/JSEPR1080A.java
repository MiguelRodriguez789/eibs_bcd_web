...
public class JSEPR1080A extends JSEIBSServlet {
	...
	private void sortDate(HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		if (Approv.equals("A")) {
			...
			if ("BANK".equals(getUserPos(req).getSource())) {
	...
	private void procReqConsultaSwift(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		try {
			...
			if (req.getParameter("TRANSREFNUM") != null)
				msg.setE01SWOACC(req.getParameter("TRANSREFNUM"));
			else {
				try {
					if (userPO.getHeader23().trim().length() > 0) {
						msg.setE01SWOACC(userPO.getHeader23()); }
					else {msg.setE01SWOACC(userPO.getIdentifier());}; 
				} catch (Exception e) {
					try {
			...
			JBObjList msglist = mp.receiveMessageRecordList("E01INDOPE");
			JBList list = new JBList();
			
			if (!msglist.isEmpty()) {
				StringBuffer myRow = null;
				while (msglist.getNextRow()) {
					msg = (EPR040001Message) msglist.getRecord();
					myRow = new StringBuffer("<TR>");
					myRow.append("<TD NOWRAP>" + msg.getE01SWOTXT() + " </TD>");
					list.addRow("", myRow.toString());
				}
			}
		...
		try {
			...
			if (mp.hasError(list)) {
				...
				while (list.getNextRow()) {
					...
					chkOfac = (msg.getH01FLGWK3().equals("3") ? "<a href=\"javascript:showInqOFAC('" + msg.getE01PRINUM() + "')\"><img src=\"" + req.getContextPath() + "/images/warning_16.jpg\" title=\"OFAC Posible Coincidencia\" align=\"middle\" border=\"0\" ></a>" : "");					
					myRow = new StringBuffer("<TR>");
					...
					myRow.append("<INPUT TYPE=HIDDEN NAME=\"TXTDATA"+indexRow+"\" VALUE=\"" + Util.formatCustomDate(user.getE01DTF(), msg.getE01PRITDM(),msg.getE01PRITDD(),msg.getE01PRITDY()) + "<br>");
		...
		try {
			...
			if (mp.hasError(list)) {
				...
				while (list.getNextRow()) {
					...
					chkOfac = (msg.getH01FLGWK3().equals("3") ? "<a href=\"javascript:showInqOFAC('" + msg.getE01PRINUM() + "')\"><img src=\"" + req.getContextPath() +"/images/warning_16.jpg\" title=\"OFAC Posible Coincidencia\" align=\"middle\" border=\"0\" ></a>" : "");					
					myRow = new StringBuffer("<TR>");
					myRow.append("<TD NOWRAP WITH=\"5%\"><input type=\"radio\" name=\"REFNUM\" value=\"" + msg.getE01PRINUM() + "\" " + chk + " onclick=\"showAddInfo('"+ msg.getE01PRINUM() +"',"+indexRow+")\"></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"RIGHT\" WITH=\"15%\"><A HREF=\"javascript:showInqApprovalPR('"+ msg.getE01PRINUM() +"')\">" + Util.formatCell(msg.getE01PRINUM()) + "</A>" + chkOfac +"</TD>");
					myRow.append("<TD NOWRAP ALIGN=\"RIGHT\" WITH=\"15%\"><A HREF=\"javascript:showInqApprovalPR('"+ msg.getE01PRINUM() +"')\">" + Util.formatCell(msg.getE01PRIOCU()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"left\" WITH=\"30%\"><A HREF=\"javascript:showInqApprovalPR('"+ msg.getE01PRINUM() +"')\">" + Util.formatCell(msg.getE01CUSNA1()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=\"left\" WITH=\"35%\"><A HREF=\"javascript:showInqApprovalPR('"+ msg.getE01PRINUM() +"')\">" + Util.formatCell(msg.getE01REMARK()) + "</A>");
					myRow.append("<INPUT TYPE=HIDDEN NAME=\"TXTDATA"+indexRow+"\" VALUE=\"" + Util.formatCustomDate(user.getE01DTF(), msg.getE01PRITDM(),msg.getE01PRITDD(),msg.getE01PRITDY()) + "<br>");
					myRow.append(Util.formatCell(msg.getE01PRIDAC()) + "<br>");
					myRow.append(Util.fcolorCCY(msg.getE01PRIAMT()) + "<br>");
					myRow.append(Util.formatCell(msg.getE01PRICCY()) + "<br>");
