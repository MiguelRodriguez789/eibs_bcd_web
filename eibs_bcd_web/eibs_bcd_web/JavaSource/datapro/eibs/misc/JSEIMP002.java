...
import datapro.eibs.beans.EIMP00002Message;
...
public class JSEIMP002 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqImport(
		...
		try {
			...
			callPage(LangPath + "EIMP000_enter_file.jsp", req, res);
		} catch (Exception e) {
	...
	protected void procActionImport(
		...
		EIMP00002Message msgEPR = null;
		EIMP00002Message msgEND = null;
		ELEERRMessage msgError = null;
		UserPos userPO = null;
		...
		try {
			...
			while (true) {
				...
				if (line.length() > 0) {
					...
					if (firstTime) {
						firstTime = false;
						msgEPR = (EIMP00002Message) mc.getMessageRecord("EIMP00002");
						msgEPR.setH02USERID(user.getH01USR());
						msgEPR.setH02PROGRM("EIMP000");
						msgEPR.setH02TIMSYS(getTimeStamp());
						msgEPR.setH02OPECOD("0001");
						msgEPR.setH02FLGWK1("");
						msgEPR.setE02DATA(linetosend);
						msgEPR.send();
						...
						msgEPR.setH02PROGRM("EIMP000");
						msgEPR.setH02TIMSYS(getTimeStamp());
						msgEPR.setH02OPECOD("0002");							
						msgEPR.setH02FLGWK1("S");
						msgEPR.setE02DATA(linetosend);
						msgEPR.send();
						//mc.sendMessage(msgEPR);
			...
			if (IsNotError) {
				msgEND = (EIMP00002Message) mc.getMessageRecord("EIMP00002");
				msgEND.setH02USERID(user.getH01USR());
				msgEND.setH02PROGRM("EIMP000");
				msgEND.setH02TIMSYS(getTimeStamp());
				msgEND.setH02OPECOD("0002");
				msgEND.setH02FLGWK1("*");
				msgEND.setE02DATA("");
				msgEND.send();
				//mc.sendMessage(msgEPR);
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EIMP000", req));
