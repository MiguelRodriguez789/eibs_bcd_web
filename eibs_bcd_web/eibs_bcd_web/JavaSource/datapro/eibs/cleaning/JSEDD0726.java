...
public class JSEDD0726 extends datapro.eibs.master.SuperServlet {
	...
	if (session == null) {
		...
		try {
			...
			if (screen == R_ENTER) {
				...
				mc = new MessageContext(super.getMessageHandler("EDD0726", req));
...
protected void  procReqEnter(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		flexLog("About to call Page: " + LangPath + "EDD0726_enter_cleaning_applay.jsp");
		callPage(LangPath + "EDD0726_enter_cleaning_applay.jsp", req, res);	
	}
	catch (Exception e) {
...
protected void procActionEnter(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	EDD072601Message msgAPP = null;
	EDD072602Message msgList = null;
	...
	try
	{
		...
		flexLog("Send Initial Data");
	 	msgAPP = (EDD072601Message)mc.getMessageRecord("EDD072601");
		msgAPP.setH01USERID(user.getH01USR());
	 	msgAPP.setH01PROGRM("EDD0726");
	...
	// Receive Data
	  ...
	  if (newmessage.getFormatName().equals("EDD072602")) {
			...
			while (true) {
				...
				msgList = (EDD072602Message)newmessage;
			...
			if (IsNotError) {  // There are no errors
				...
				try {
					flexLog("About to call Page: " + LangPath + "EDD0726_cleaning_applay_list.jsp");
					callPage(LangPath + "EDD0726_cleaning_applay_list.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
 ...
 * This method was created in VisualAge.
 */
protected ELEERRMessage procSendData(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, EDD072602Message msgList)
			throws ServletException, IOException {
	
	ELEERRMessage msgError = null;
	EDD072603Message msgAPP = null;
	MessageRecord newmessage = null;
	
	try {
	  		msgAPP = (EDD072603Message)mc.getMessageRecord("EDD072603");
	...
	try
	{
		...
		while (i <= chkList.getLastRec()) {
			...
			if (chkList.getRecord().equals("checked")) { //selected
				...
				msgList = (EDD072602Message) beanList.getRecord();
		...
		if (IsNotError) {  // There are no errors
				...
				try {
					flexLog("About to call Page: " + LangPath + "EDD0726_cleaning_applay_confirm.jsp");
					callPage(LangPath + "EDD0726_cleaning_applay_confirm.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
