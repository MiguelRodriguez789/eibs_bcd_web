...
public class JSEDD0102 extends datapro.eibs.master.SuperServlet {
...
public void destroy() {
	...
	flexLog("free resources used by JSEDD0102");
...
protected void procReqControl(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		flexLog("Send Initial Data");
	 	msgCTRL = (EDD010201Message)mc.getMessageRecord("EDD010201");
		msgCTRL.setH01USERID(user.getH01USR());
	 	msgCTRL.setH01PROGRM("EDD0102");
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD010201")) {
			
			msgCTRL = (EDD010201Message)newmessage;
			...
			if (IsNotError) {  // There are no errors
				...
				try {
					flexLog("About to call Page: " + LangPath + "EDD0102_uncoll_release.jsp");
					callPage(LangPath + "EDD0102_uncoll_release.jsp", req, res);		
				}
				catch (Exception e) {
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EDD0102", req));
...
protected void procActionControl(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD010201")) {
			...
			if (IsNotError) {  // There are no errors
				try {
					flexLog("About to call Page: " + LangPath + "EDD0102_uncoll_release_confirm.jsp");
					callPage(LangPath + "EDD0102_uncoll_release_confirm.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
