...
public class JSEACH410 extends JSEIBSServlet { 
	...
	protected void processRequest(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, 
		...
		if (userPO.getPurpose().equals("GENERATE") && screen !=1) {
			screen = 5;
		}
	...
	protected void procActionFile(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			msg.setH01FLGWK1("");
			try {
				msg.setE01DATA(req.getParameter("E01DATA") + req.getParameter("E01NACHA"));
			} catch (Exception e) {
				msg.setE01DATA("ALL");
			}
			// Send Message
			...
			if (mp.hasError(newmessage)) {
				...
				while (true) {
					...
					if (marker.equals("*")) {
						...
						sTmp = msg.getE01DATA();
						if (sTmp.length() < achLen) {
							for (int i=msg.getE01DATA().length(); i < achLen; i++) {
								sTmp = sTmp + " ";
							}
						}
						buf.append(sTmp + "\r\n");
