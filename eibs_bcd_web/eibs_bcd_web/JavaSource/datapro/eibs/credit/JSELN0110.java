...
protected void procReqEnterCust(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		...
		try {
			userPO.setCusType(req.getParameter("OPTION"));
		} catch (Exception e) {
			userPO.setCusType("");
		}
		userPO.setPurpose("");
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					myRow.append("<TD NOWRAP ALIGN=CENTER><A HREF=\"javascript:showInqAcc('" + acc + "')\">" 
							+ Util.formatCustomDate(user.getE01DTF(),
									msgList.getBigDecimalE03OPROPM().intValue(),
									msgList.getBigDecimalE03OPROPD().intValue(),
									msgList.getBigDecimalE03OPROPY().intValue()) + "</A></TD>");
					myRow.append("<TD NOWRAP ALIGN=CENTER><A HREF=\"javascript:showInqAcc('" + acc + "')\">" 
							+ Util.formatCustomDate(user.getE01DTF(),
									msgList.getBigDecimalE03OPRMTM().intValue(),
									msgList.getBigDecimalE03OPRMTD().intValue(),
									msgList.getBigDecimalE03OPRMTY().intValue()) + "</A></TD>");
...
protected void procReqListByCust(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		msgList.setH01USERID(user.getH01USR());
	 	...
	 	try {
	 		msgList.setH01FLGWK1(userPO.getCusType());
	 	} catch (Exception e) {
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					if (firstTime) {
						...
						chk = "checked  getParams('" + msgList.getE01LNECUN() + "', '" + msgList.getE01LNENUM() + "', '" + msgList.getE01LNETYL() + "')\"";
					}
					else {
						chk = "";
					}
					...
					myRow = new StringBuffer("<TR>");
					myRow.append("<TD NOWRAP><input type=\"radio\" name=\"CUS_LINE\" id=\"CUS_LINE\" value=\"" + msgList.getE01LNECUN() + "_" + msgList.getE01LNENUM() + "\" " + chk + " onclick=\"getParams('" + msgList.getE01LNECUN() + "', '" + msgList.getE01LNENUM() + "', '" + msgList.getE01LNETYL() + "'" + ")\"></TD>");
					...
					myRow.append("<TD NOWRAP ALIGN=CENTER>" 
							+ Util.formatCustomDate(user.getE01DTF(),
									msgList.getBigDecimalE01LNEMTM().intValue(),
									msgList.getBigDecimalE01LNEMTD().intValue(),
									msgList.getBigDecimalE01LNEMTY().intValue()) + "</A></TD>");
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					myRow.append("<TD NOWRAP ALIGN=CENTER>"
							+ Util.formatCustomDate(user.getE01DTF(),
									msgList.getBigDecimalE04LNEMTM().intValue(),
									msgList.getBigDecimalE04LNEMTD().intValue(),
									msgList.getBigDecimalE04LNEMTY().intValue()) + "</A></TD>");
