...
import datapro.eibs.beans.EPC008501Message;
...
public class JSEPC0085 extends datapro.eibs.master.SuperServlet {
	...
	public void destroy() {
		...
		flexLog("free resources used by JSEPC0085");
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EPC0085", req));
	...
	protected void procReqNew(
		...
		// Send Initial data
		try {
			msgPC = (EPC008501Message) mc.getMessageRecord("EPC008501");
			msgPC.setH01USERID(user.getH01USR());
			msgPC.setH01PROGRM("EPC0085");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EPC008501")) {
				try {
					msgPC = new EPC008501Message();
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgPC = (EPC008501Message) newmessage;
				...
				if (IsNotError) { // There are no errors 
					try {
						flexLog(
							"About to call Page3: "
								+ LangPath
								+ "EPC0085_pc_opening.jsp");
						callPage(LangPath + "EPC0085_pc_opening.jsp", req, res);
					} catch (Exception e) {
	...
	protected void procActionNew(
		...
		try {
			...
			msgPC.destroy();
			flexLog("EPC008501 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
			throw new RuntimeException("Socket Communication Error");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EPC008501")) {
				try {
					msgPC = new EPC008501Message();
					flexLog("EPC008501 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0085_pc_maint.jsp");
						callPage(LangPath + "EPC0085_pc_maint.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				} else { // There are errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0085_pc_enter_maint.jsp");
						callPage(
							LangPath + "EPC0085_pc_enter_maint.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0085_pc_maint_sch.jsp");
						callPage(LangPath + "EPC0085_pc_maint_sch.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				} else { // There are errors
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0085_pc_maint_sch_inq.jsp");
						callPage(LangPath + "EPC0085_pc_maint_sch_inq.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				} else { // There are errors
					try {
						flexLog(
							"About to call Page: "
								...
								+ "EPC0085_pc_maint_sch_inq.jsp");
						callPage(
							LangPath + "EPC0085_pc_maint_sch_inq.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
	...
	protected void procActionMaintenance(
		...
		try {
			flexLog("Send Initial Data");
			msgPC = (EPC008501Message) ses.getAttribute("pcMant");
			//msgCD = (EPC008501Message)mc.getMessageRecord("EPC008501");
			msgPC.setH01USERID(user.getH01USR());
			msgPC.setH01PROGRM("EPC0085");
			msgPC.setH01TIMSYS(getTimeStamp());
			msgPC.setH01SCRCOD("01");
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EPC0085_pc_enter_maint.jsp");
			callPage(LangPath + "EPC0085_pc_enter_maint.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
...
protected void procReqEnterInquiry(
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EPC0085_pc_enter_inquiry.jsp");
			callPage(LangPath + "EPC0085_pc_enter_inquiry.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
