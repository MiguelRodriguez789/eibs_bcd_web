...
public class JSESD0940 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqProductChangeListDelete(
		...
		if (req.getParameter("ACD")!=null){
			userPO.setHeader9(req.getParameter("ACD"));
		}
...
// NEW RG
	...
	protected void procReqProductChangeListApprov(
		...
		try {
			msgError =
				(datapro.eibs.beans.ELEERRMessage) Beans.instantiate(getClass().getClassLoader(),"datapro.eibs.beans.ELEERRMessage");
		} catch (Exception ex) {
			flexLog("Error: " + ex);
		}
		...
		// Send Initial data
		try {
			JBObjList list = (JBObjList)ses.getAttribute("ESD094001Help");
			int index = Integer.parseInt(req.getParameter("CURRCODE").trim());
			
			ESD094001Message listMessage = (ESD094001Message)list.get(index);
			
			ESD094001Message msgList = (ESD094001Message) mc.getMessageRecord("ESD094001");
			...
			msgList.setE01CHGACD(listMessage.getE01CHGACD());
			msgList.setE01CHGACC(listMessage.getE01CHGACC());
			msgList.setE01CHGTYP(listMessage.getE01CHGTYP());
			msgList.send();
			msgList.destroy();
			flexLog("ESD094001 Message Sent");
		} catch (Exception e) {
		...
		try {
			...
			if (newmessage.getFormatName().equals("ESD094001")) {
				...
				ESD094001Message msgList = (ESD094001Message) newmessage;
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ESD0940", req));
