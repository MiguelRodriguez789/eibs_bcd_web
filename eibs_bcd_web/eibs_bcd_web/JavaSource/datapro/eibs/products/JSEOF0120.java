...
public class JSEOF0120 extends datapro.eibs.master.SuperServlet {
	...
	protected static final int R_ENTER_CANCEL_IMP  	= 18;
	protected static final int R_DET_CHK_CANCEL_IMP	= 22;
	...
	protected  void procReqChkEnterCancel(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses, int screen)
		throws ServletException, IOException {
			...
			userPO.setHeader1("11");
			try {
				if (screen == 18) {
					userPO.setHeader1("24");}
	...
	protected  void procReqChkDetCancel(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
		...
		try
		{
			...
			if (!userPO.getHeader1().equals("11"))
				msgDV.setH01FLGWK1("P"); // anular impresion
		...
		try {
			...
			if (newmessage.getFormatName().equals("EOF012001")) {
				...
				if (IsNotError) { // There are no errors
					...
					try {
						userPO.setRedirect(srctx + "/servlet/datapro.eibs.products.JSEOF0120");
						flexLog(
							"About to call Page: "
								+ LangPath
	...
	protected void procActionChkCancel(
		...
		try {
			...
			tipo_cancel = userPO.getHeader1();
			if (tipo_cancel == "24") {
				msgDeal.setH01OPECOD("0006");
			}else {
			...
			if (session == null) {
				...
				try {
					...
					try {
						...
						mc = new MessageContext(super.getMessageHandler("EOF0120", req));
					} catch (Exception e) {
						e.printStackTrace();
					...
					switch (screen) {
						...
						case R_CHK_CANCEL :
							...
							break;
						case R_ENTER_CANCEL :
						case R_ENTER_CANCEL_IMP:	
							procReqChkEnterCancel(mc, msgUser, req, res, session, screen);
							break;
