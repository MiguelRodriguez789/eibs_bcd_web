...
public class JSEDD0430 extends datapro.eibs.master.SuperServlet {
	...
	// Corresponsales
	protected static final int R_ENTER_CBP = 1000;
		...
		try {
			...
			switch (option) {
				...
				case 5 : // Overdrf with Loan
					...
					break;
				case 8 :
					procReqRTOverd08(mc, user, req, res, ses);
					break;
				default :
					res.sendRedirect(super.srctx +LangPath + "Under_construction.jsp");
			}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDD0430", req));
					...
					switch (screen) {
						...
						case R_ENTER_ACC :
							...
							break;
						case R_ENTER_CBP :
							procReqEnterCbp(msgUser, req, res, session);
							break;
						case R_ENTER_SV_ACC :
	...
	protected void procReqRTOverd08(
			MessageContext mc,
			ESS0030DSMessage user,
			HttpServletRequest req,
			HttpServletResponse res,
	...
	protected void procReqEnterCbp(
			ESS0030DSMessage user,
			HttpServletRequest req,
			HttpServletResponse res,
			HttpSession ses)
