...
public class JSEPC0130 extends datapro.eibs.master.SuperServlet {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "EPC0130_st_enter_stat.jsp");
					callPage(LangPath + "EPC0130_st_enter_stat.jsp", req, res);
				} catch (Exception e) {
	...
	protected void procReqList(
		...
		MessageRecord newmessage = null;
		ELEERRMessage msgError = null;
		EPC013001Message msgSearch = null;
		EPC013001Message msgList = null;
		EPC013002Message msgHeader = null;
		...
		try {
			...
			msgSearch = (EPC013001Message) mc.getMessageRecord("EPC013001");
			msgSearch.setH01USERID(user.getH01USR());
			msgSearch.setH01PROGRM("EPC0130");
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "EPC0130_st_selection.jsp");
					callPage(LangPath + "EPC0130_st_selection.jsp", req, res);

				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}

			} else if (newmessage.getFormatName().equals("EPC013002")) {
				try {
					msgHeader = new datapro.eibs.beans.EPC013002Message();
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgHeader = (EPC013002Message) newmessage;

				flexLog("Putting java beans into the session");
				ses.setAttribute("pcMant", msgHeader);

				newmessage = mc.receiveMessage();

				if (newmessage.getFormatName().equals("EPC013001")) {
					...
					while (true) {
						...
						msgList = (EPC013001Message) newmessage;
...
//							}
					...
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0130_st_list_fp.jsp");
						callPage(LangPath + "EPC0130_st_list_fp.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				...
				if (newmessage.getFormatName().equals("EPC013001")) {
					...
					try {
						...
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EPC0130_st_list_print_fp.jsp");
						callPage(
							LangPath + "EPC0130_st_list_print_fp.jsp",
							req,
							res);
	...
	protected void procReqSearch(
		...
		try {
			flexLog(
				"About to call Page: " + LangPath + "EPC0130_st_selection.jsp");
			callPage(LangPath + "EPC0130_st_selection.jsp", req, res);
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Exception calling page " + e);
	...
	protected void procReqSTEnterSearch(
		...
		throws ServletException, IOException {
			...
			try {
				...
				userPO.setRedirect("/servlet/datapro.eibs.client.JSEPC0130?SCREEN=200");
	...
	protected void procReqSTEnterSearchAcc(
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EPC0130_st_enter_stat_acc.jsp");
			callPage(LangPath + "EPC0130_st_enter_stat_acc.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EPC0130", req));
