...
protected void procReqEnterHoldUncollRT(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		...
		try {
			flexLog("About to call Page: " + getLangPath(user) + "GENERIC_account_enter.jsp");
			callPage(getLangPath(user) + "GENERIC_account_enter.jsp", req, res);
		} catch (Exception e) {
...
protected void procReqHoldUncollList(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
	  	...
	  	else if (newmessage.getFormatName().equals("EDD030002")) {
			...
			while (true) {
				...
				msgHead = (EDD030002Message)newmessage;

				marker = msgList.getH01FLGMAS();

				if (firstTime) {
					firstTime = false;
					userPO.setCurrency(msgHead.getE02UNCCCY());
					userPO.setHeader1(msgHead.getE02UNCCUN());
					userPO.setCusNum(msgHead.getE02UNCCUN());
					userPO.setHeader5(msgHead.getE02CUSNA1());
					userPO.setCusName(msgHead.getE02CUSNA1());
					userPO.setHeader6(msgHead.getE02UNCPRO());
					userPO.setProdCode(msgHead.getE02UNCPRO());
					userPO.setHeader10(msgHead.getE02UNCBNK());
					userPO.setBank(msgHead.getE02UNCBNK());
					userPO.setHeader11(msgHead.getE02UNCBRN());
					userPO.setBranch(msgHead.getE02UNCBRN());
				...
				if (marker.equals("*")) {
					...
					myRow[2] =   msgList.getE01UNCRDD();	// Proccess Date 1
					myRow[3] =   msgList.getE01UNCRDM();	// Proccess Date 2
					myRow[4] =   msgList.getE01UNCRDY();	// Proccess Date 3
					myRow[5] =   msgList.getE01UNCMDD();	// Maturity Date 1
					myRow[6] =   msgList.getE01UNCMDM();	// Maturity Date 2
					myRow[7] =   msgList.getE01UNCMDY();	// Maturity Date 3
			...
			try {
				flexLog("About to call Page: " + getLangPath(user) + "EDD0300_hold_uncollected.jsp");
				callPage(getLangPath(user) + "EDD0300_hold_uncollected.jsp", req, res);
