...
public class JSESD0900 extends datapro.eibs.master.SuperServlet {
	...
	protected static final int R_ENTER_ACCOUNT = 10;
	protected static final int A_ENTER_ACCOUNT = 20;
	...
	protected void procReqProductChangeEnter(
		...
		try {
			callPage(LangPath + "ESD0900_product_change_account_enter.jsp", req, res);
		} catch (Exception e) {
	...
	protected void procReqProductChangeAccountEnter(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
	...
	protected void procActionProductChangeAccountEnter(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
			...
			if (newmessage.getFormatName().equals("ESD090001")) {
				...
				//
					if (IsNotError) { // There are no errors
						...
						try {
							flexLog("About to call Page: " + LangPath + "ESD0900_product_change_account_enter.jsp");
							callPage(LangPath + "ESD0900_product_change_account_enter.jsp", req, res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
	...
	protected void procActionPos(
		...
		switch (inptOPT) {
			case 0 : //New Individual Account Change
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.products.JSESD0900?SCREEN=10&changeType=A");
				break;
	...
	protected void procReqProductChangeListDelete(
		...
		try {
			...
			msgList.setE01CHGACD(userPO.getHeader9());
	...
	protected void procActionMaintenance(
		...
		if (changeType.equals("A")){
			if (req.getParameter("E01CHGACD") != null) {
				userPO.setHeader9(req.getParameter("E01CHGACD"));
			}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ESD0900", req));
					...
					switch (screen) {
						...
						case R_ENTER_ACCOUNT :
							procReqProductChangeAccountEnter(mc, msgUser, req, res, session);
							break;
						case A_ENTER_ACCOUNT :
							procActionProductChangeAccountEnter(mc, msgUser, req, res, session);
							break;
						case R_PRODUCT_LIST :
