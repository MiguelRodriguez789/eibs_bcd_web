...
public class JSECD0006 extends datapro.eibs.master.SuperServlet {
	...
	public void destroy() {
		...
		flexLog("free resources used by JSECD0006");
	...
	protected void procReqList(
		...
		// Send Initial data
		try {
			msgList = (ECD0006DSMessage) mc.getMessageRecord("ECD0006DS");
			msgList.setH06USERID(user.getH01USR());
			msgList.setH06PROGRM("ECD0006");
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog("About to call Page: " + LangPath + "ECD0006_plastic_stock_list.jsp");
					callPage(LangPath + "ECD0006_plastic_stock_list.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}

			} else if (newmessage.getFormatName().equals("ECD0006DS")) {
					appList = new JBObjList();
					boolean firstTime = true;
					String marker = "";

					while (true) {
						msgList = (ECD0006DSMessage) newmessage;
	...
	protected void procReqNew(
		...
		try {
			...
			try {
				flexLog("About to call Page: " + LangPath + "ECD0006_plastic_stock_new.jsp");
				callPage(LangPath + "ECD0006_plastic_stock_new.jsp", req, res);
			} catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
	...
	protected void procReqMaintenance(
		...
		try {
			...
			msgCD = (ECD0006DSMessage) appList.getRecord();
			...
			try {
				flexLog("About to call Page: " + LangPath + "ECD0006_plastic_stock_maint.jsp");
				callPage(LangPath + "ECD0006_plastic_stock_maint.jsp", req, res);
			} catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
	...
	protected void procReqInquiry(
		...
		try {
			...
			try {
				flexLog("About to call Page: " + LangPath + "ECD0006_plastic_stock_inq.jsp");
				callPage(LangPath + "ECD0006_plastic_stock_inq.jsp", req, res);
			} catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
	...
	protected void procActionMaintenance(
		...
		try {
			flexLog("Send Initial Data");
			msgCD = (ECD0006DSMessage) mc.getMessageRecord("ECD0006DS");
			msgCD.setH06USERID(user.getH01USR());
			msgCD.setH06PROGRM("ECD0006");
			msgCD.setH06TIMSYS(getTimeStamp());
			msgCD.setH06SCRCOD("01");
			msgCD.setH06OPECOD(opecode);
		...
		try {
			...
			if (newmessage.getFormatName().equals("ECD0006DS")) {
				msgCD = (ECD0006DSMessage) newmessage;

				if (IsNotError) { // There are no errors
					try {
						res.sendRedirect(super.srctx
											+ "/servlet/datapro.eibs.products.JSECD0006?SCREEN=1");
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ECD0006", req));
