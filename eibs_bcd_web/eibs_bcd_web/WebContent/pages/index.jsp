...
function OpenWin(lang_path) {
   ...
   var name = 'eibs_' + sessionId;
   if (checkCookie("eibs")) {
   		deleteCookie("eibs");
   }
   setCookie("eibs", sessionId, 1);
   var newWindow=window.open('<%=request.getContextPath()%>/pages/' +lang_path +'/ESS0030_LogIn.jsp',name,'status=yes,toolbar=no,location=no,directories=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,height=150,width=300,'+position)
