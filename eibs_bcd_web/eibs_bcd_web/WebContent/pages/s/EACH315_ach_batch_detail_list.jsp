...
function goSearch() {
	...
	bth = document.getElementById("E01ACUBTH").value;
	num = document.getElementById("SEARCHCDE").value;
	pg = "<%=request.getContextPath()%>/servlet/datapro.eibs.ach.JSEACH315?SCREEN=1&BATCH=" + bth + "&FromRecord=0&SEARCHCDE=" + num;
...
function showOfac(fieldValue){
     <% if(isApproval) { %>
		page = "${pageContext.request.contextPath}/servlet/datapro.eibs.helps.JSEWD0092?shrAcc=" + fieldValue + "&shrCategory=ALL" + "&FromRecord=0&shrAction=APV";
...
<h3 align="center">Transacciones para el Archivo ACH Nro. <%= userPO.getIdentifier().trim() %><img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" 
	name="EIBS_GIF" title="ach_batches_list.jsp, EACH315"></h3>
<hr size="4">
...
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.ach.JSEACH315">
  ...
  <INPUT TYPE="HIDDEN" id="E01ACUBTH" name="E01ACUBTH" value="<%= userPO.getIdentifier().trim() %>">
  <INPUT TYPE=HIDDEN id="E01ACUNUM" NAME="E01ACUNUM" VALUE="<%= userPO.getProdCode().trim() %>">
...
%>
	<TR>
		<TD align="CENTER" class="TDBKG" width="25%">
			<a href="javascript:goNew()"><b>Crear<br>Transacción</b></a>
      	</TD>
		<TD align="CENTER" class="TDBKG" width="25%">
...
<%	} else { %>
	<TR>
		...
		<%	} else { %>
			<a href="<%=request.getContextPath()%>/servlet/datapro.eibs.ach.JSEACH310?SCREEN=11"><b>Salir</b></a>
		<%  } %>	
      	</TD>
	</TR>
<% 	} %>
  ...
  <TABLE class="tableinfo" id="dataTable">
    ...
    <TR id=trdark>
		...
		<td NOWRAP align="center" width="10%">
			<INPUT type="text" id="SEARCHCDE" name="SEARCHCDE" size="13" maxlength="12" value="<%= userPO.getProdCode() %>">
        ...
        <%
          ...
          while (EACH315List.getNextRow()) {
			...
			if ((msgList.getH01FLGWK2().trim().equals("W")) && !(msgList.getH01FLGWK3().trim().equals("3"))) {
				warnImg= "<IMG border=\"0\" src=\"../images/warning01.gif\" title=\"Advertencias\" onClick=\"showWarnings('" + msgList.getE01ACUNUM() + "','ROW')\">";
			...
			if (msgList.getH01FLGWK3().trim().equals("3")) {
					ofacImg= "<IMG border=\"0\" src=\"../images/warning_16.jpg\" title=\"OFAC Match List\" onClick=\"showOfac('" + msgList.getE01ACUNUM() + "')\">";
      ...
      <TD WIDTH="50%" ALIGN=LEFT height="25">
       <%
    	if ( EACH315List.getShowPrev() ) {
  			...
  			out.print("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.ach.JSEACH315?SCREEN=1&BATCH=" + userPO.getIdentifier() + "&FromRecord=" + pos + "&SEARCHCDE=" + userPO.getProdCode() + "\" > <img src=\""+request.getContextPath()+"/images/s/previous_records.gif\" border=0></A>");
    	} %>
      </TD>
 	  <TD WIDTH="50%" ALIGN=RIGHT height="25"> 
    	...
    	if (EACH315List.getShowNext()) {
  			...
  			out.print("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.ach.JSEACH315?SCREEN=1&BATCH=" + userPO.getIdentifier() + "&FromRecord=" + pos + "&SEARCHCDE=" + userPO.getProdCode() + "\" ><img src=\""+request.getContextPath()+"/images/s/next_records.gif\" border=0></A>");
	    } %>
  </TD>
 </TR>
 </TABLE>
