       ...
       if (opt == "9") { //Delete
     	 ...
     	 ok = confirm("Esta seguro que desea eliminar el registro seleccionado?");
	  	 if (ok) document.forms[0].submit();
	        ...
	        page= "${pageContext.request.contextPath}/pages/s/EDD0000_rt_condit_signers_maint.jsp?OPTION=" + opt + "&CCY=" + document.forms[0].E01DEACCY.value;
	  		if (opt == "2") { //Maint	
...
<BODY>
 ...
 <img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="rt_condit_signers_list.jsp, EDD0000"> 
    </h3>
   <hr size="4">
...
<%
	if (!userPO.getPurpose().equals("APPROVAL_INQ") ) {
%>
                <INPUT TYPE="radio" NAME="ROW" VALUE="<%= signersList.getCurrentRow() %>" <%=chk%> onclick="setInfo('<%= signersList.getCurrentRow()%>','<%= dds.getE01SQN()%>','<%= dds.getE01NM1()%>','<%= dds.getE01CS1()%>','<%= dds.getE01CS2()%>','<%= dds.getE01CS3()%>','<%= dds.getE01CCY()%>')">
<%
	}
%>
              </TD>
