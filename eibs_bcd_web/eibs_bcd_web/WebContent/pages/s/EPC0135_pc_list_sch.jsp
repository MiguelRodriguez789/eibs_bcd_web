...
<HTML>
<HEAD>
<TITLE>Créditos Asociados a Proyecto</TITLE>
...
function goAction(op) {
 var delok= false;
 var page="";
 var pres = document.forms[0].PRES.value;
 if (op == 1) {
     page = '${pageContext.request.contextPath}/pages/s/EPC0135_pc_new_sch.jsp?LASTROW=' + document.forms[0].LASTROW.value;
 } else if (op == 2) {
   		 page = "${pageContext.request.contextPath}/servlet/datapro.eibs.products.JSEXEDL0160?SCREEN=200&PRES=" + pres;
 } else if (op == 4) {     
     page = '${pageContext.request.contextPath}/pages/s/EPC0135_pc_delete_sch.jsp?ROW=' + document.forms[0].ROW.value; 
 } else if (op == 5) {     
     page = '${pageContext.request.contextPath}/pages/s/EPC0135_pc_sale_sch.jsp?ROW=' + document.forms[0].ROW.value; 
 }
...
<FORM Method="post" Action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEPC0135">
<INPUT TYPE=HIDDEN NAME="SCREEN" VALUE="2">
<INPUT TYPE=HIDDEN NAME="OPT" VALUE="3">
<INPUT TYPE=HIDDEN NAME="ROW" VALUE="0">
<INPUT TYPE=HIDDEN NAME="LASTROW" VALUE="1">

<h3 align="center">Créditos Asociados a Proyecto<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="pc_list_sch.jsp,EPC0135"> 
</h3>
<hr size="4">
 ...
 <TR> 
   <TD NOWRAP valign="top" width="100%">
    ...
    <TR id="trdark"> 
      ...
      <TH ALIGN=CENTER nowrap>Número Contrato</TH>
    ...
    <%
         ...
         while (extList.getNextRow()) {
            EPC013501Message msgPC = (EPC013501Message) extList.getRecord();
            k++;            
     %> 
            <TR> 
              <TD ALIGN=CENTER NOWRAP> 
                <INPUT TYPE="radio" NAME="PRES" VALUE="<%= msgPC.getE01PDEDEA() %>"
                <% if (extList.getCurrentRow() == 0) out.print(" checked");%>>
                 <%-- if (extList.getCurrentRow() == row) out.print(" checked"); %> 
                onClick="showInfo(<%= extList.getCurrentRow() %>,'<%=msgPC.getE01PDESEQ()%>');">
                --%>
             </TD>
              <TD ALIGN=CENTER NOWRAP><%= Util.formatCell(msgPC.getE01PDESEQ()) %></TD>
              <TD ALIGN=CENTER NOWRAP><%= Util.formatCell(msgPC.getE01PDEDEA()) %></TD>
              <TD ALIGN=LEFT NOWRAP><%= Util.formatCell(msgPC.getE01PDEDSC()) %></TD>
              <TD ALIGN=CENTER NOWRAP><%= Util.formatDate(msgPC.getE01PDEOPM(),msgPC.getE01PDEOPD(),msgPC.getE01PDEOPY()) %></TD> 
              <TD ALIGN=CENTER NOWRAP><%= Util.formatCell(msgPC.getE01PDETYP()) %></TD>              
             </TR>
      <%              
		}
    %>
