...
function enterAction(code,desc,id) {
	...
	if(top.top.opener.fieldDesc.trim() !== ""){
  		setField(form[top.top.opener.fieldDesc], desc);
  		selectField(form[top.top.opener.fieldDesc]);
	}
	if(top.top.opener.fieldId.trim() !== ""){
  		setField(form[top.top.opener.fieldId], id);
  		selectField(form[top.top.opener.fieldId]);
	}
	if(top.top.opener.fieldName.trim() !== ""){
  		setField(form[top.top.opener.fieldName], code);
  		selectField(form[top.top.opener.fieldName]);
	}
...
function enter(NameSearch) {
	...
	if (NameSearch.length > 0) {
    	parent.results.window.location.href="<%=request.getContextPath()%>/pages/s/MISC_search_wait.jsp?URL='<%=request.getContextPath()%>/servlet/datapro.eibs.helps.JSEWD0065?NameSearch=" + NameSearch + "@Type=" + Type + "@FromRecord=" + FromRecord + "@CusType=" + CusType + "'";
