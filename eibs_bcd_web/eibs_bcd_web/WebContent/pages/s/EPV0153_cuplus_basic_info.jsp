...
<html>
<head>
<title> Credilinea Basic Information for Inquiry,Maintenance and Creation </title>
...
<%}%> L&iacute;nea Cr&eacute;dito - <%=RecData.getE01DSCPRO()%>
 <img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="cuplus_basic_info.jsp, EPV0153"></h3>
<hr size="4">
			...
			<td nowrap> 
				<table cellspacing="0" cellpadding="2" width="100%" border="0" class="tbhead">
					<tr id="trdark">
						...
						<td nowrap width="10%" > 
							<div align="left">
								<eibsinput:text property="E01DEACUN" name="RecData" eibsType="<%=EibsFields.EIBS_FIELD_TYPE_CUSTOMER %>" readonly="true"/>
				</div>
						</td>
						<td nowrap width="10%" > 
				...
				<table cellpadding=2 cellspacing=0 width="100%" border="0">
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="25%"> 
							...
							<%}else{%>
							<input type="text" name="E01DEATRM" id="E01DEATRM" size="6" maxlength="5" value="<%= RecData.getE01DEATRM().trim()%>" onchange="" readonly>
							<INPUT TYPE=HIDDEN NAME="E01DEATRC" id="E01DEATRC" VALUE="<%=RecData.getE01DEATRC().trim()%>">
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="27%" > 
							<input type="text" name="E01DEAGPD" size="3" maxlength="2" value="<%= RecData.getE01DEAGPD().trim()%>" >
						</td>
					</tr>
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>"> 
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="25%" > 
							<eibsinput:text property="E01DEASPR" name="RecData" eibsType="<%=EibsFields.EIBS_FIELD_TYPE_RATE %>" readonly="true"/>
						</td>
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="25%" > 
							<input type="text" name="E01DEARRP" size="3" maxlength="3" value="<%= RecData.getE01DEARRP().trim()%>">
						...
						<td nowrap width="25%" > 
							<eibsinput:date name="RecData"  fn_month="E01DEARRM" fn_day="E01DEARRD" fn_year="E01DEARRY" /> 
						</td>
					</tr>
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="20%" > 
							<input type="text" size="4" maxlength="3" name="E01DEAIPD" value="<%= RecData.getE01DEAIPD().trim()%>" onKeyPress="enterInteger(event);"> 
						</td>
						<td nowrap width="25%" ><div align="right">Pr&oacute;xima Fecha de Pago :</div></td>
						<td nowrap width="20%" > 
							<eibsinput:date name="RecData"  fn_month="E01DEARDM" fn_day="E01DEARDD" fn_year="E01DEARDY" readonly="false" required="false"/> 
						</td>
					</tr>
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						<td nowrap width="25%" ><div align="right">Ciclo Pago de Capital :</div></td>
						<td nowrap width="20%" > 
							<input type="text" name="E01DEAPPD" size="4" maxlength="3" value="<%= RecData.getE01DEAPPD().trim()%>" onKeyPress="enterInteger(event);">
						</td>
						<td nowrap width="25%" ><div align="right">Pr&oacute;xima Fecha de Pago :</div></td>
						<td nowrap width="20%" > 
							<eibsinput:date name="RecData" fn_month="E01DEAHEM" fn_day="E01DEAHED" fn_year="E01DEAHEY" readonly="FALSE" required="TRUE"/> 
						</td>
					</tr>
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="25%" > 
							<input type="text" name="E01DEAIVC" size="14" maxlength="12" value="<%= RecData.getE01DEAIVC().trim()%>">				
						</td>
					</tr>
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
					...
					<tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>">
						...
						<td nowrap width="25%" >
							...
							<input type="radio" name="CE01DEAROC" value="Y" onClick="document.forms[0].E01DEAROC.value='Y' ; ShowReno();"
							<%if(RecData.getE01DEAROC().equals("Y")) out.print("checked");%> >
							S&iacute; 
							<input type="radio" name="CE01DEAROC" value="N" onClick="document.forms[0].E01DEAROC.value='N' ; ShowReno();"
							<%if(RecData.getE01DEAROC().equals("N")) out.print("checked");%> >
							No <img src="<%=request.getContextPath()%>/images/Check.gif" title="campo obligatorio" border="0" width="16" height="20"  >
						</td>
						<td width="25%" id="datos1" style="display: none;"><div align="right" >Meses de Plazo para Renovaci&oacute;n :</div></td>
						<td width="25%" id="datos2" style="display: none;"></td>			
						<td width="25%" id="datos3" style="display: none;">
							<div align="left" > 
								<eibsinput:text name="RecData" property="E01DEAROY" eibsType="<%=EibsFields.EIBS_FIELD_TYPE_DAYS%>"/>
							</div>
						</td>    
