  ...
  function goAction(opt) {
    ...
    if (opt == "D") {
	  ...
	  if (ok) document.forms[0].submit();
    	...
    	if (opt == "M") {
    		page = "<%=request.getContextPath()%>/pages/s/EPR0310_foreign_currency_exchange_maint.jsp?ROW=" + row;
		}
    	if (opt == "C") {
    		page = "<%=request.getContextPath()%>/pages/s/EPR0310_foreign_currency_exchange_detail.jsp?ROW=" + row;
		}
  		CenterNamedWindow(page,'Information',550,450,2);
  	} else {
  	    page = "<%=request.getContextPath()%>/pages/s/EPR0310_foreign_currency_exchange_new.jsp?NEW=1";
  		CenterNamedWindow(page,'Information',550,450,2);
  	}
  }
...
<h3 align="center">Compraventa Moneda Extranjera-Actualizacion de Oficinas
<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="foreign_currency_exchange_list.jsp, EPR0310">
</h3>
<hr size="4">
    ...
    <TD NOWRAP valign=top>
  	 <TABLE id="dataTable" width="100%">
        ...
        while (mtList.getNextRow()) {
           ...
           datapro.eibs.beans.EPR031001Message msgMT = (datapro.eibs.beans.EPR031001Message) mtList.getRecord();
     	%>               
        <TR>
