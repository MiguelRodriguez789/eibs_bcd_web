...
addEventHandler(document, 'click', closeHiddenDivMod);
 ...
 function goAction(op) {
   ...
   switch (op) {
    ...
    case 2 :{
            page= "${pageContext.request.contextPath}/pages/s/EDP0030_client_financial_plan.jsp";
 			CenterWindow(page,600,500,4);
            break;
            }
    case 3 :{
            page = "${pageContext.request.contextPath}/servlet/datapro.eibs.client.JSEDP0030?SCREEN=3";
...
<h3 align="center">Información Financiera<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="client_financial.jsp,EDP0030"></h3>
<hr size="4">
   ...
   <TABLE id="headTable" width="100%" cellpadding="0" cellspacing=0>
    ...
    <tr id="trdark"> 
      ...
      <td nowrap > 
         ...
         <input type="text" name="E01IFMFEY" size="5" readonly value="<%=userPO.getHeader6()%>">/
         <input type="text" name="E01IFMFEM" size="3" readonly value="<%=userPO.getHeader7()%>">
      </td>        
    </tr>  
  </table> 
 </td>        
    	...
    	<table id="dataGrpTable<%= i%>" style="display:none;">
     		...
     		<TR>       			
                ...
                <input type="radio" name="GRPNUM<%= i%>" value="<%= grpList.getFlag() + "" + j %>" onClick="showGrpAcc(this.value,<%= j %>)" <%= grpChk %>>
      			...
      			<TD ALIGN=LEFT NOWRAP><a href="javascript:radioClick('GRPNUM<%= i%>',<%= j %>)"><DIV ><%= grpList.getRecord(0) %></DIV></a></TD>      
      			<TD ALIGN=RIGHT NOWRAP><DIV ><a href="javascript:radioClick('GRPNUM<%= i%>',<%= j %>)"><%= datapro.eibs.master.Util.fcolorCCY(grpList.getRecord(1)) %></a></DIV></TD>
      		</TR>
  <% 
                     j++;
                    }
     		...
     		<TR> 
              ...
              <TD ALIGN=Left NOWRAP>
                <input type="radio" name="ACCNUM<%= accList.getFlag() %>" value="<%= accList.getRecord(3) %>" <%=accChk%> onClick="setAccInfo(this.value,'<%=j%>','<%= accList.getCurrentRow() %>')">                
      			...
      			<TD ALIGN=LEFT NOWRAP><div ><a href="javascript:radioClick('ACCNUM<%= accList.getFlag() %>',<%= j %>)"><%= accList.getRecord(0) %></a></DIV></TD>
      			<TD ALIGN=LEFT NOWRAP><div ><a href="javascript:radioClick('ACCNUM<%= accList.getFlag() %>',<%= j %>)"><%= accList.getRecord(1) %></a></DIV></TD>      
      			<TD ALIGN=RIGHT NOWRAP><div ><a href="javascript:radioClick('ACCNUM<%= accList.getFlag() %>',<%= j %>)"><%= datapro.eibs.master.Util.fcolorCCY(accList.getRecord(2)) %></a></DIV></TD>
     		</TR>
  <% 
						j++;
                    }		
