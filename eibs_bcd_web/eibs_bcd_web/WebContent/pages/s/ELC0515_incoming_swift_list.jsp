...
<script type="text/javascript">
	...
	function showInq() {
		var page = "<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSELC0515?SCREEN=2&RBK=" + document.forms[0].RBK.value + "&RNO=" + document.forms[0].RNO.value;
	...
	function goAction() {
   		if (document.forms[0].FMT.value == '700' 
   			|| document.forms[0].FMT.value == '710') {
   	   		showHiddenDivPrd;
		}
	}
	...
	function closeHiddenDivPrd(){
		setVisibility(getElement("hiddenDivPrd"), "hidden");
    	document.forms[0].PRODUCT.value = "";
	}


	function showHiddenDivPrd(event) {
		if (document.forms[0].FMT.value == '700' 
			|| document.forms[0].FMT.value == '710') {
			
			evt = getEvent(event);
			var hiddenDivPrd = getElement("hiddenDivPrd");
...
addEventHandler(document, 'click', closeHiddenDivPrd);
...
<BODY>
<h3 align="center">Lista de Mensajes Swift Recibidos<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="incoming_swift_list.jsp, ELC0515"></h3>
<hr size="4">
 ...
 <TABLE id=tbhelp style="border-top-width : 1px;border-right-width : 1px;border-bottom-width : 1px;border-left-width : 1px;
	...
	<tr id="trdark"> 
      ...
      <td align=CENTER width="75%"> 
        <div align="left"> 
          <input type="text" maxlength="4" size="5" name="PRODUCT" value="">
          <a href="javascript:GetProduct('PRODUCT','40','')">
 ...
 %>
  ...
  <table class="tbenter" width=100% align=center>
    <tr> 
	  ...
	  <td class=TDBKG width="30%"> 
        <div align="center"><a href="javascript:deleteMessage()"><b>Eliminar</b></a></div>
      </td>
      ...
      <td NOWRAP valign="top" width="100%"> 
        <table id="headTable" width="100%">
          <tr id="trdark"> 
            ...
            <th align=CENTER nowrap width="10%">Banco<br>Remitente</th>
          ...
          <tr> 
            <td NOWRAP align=CENTER width="5%"> 
              <input type="radio" name="CURRCODE" value="<%= msgList.getCurrentRow() %>" <%=chk%> 
              	onClick="getParams(this.value,'<%= msgLC.getE01WAPSBK() %>','<%= msgLC.getE01WAPRNO() %>','<%= msgLC.getE01WAPRBK() %>','<%= msgLC.getE01WAPFMT() %>');">
            </td>          
            <td NOWRAP  align=LEFT width="10%"><%= msgLC.getE01WAPSBK() %></td>
			<td NOWRAP  align=CENTER width="10%"><%= msgLC.getE01WAPSRF() %></td>
            <td NOWRAP  align=LEFT width="30%"><%= msgLC.getE01WAPBEN() %></td>
            <td NOWRAP  align=CENTER width="10%"><%= Util.formatCustomDate(currUser.getE01DTF(), msgLC.getE01WAPDTM(),msgLC.getE01WAPDTD(),msgLC.getE01WAPDTY())  %></td>
            <td NOWRAP  align=CENTER width="10%"><%= msgLC.getE01WAPFMT() %></td>
            <td NOWRAP  align=LEFT width="10%"><%= msgLC.getE01WAPCCY() %></td>
            <td NOWRAP  align=RIGHT width="15%"><%= Util.formatCCY(msgLC.getE01WAPAMT()) %></td>
...
<%}%>
	<SCRIPT type="text/javascript">
	 	getElement("hiddenDivPrd").onclick = cancelBub;
		<% if (!msgList.getNoResult()) { %>	 	
		   	   getElement("eibsPrd").onclick = showHiddenDivPrd;
		<% } %>     
	</SCRIPT>
