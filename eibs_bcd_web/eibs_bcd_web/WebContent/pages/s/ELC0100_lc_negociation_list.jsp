...
// var reason = '';
 ...
 function setReason(op, _reason){
 	option = op;
 	reason  = _reason;
	var page= '${pageContext.request.contextPath}/pages/s/ELC0525_enter_reason_text.jsp';
 ...
 function goMsgSwift() {
     ...
     if (letterOfCreditForm.E01LCMACC.value !== "") {
		   ...
		   page = "${pageContext.request.contextPath}/servlet/datapro.eibs.products.JSELC0100?SCREEN=0&E01LCMACC="+letterOfCreditForm.E01LCMACC.value;
 ...
 function showAddInfo(idxRow){
   ...
   tbAddInfo.rows[0].cells[1].innerHTML=extraInfo(getElement("TXTDATA"+idxRow).value,6);   
...
<FORM name="letterOfCreditForm" method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSELC0100">
	...
	<h3 align="center">Aperturas y Enmiendas de Cartas de Credito
		<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="lc_negociation_list.jsp,ELC0100">
	</h3>
	<hr size="4">
					    ...
					    %>               
						        <TR>
									...
									<TD NOWRAP ALIGN="CENTER">
										<a href="javascript:goLetterDetail('V','<%=msgPart.getH01FLGWK3()%>','<%=msgPart.getE01LCRNUM()%>');">
											<%=Util.formatCell(msgPart.getE01DRWNUM())%>
										</a>
									</TD>
									<TD NOWRAP ALIGN="CENTER">
										<a href="javascript:goLetterDetail('V','<%=msgPart.getH01FLGWK3()%>','<%=msgPart.getE01LCRNUM()%>');">
