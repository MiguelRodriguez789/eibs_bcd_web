...
<title>Mantenimiento de Clientes CRS</title>
...
<jsp:useBean id= "ESD0085List" class= "datapro.eibs.beans.JBObjList"  scope="session" />
...
function goSel() {
	...
	getElement("E02CUN").value = getElement("NEWMNT").value;
	document.forms[0].SCREEN.value="23";
	document.forms[0].submit();
}
     	...
     	var elementName= document.forms[0].elements[n].name;
      	if(elementName == "CODE") {
			if (document.forms[0].elements[n].checked == true) {
				getElement("E02CUN").value = document.forms[0].elements[n].value;
				ok = true;
        		break;
			}
      	}
...
<h3 align="center">Mantenimiento de Clientes CRS<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" 
	name="EIBS_GIF" title="client_crs_list.jsp, ESD0085"></h3>
<hr size="4">
...
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSESD0085">
  ...
  <INPUT TYPE=HIDDEN NAME="E02CUN" VALUE="">
  <INPUT TYPE=HIDDEN NAME="E02SHN" VALUE="">
  <INPUT TYPE=HIDDEN NAME="E02IDN" VALUE="">
  <INPUT TYPE=HIDDEN NAME="OPE" VALUE="<%=userPO.getSource()%>">
...
%>    
  ...
  <table  class="tableinfo" width="100%">
    ...
    <TR id=trdark>
		...
		<td NOWRAP align="center" width="305%"></td>
        ...
        <%
    	  int i = 0;
          ESD0085List.initRow();    
          while (ESD0085List.getNextRow()) {
            ESD008502Message msgList = (ESD008502Message) ESD0085List.getRecord(); 
		%>              
    <TR id=trclear>

		<td NOWRAP align="center" width="5%"> 
            <INPUT type="radio" name="CODE" value="<%= msgList.getE02CUN() %>">
            <% if (msgList.getE02STS().equals("4")) { %>
             <a href="javascript:showInqOFAC('<%= msgList.getE02CUN() %>')">
		...
		<td NOWRAP align="left" width="10%" ><A HREF="javascript:custInquiry('<%= msgList.getE02CUN() %>','<%="01"%>')"><%= msgList.getE02CUN() %></A> </td>
		<td NOWRAP align="left" width="35%" ><A HREF="javascript:custInquiry('<%= msgList.getE02CUN() %>','<%="01"%>')"><%= msgList.getE02NA1() %></A> </td>
		<td NOWRAP align="left" width="20%" ><%= msgList.getE02SHN() %></td>
		<td NOWRAP align="left" width="15%" ><%= msgList.getE02IDN() %></td>
		<td NOWRAP align="center" width="10%" ><%= msgList.getE02TID() %></td>
		<td NOWRAP align="center" width="5%" >
			<% if (msgList.getE02LGT().equals("2")) out.print("Natural") ;
			    else out.print("Juridico");
			%>
		</td>
		<td NOWRAP align="center" width="5%" style="color:green;font-size:10;">
			<% if (msgList.getE02CRT().equals("Y")) out.print("CREAR") ;
  ...
  <TABLE  class="tbenter" WIDTH="88%" ALIGN=CENTER>
      ...
      <TD WIDTH="50%" ALIGN=LEFT height="25">
       <% 
		...
		int screen = 26;
    	if ( ESD0085List.getShowPrev() ) {
  			int pos = ESD0085List.getFirstRec() - 21;
  			out.print("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.client.JSESD0085?SCREEN="+screen+"&FromRecord=" + pos + "&SEARCHCDE=" + userPO.getHeader11() + "&SEARCHTYPE=" + userPO.getType() + "&OPE=" + ope + "\" > <img src=\""+request.getContextPath()+"/images/s/Regresar.gif\" border=0></A>");
    	} %>
      </TD>
 	  <TD WIDTH="50%" ALIGN=RIGHT height="25"> 
 	  	<%       
    	if (ESD0085List.getShowNext()) {
  			int pos = ESD0085List.getLastRec();
  			out.print("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.client.JSESD0085?SCREEN="+screen+"&FromRecord=" + pos + "&SEARCHCDE=" + userPO.getHeader11() + "&SEARCHTYPE=" + userPO.getType() + "&OPE=" + ope + "\" ><img src=\""+request.getContextPath()+"/images/s/Avanzar.gif\" border=0></A>");
	    } %>
  </TD>
 </TR>
 </TABLE>
...
<SCRIPT Language="javascript">
	...
	document.forms[0].NEXTROWS.value = <%= ESD0085List.getLastRec()%>;
	document.forms[0].CURRROWS.value = <%= ESD0085List.getFirstRec()%>;
</SCRIPT>
<%      
  }
%> 
