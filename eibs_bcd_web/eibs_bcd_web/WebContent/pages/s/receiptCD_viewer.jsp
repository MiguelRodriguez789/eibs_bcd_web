...
<SCRIPT type="text/javascript">
	...
	function setImgVisible(val){
		document.getElementById("IMGSUBMIT").style.display = (val) ? "block" : "none"; 
	}

	function doPrint() {
		if (!window.print) {
	   		alert("Debe actualizar su navegador para imprimir");
	   		return;
		} else {
		    window.focus();
...
<%
	String title = "Recibo de Apertura de Certificados"; 
	boolean payments = (receiptH != null) && (receiptH.getLastRec() > 0);
	if (payments) {
		title = "Recibo de Pago de Certificado";
	}
%>	
...
<% } %> 
   ...
   <div align="center">
    	<img  class ="imgfilter" id ="IMGSUBMIT" name="Submit" 
    		src="<%=request.getContextPath()%>/images/s/bt_imprimir.gif" 
    		OnClick="doPrint();" 
