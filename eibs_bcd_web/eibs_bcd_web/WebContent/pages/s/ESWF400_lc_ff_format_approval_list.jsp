	...
	switch (op) {
		...
		case 9 : // Inquiry
			showInqApproval();
	 		break;
		case 4 : // Approval
 			document.forms[0].SCREEN.value = '9501';		
		...
		case 5 : // Delete
			...
			delok = confirm("Desea borrar el mensaje swift seleccionado? "); 
	 		if ( delok) {
	 			document.forms[0].SCREEN.value = '9502';
	 			document.forms[0].ACTION.value = 'D';
	 			document.forms[0].submit();
	 		}
	 		break;
		case 6 : // Reject
 			document.forms[0].SCREEN.value = '9503';
...
function showInqApproval()
{
	...
	page = "<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSESWF400?SCREEN=909&LCOACC=" + account +"&LCOSEQ=" + seq;
...
%>
<h3 align="center">Aprobar Formato Libre Swift <%= titTypePlu %><img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="lc_ff_format_approval_list.jsp,ESWF4000"> 
</h3>
<hr size="4">
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSESWF400" >
  ...
  <table class="tbenter" width="100%" height="59">
    <tr>
      <td align="CENTER" class="TDBKG" width="20%"> <a href="javascript:goAction(9)" id="linkInquiry"><b>Consultar</b></a> 
      </td>    
      <td align="CENTER" class="TDBKG" width="20%"> <a href="javascript:goAction(4)" id="linkApproval"><b>Aprobar</b></a> 
      </td>
      <td align="CENTER" class="TDBKG" width="20%"> <a href="javascript:enterReason(6, '<%=appCode%>')" id="linkReject"><b>Rechazar</b></a> 
      </td>
