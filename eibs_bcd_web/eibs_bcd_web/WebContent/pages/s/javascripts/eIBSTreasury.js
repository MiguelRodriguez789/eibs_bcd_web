...
function GetDealerAcc(code,type){

	page = webapp + "/servlet/datapro.eibs.treasury.JSEWD0331?type=" + type;
	fieldName=code;
...
function GetClassFex(clas, opt, ccy) {
	...
	page = webapp + "/servlet/datapro.eibs.helps.JSEWD0320?opt=" + opt
			+ "&ccy=" + ccy;
	fieldName = clas;
	CenterWindow(page, 500, 230, 3);
}
...
function showFxRang(ref, useid) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSEFE0130A?SCREEN=2&E01FESREF="
			+ ref + "&E01FESDID=" + useid;
	CenterWindow(page, 560, 500, 2);
}

function GetAccountT(acc, bnk, app, cun) {
	page = webapp + "/servlet/datapro.eibs.forex.JSEWD0005T?sel=N&bnk=" + bnk
			+ "&app=" + app + "&cun=" + cun;
	fieldName = acc;
	CenterWindow(page, 500, 230, 3);
}
...
function showTodDealsFr(ref, dealer, typ, acd) {
	page = webapp + "/servlet/datapro.eibs.forex.JSEWD0325BI?SCREEN=2&ref="
			+ ref + "&dealer=" + dealer + "&typ=" + typ + "&acd=" + acd;
	CenterWindow(page, 560, 500, 2);
}

function showFxDeals(acc, prog) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSEFE0000?SCREEN=2200&REFERENCE="
			+ acc + "&PROGRAM=" + prog;
	CenterWindow(page, 560, 500, 2);
}
...
function showFxAcc(ref) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSEFE0160?SCREEN=40&E01FEMACC="
			+ ref;
	CenterWindow(page, 560, 500, 2);
}
...
function showFRAAcc(ref) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSETR0160?SCREEN=100&E01FRAACC="
			+ ref;
	CenterWindow(page, 560, 500, 2);
}
...
function showFRAAccInq(ref) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSETR0160I?SCREEN=100&E01FRAACC="
			+ ref;
	CenterWindow(page, 560, 500, 2);
}
function showFRAAcc_Stl(ref) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSETR0150?SCREEN=70&E01FRAACC="
			+ ref;
}
function showDeaAcc(ref) {
	page = webapp
			+ "/servlet/datapro.eibs.forex.JSEDL0130IT?SCREEN=600&E01DEAACC="
			+ ref;
	CenterWindow(page, 560, 500, 2);
}
...
function showInqApprovalT(app, account, type, cus) {
	...
	if (app == '13' && type == '2') {
		page = webapp
				+ "/servlet/datapro.eibs.forex.JSEDL0108B?SCREEN=200&E02DEAACC="
				+ account + "&E02DEACUN=" + cus;
	} else {
		page = webapp
				+ "/servlet/datapro.eibs.forex.JSEDL0140T?SCREEN=3&ACCNUM="
				+ account + "&appCode=" + app + "&typeCode=" + type;
	}
...
function GetProductT(name, desc, ataken, atype, appcode) {
	page = webapp + "/servlet/datapro.eibs.helps.JSEWD0008T?ATAKEN=" + ataken
			+ "&ATYPE=" + atype + "&FLAG=X&APPCODE=" + appcode;
	fieldName = name;
	fieldAux1 = desc;
	ATAKEN = ataken;
	ATYPE = atype;
