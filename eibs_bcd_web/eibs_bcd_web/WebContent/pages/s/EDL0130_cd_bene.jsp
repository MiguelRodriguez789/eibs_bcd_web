...
<h3 align="center">Beneficiarios <img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="cd_bene.jsp, EDL0130"></h3>
<hr size="4">
...
<div id="dataDiv" align="center" style="height:100%; overflow-y:scroll; width:100%;" class="scbarcolor">
<%
  int bene_amount = 10;
  String name;
      ...
      <td> 
        <table cellspacing="0" cellpadding="2" width="100%" border="0" title="TB_BENEFICIARY_CORP" bgcolor="#FFFFFF" bordercolor="#FFFFFF"  align="center">
	  		<tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>"> 
	   			...
	   			<td width="60%" nowrap> 
	     			<input type="text" name="E<%= name %>4RCN" maxlength="12" size="12" value="<%= bene.getField("E" + name + "4RCN").getString().trim()%>">
			  		<a href="javascript:GetCustomerDetails('E<%= name %>4RCN','','','','','','','','','','','','','','','','','E<%= name %>4HPN')"><img src="<%=request.getContextPath()%>/images/1b.gif" title="Ayuda" align="bottom" border="0" ></a>
	   			</td>
	  		</tr>
        	<tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>"> 
            	<td width="40%" nowrap> 
            	...
            	<td nowrap width="60%"> 
              		<input type="text" name="E<%= name %>4MA1" maxlength="45" size="55" value="<%= bene.getField("E" + name + "4MA1").getString().trim()%>">
            	</td>
        	</tr>
	  		<tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>"> 
	   			<td width="40%" nowrap>
	   			...
	   			<td width="60%" nowrap> 
	        		<input type="hidden" name="CTYCode<%=name%>" id="CTYCode<%=name%>" value="">
	     			<input type="text" name="E<%= name %>4CTR" maxlength="35" size="37" value="<%= bene.getField("E" + name + "4CTR").getString().trim()%>" readonly onFocus="javascript:showIdFields(this.value,'IDVEN_<%= name %>','IDOTHER_<%= name %>','IDPAPERS_<%= name %>');">
			...
			<tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>">
				...
				<td width="60%" nowrap>
					...
					<div id="IDOTHER_<%= name %>" style="position:relative; display:none;">
						<input type="text" name="OIDN_<%= name %>" value="<%= bene.getField("E" + name + "4BNI").getString() %>" maxlength="25" size="27" onblur="document.forms[0].E<%= name %>4BNI.value=this.value" >
					</div>
	        ...
	        <tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>"> 
	            ...
	            <td width="60%" nowrap> 
	              <input type="text" name="E<%= name %>4HPN" maxlength="15" size="17" value="<%= bene.getField("E" + name + "4HPN").getString().trim()%>">
	            </td>
	        </tr>
        </table>
      </td>
