...
<jsp:useBean id="msg046004" class="datapro.eibs.beans.ELC046004Message" scope="session" />
...
<SCRIPT type="text/javascript">
  <% if(request.getParameter("OPTMENU") != null) { %>
   ...
   <%} else {
   		if (msg046004.getE04LCMTYP().equals("R") || msg046004.getE04LCMTYP().equals("S")) {%>
...
function goMsgSwift() {
   	...
   	page = "<%=request.getContextPath()%>/servlet/datapro.eibs.approval.JSEPR1080A?SCREEN=8&TRANSREFNUM=<%=msg046004.getE04LCMACC().trim()%>";
...
<BODY>
<H3 align="center">Historial de Comisiones de Cartas de Credito<IMG src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="lc_commissi.jsp, ELC0460"></H3>
<HR size="4">
...
<TABLE width="100%" border="0" cellpadding="2" cellspacing="0" class="tableinfo">
    <TR id="trdark">
      ...
      <TD width="31%" nowrap="nowrap"><INPUT type="text" name="E04LCMBNK" readonly
					size="4" maxlength="2" value="<%=msg046004.getE04LCMBNK().trim()%>"> 

        <B>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Producto</B> <B>
        <INPUT type="text" name="E01LCMPRO" size="4"
					maxlength="4" value="<%=msg046004.getE04LCMPRO().trim()%>" readonly>
        </B></TD>
      ...
      <TD width="19%" nowrap="nowrap"><B>
        <INPUT type="text" name="E04LCMACC" size="12"
					maxlength="12" value="<%=msg046004.getE04LCMACC().trim()%>" readonly>
      </B></TD>
    </TR>
    <TR id="trclear">
      <TD align="right" nowrap="nowrap"><B>Fecha Emision</B></TD>
      <TD nowrap="nowrap">
			<eibsinput:date name="msg046004" fn_month="E04LCMIDM" fn_day="E04LCMIDD" fn_year="E04LCMIDY" readonly="true"/>
		</TD>
      <TD align="right" nowrap="nowrap"><B>Tarifa de Cargos y Moneda</B></TD>
      <TD nowrap="nowrap"><INPUT type="text" name="E04LCMTAR" size="2"
					maxlength="2" value= "<%=msg046004.getE04LCMTAR()%>" readonly>
        <INPUT type="text" name="E04LCMTCY" size="3"
					maxlength="3" value= "<%=msg046004.getE04LCMTCY()%>" readonly></TD>
    </TR>
    <TR id="trdark">
      <TD align="right" nowrap="nowrap"><B>Fecha Expiracion</B></TD>
      <TD nowrap="nowrap">
			<eibsinput:date name="msg046004" fn_month="E04LCMEXM" fn_day="E04LCMEXD" fn_year="E04LCMEXY" readonly="true"/>
		</TD>
      <TD align="right" nowrap="nowrap"><B>Monto del Credito</B></TD>
      <TD nowrap="nowrap"><B>
        <INPUT type="text" name="E04LCMOAM" size="12"
					maxlength="12" value="<%=msg046004.getE04LCMOAM()%>" readonly>
      </B></TD>
    </TR>
  </TABLE>
  <BR>
...
<TABLE class="tableinfo" border="0" cellspacing="0" cellpadding="4" width="100%">
					...
					if(msg046004.getE04LCMOPT().equals("N"))
						{
					...
					{
						value2.add(msg046004.getField((String)field2.get(i)).getString().trim());
					}
					value2.add("");
					value2.add("");
						...
						<TR id="<%=( i%2==0 ? "trclear" : "trdark" )%>">
							...
							<TD nowrap  align="center">
								<INPUT type="text" name="<%=field1.get(i)%>" size="15" maxlength="15"
									value="<%=msg046004.getField((String) field1.get(i)).getString().trim()%>" readonly>
							</TD>
							<TD nowrap width="11%">
								<SELECT name="<%=field2.get(i)%>" disabled >
									...
									<%if(i < name.size()-2)
									{%>
										<OPTION value="B" <%=( msg046004.getField((String) field2.get(i)).getString().trim().equals("B") ? "selected" : "" )%>>
											Beneficiario
										</OPTION>
									<%}
									else{%>
