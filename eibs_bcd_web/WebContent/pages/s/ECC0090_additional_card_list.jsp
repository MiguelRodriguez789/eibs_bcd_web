...
function add(num) {
	...
	if (i == -1) {
		...
		E04CCRCRA.value = acc[i].value;
		var key = acc[i].label;
    	var temp = new Array();
    	temp = key.split('-');
    	E04CCRTYP.value = temp[0];
    	E04CCRSTP.value = temp[1];
		
		if (num == 1) {
			if (E04CCRTYP.value != "CCTE" &&
				E04CCRTYP.value != "CINL" &&
				E04CCRTYP.value != "CTEI") {
		...
		if (num == 4) {
			if (E04CCRTYP.value != "CCTE" &&
				E04CCRTYP.value != "CINL" &&
				E04CCRTYP.value != "CTEI" &&
				E04CCRTYP.value != "CAHO" &&
				E04CCRTYP.value != "CFAL") {
				alert("La cuenta seleccionada no puede ser asignada como cuenta secundaria.");
			}
			else {
				var option = new Option(E04CCRCRA.value + " (" + E04CCRTYP.value + "/" + E04CCRSTP.value + ")", E04CCRCRA.value, false);
				var accs = document.getElementById("AccSList");
				if (accs[0].value == "X") {
					accs.remove(0);
				}
				accs.options[accs.length] = option;
				accs[accs.length-1].label = E04CCRTYP.value + "-" + E04CCRSTP.value;
				acc.remove(i);
			}
		}
		if (acc.length == 0) {
...
function del(num) {
	...
	if (num == 1) {
		if (ccp.value == "") {
			...
			var option = new Option(ccp.value + " (" + type1.value + "/" + subtype1.value + ")", ccp.value, false);
			var acc = document.getElementById("AccList");
			if (acc[0].value == "X") {
				acc.remove(0);
			}
			acc.options[acc.length] = option;
			acc[acc.length-1].label = type1.value + "-" + subtype1.value;
			ccp.value = "";
	...
	if (num == 2) {
		if (cap.value == "") {
			...
			var option = new Option(cap.value + " (" + type2.value + "/" + subtype2.value + ")", cap.value, false);
			var acc = document.getElementById("AccList");
			if (acc[0].value == "X") {
				acc.remove(0);
			}
			acc.options[acc.length] = option;
			acc[acc.length-1].label = type2.value + "-" + subtype2.value;
			cap.value = "";
	...
	if (num == 3) {
		if (cfp.value == "") {
			...
			var option = new Option(cfp.value + " (" + type3.value + "/" + subtype3.value + ")", cfp.value, false);
			var acc = document.getElementById("AccList");
			if (acc[0].value == "X") {
				acc.remove(0);
			}
			acc.options[acc.length] = option;
			acc[acc.length-1].label = type3.value + "-" + subtype3.value;
			cfp.value = "";
	...
	if (num == 4) {
		if (i == -1) {
			...
			E04CCRCRA.value = accs[i].value;
			var key = accs[i].label;
			var temp = new Array();
			temp = key.split('-');
			E04CCRTYP.value = temp[0];
			E04CCRSTP.value = temp[1];
			...
			acc.options[acc.length] = option;
			acc[acc.length-1].label = E04CCRTYP.value + "-" + E04CCRSTP.value;
			accs.remove(i);
		}
	}
	if (accs.length == 0) {
...
de Tarjetas de Débito<BR>Mantenimiento de Tarjeta Adicional<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="additional_card_list.jsp, ECC0090"></h3>
<hr size="4">
<% 
  ...
  <table class="tableinfo">
    <tr>
      ...
      %>
		...
		<OPTION value="<%= msgAcc.getE04CCRCRA() %>" label="<%= msgAcc.getE04CCRTYP()%>-<%= msgAcc.getE04CCRSTP()%>"><%= msgAcc.getE04CCRCRA() %> (<%= msgAcc.getE04CCRTYP() %>/<%= msgAcc.getE04CCRSTP() %>)</OPTION>
	  	<%
	  		aux1++;
	  	} %>
	  <% } %>
