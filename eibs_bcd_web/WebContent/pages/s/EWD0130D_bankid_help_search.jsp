...
<SCRIPT type="text/javascript">
	function enter(){
 		...
 		var myArgs = <%=request.getParameter("args")%>;
 		var NameSearch = document.forms[0].NameSearch.value
 		
 		if (NameSearch.length > 0){
    		parent.results.window.location.href="<%=request.getContextPath()%>/pages/s/MISC_search_wait.jsp?URL='<%=request.getContextPath()%>/servlet/datapro.eibs.helps.JSEWD0130D?NameSearch=" + NameSearch + "@PmtVia=" + PmtVia + "@args=" + myArgs + "'";    
		} else {
			alert("Debe entrar un nombre válido")
		}
	}
</SCRIPT>
<TITLE></TITLE>
