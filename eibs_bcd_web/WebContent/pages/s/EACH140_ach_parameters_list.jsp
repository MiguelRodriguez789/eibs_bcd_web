...
<h3 align="center">Parametros de ACH<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" 
	name="EIBS_GIF" title="ach_parameters_list.jsp, EACH140"></h3>
<hr size="4">
    ...
    <TR id=trdark>
		...
		<td NOWRAP align="center" width="5%"><B>Lote Contable</B><BR>
			<B><IMG src="<%=request.getContextPath()%>/images/Check.gif"
				title="Campo Obligatorio" align="bottom"></B>
		</td>
		...
		<td NOWRAP align="center" width="5%"><B>Lote Contable</B><BR>
			<b><IMG src="<%=request.getContextPath()%>/images/Check.gif"
				title="Campo Obligatorio" align="bottom"><B></B></b></td>
		<TD nowrap align="center" width="15%"><B>Cuenta ACH</B><BR>
		<B><IMG src="<%=request.getContextPath()%>/images/Check.gif"
			title="Campo Obligatorio" align="bottom"></B></TD>
		<TD nowrap align="center" width="15%"><B>Cuenta Puente<BR>
		<IMG src="<%=request.getContextPath()%>/images/Check.gif"
			title="Campo Obligatorio" align="bottom">
		</B></TD>
		<TD nowrap align="center" width="10%"><B>Consecutivo<BR>
		Lote de ACH</B>
		</TD>
	</TR>
		...
		<td NOWRAP align="center" width="5%" valign="top">
			<INPUT type="text" name="E01ACHBNK_<%= EACH140List.getCurrentRow() %>" size="3" maxlength="2"
					onkeypress="enterInteger(event)" value='<%= msgList.getE01ACHBNK()%>' <%= read %>>
		</td>
		...
		<td NOWRAP align="center" width="10%" valign="top">
			...
			<A href="javascript:GetAchOperator('E01ACHOCD_<%= EACH140List.getCurrentRow() %>','TYPE','OPEDSC','O')">
				<IMG src="<%=request.getContextPath()%>/images/1b.gif" title="Ayuda" border="0" align="top"></A>
			<BR>
			<INPUT type="text" name="E01ACHODF_<%= EACH140List.getCurrentRow() %>"
				value="<%= msgList.getE01ACHODF() %>" size="11" maxlength="10" <%= read %>>
			<A href="javascript:GetAchOperator('E01ACHODF_<%= EACH140List.getCurrentRow() %>','TYPE','OPEDSC','D')">
				<IMG src="<%=request.getContextPath()%>/images/1b.gif" title="Ayuda" border="0" align="top">
			</A>
		</td>

		<td NOWRAP align="center" width="5%" valign="top">
			<INPUT type="text" name="E01ACHBAI_<%= EACH140List.getCurrentRow() %>" onkeypress="enterInteger(event)"value="<%= msgList.getE01ACHBAI() %>" size="6" maxlength="5" <%= read %>>
		</td>
		...
		<TD nowrap align="left" width="15%" valign="top">
			<INPUT type="CHECKBOX" name="E01ACHNSF_<%= EACH140List.getCurrentRow() %>" <%= disabled %> 
			 	value="Y" <% if (msgList.getE01ACHNSF().equals("Y")) out.print(" checked"); %>>Cargos Fondos Insuficientes<BR>
		...
		<td NOWRAP align="center" width="5%" valign="top">
			<INPUT type="text" name="E01ACHBAT_<%= EACH140List.getCurrentRow() %>" value="<%= msgList.getE01ACHBAT() %>" size="6" maxlength="5"
		...
		<TD nowrap align="center" width="10%" valign="top">
			...
			value="<%= msgList.getE01ACHNUM() %>" size="6" maxlength="5" onkeypress="enterInteger(event)" <%= read %>>
		</TD>
	</TR>
       <% 
       	 i++; 
