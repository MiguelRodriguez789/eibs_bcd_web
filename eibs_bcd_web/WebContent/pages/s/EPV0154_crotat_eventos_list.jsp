...
<html>
<head>
<title>Lista de Cupo Rotativo</title>
<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=ISO-8859-1">
<link Href="<%=request.getContextPath()%>/pages/style.css" rel="stylesheet">

<jsp:useBean id= "HeadInfo" class="datapro.eibs.beans.EPV015401Message" scope="session" />
<jsp:useBean id= "EPV015407List" class= "datapro.eibs.beans.JBObjList"  scope="session" />
...
<h3 align="center">Eventos Hist&oacute;ricos - Cupo Rotativo<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="crotat_eventos_list.jsp, EPV0154"></h3>
<hr size="4">
<FORM name="form1" METHOD="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.salesplatform.JSEPV0154" >
		...
		<tr  height="100%">    
			<td nowrap="nowrap">      
				<div id="dataDiv1" style="max-height:400px; overflow:auto;"> 
					...
					<%
						EPV015407List.initRow();
						boolean firstTime = true;
						String chk = "";
						while (EPV015407List.getNextRow()) {
							if (firstTime) {
								firstTime = false;
								chk = "checked";
							} else {
							...
							datapro.eibs.beans.EPV015407Message msgList = (datapro.eibs.beans.EPV015407Message) EPV015407List.getRecord();
					%>
						<tr id="dataTable<%= EPV015407List.getCurrentRow() %>"> 
							...
							<td NOWRAP align=LEFT width="25%">&nbsp;
								...
								<%= msgList.getE07HEVTYP().equals("D") ? "UTILIZACIONES" : "" %>
							...
							<td NOWRAP  align=CENTER width="6%">&nbsp;
								<% if(msgList.getE07HEVTYP().equals("S")  || msgList.getE07HEVTYP().equals("A") || msgList.getE07HEVTYP().equals("M")|| msgList.getE07HEVTYP().equals("D")|| msgList.getE07HEVTYP().equals("I")|| msgList.getE07HEVTYP().equals("N")) { %>
								...
								<%= msgList.getE07HEVSTS().equals("6") ? "Cerrado" : "" %>
								<%= msgList.getE07HEVSTS().equals("7") ? "Bloqueo Manual" : "" %>
								<%}%>          		 
							</td>   
						</tr>
					<% } %>
	...
	<TABLE  class="tbenter" WIDTH="98%" ALIGN="center"  height="10%">
		<TR>
			...
			<% 
				if ( EPV015407List.getShowPrev() ) {
					int pos = EPV015407List.getFirstRec() - 51;
					out.print("<A HREF=\"" + request.getContextPath() + "/servlet/datapro.eibs.salesplatform.JSEPV0154DADC?SCREEN=700&FromRecord=" + pos + "\" ><img src=\""+request.getContextPath()+"/images/s/previous_records.gif\" border=0></A>");
				}
			%>
			</TD>
			<TD WIDTH="50%" ALIGN=RIGHT height="25">
			<% 
				if ( EPV015407List.getShowNext() ) {
					int pos = EPV015407List.getLastRec();
					out.print("<A HREF=\"" + request.getContextPath() + "/servlet/datapro.eibs.salesplatform.JSEPV0154DADC?SCREEN=700&FromRecord=" + pos + "\" ><img src=\""+request.getContextPath()+"/images/s/next_records.gif\" border=0></A>");
				}
			%>
			</TD>
		</TR>
