...
<H3 align="center">Referencia Bancaria<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="client_bank_references.jsp, ESD0086"></H3>
<hr size="4">
  ...
  <table  class="tableinfo" width="100%">
    <tr bordercolor="#FFFFFF"> 
      <td nowrap> 
        ...
        <tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>"> 
            <td nowrap width="15%" align="right">Oficial / Contacto :</td>
            <td nowrap width="35%" align="left"> 
               	<input type="text" name="E01MA3" size="61" maxlength="60" value="<%= ESD0086Record.getE01MA3().trim()%>" <%=read%>>
            </td>
         ...
         <tr id="<%= (row % 2 == 1) ? "trdark" : "trclear" %><%row++;%>"> 
            <td nowrap width="15%" align="right">Tipo y Número de Cuenta :</td>
            <td nowrap width="35%" align="left"> 
               	<input type="text" name="E01MA2" size="61" maxlength="60" value="<%= ESD0086Record.getE01MA2().trim()%>" <%=read%>>
            </td>
            <td nowrap width="15%" align="right">Saldo Promedio :</td>
            <td nowrap width="35%" align="left">
    	       	 <SELECT name="E01FL8" <%=disabled%>>
                    <OPTION value="">--Seleccione--</OPTION>
					<OPTION value="3" <%if(ESD0086Record.getE01FL8().equals("3")){%> selected <%}%>>3 cifras</OPTION>
					<OPTION value="4" <%if(ESD0086Record.getE01FL8().equals("4")){%> selected <%}%>>4 cifras</OPTION>
					<OPTION value="5" <%if(ESD0086Record.getE01FL8().equals("5")){%> selected <%}%>>5 cifras</OPTION>
					<OPTION value="6" <%if(ESD0086Record.getE01FL8().equals("6")){%> selected <%}%>>6 cifras</OPTION>
					<OPTION value="7" <%if(ESD0086Record.getE01FL8().equals("7")){%> selected <%}%>>7 cifras</OPTION>
					<OPTION value="8" <%if(ESD0086Record.getE01FL8().equals("8")){%> selected <%}%>>8 cifras</OPTION>
					<OPTION value="9" <%if(ESD0086Record.getE01FL8().equals("9")){%> selected <%}%>>9 cifras</OPTION>
					<OPTION value="0" <%if(ESD0086Record.getE01FL8().equals("0")){%> selected <%}%>>10 cifras</OPTION>
					<OPTION value="1" <%if(ESD0086Record.getE01FL8().equals("1")){%> selected <%}%>>11 cifras</OPTION>
					<OPTION value="2" <%if(ESD0086Record.getE01FL8().equals("2")){%> selected <%}%>>12 cifras</OPTION>
				</SELECT>
				<SELECT	name="E01FL9" <%=disabled%>>
			        <OPTION value="">--Seleccione--</OPTION>
					<OPTION value="A" <%if(ESD0086Record.getE01FL9().equals("A")){%> selected <%}%> >Altas</OPTION>
					<OPTION value="M" <%if(ESD0086Record.getE01FL9().equals("M")){%> selected <%}%> >Medias</OPTION>
					<OPTION value="B" <%if(ESD0086Record.getE01FL9().equals("B")){%> selected <%}%> >Bajas</OPTION>
			</SELECT>
            </td>
         </tr>          
       </table>
