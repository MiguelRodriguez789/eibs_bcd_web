...
<%      
	            ...
	            String marker = "";
	            try {
	            	mp = new MessageProcessor("EPR0300");
	           		EPR030001Message msgHelp = (EPR030001Message) mp.getMessageRecord("EPR030001");
					...
					if (mp.hasError(list)) {
						...
						while (list.getNextRow()) {
							msgHelp = (EPR030001Message) list.getRecord();
%>
				           	<tr>
				           		<td nowrap> <%=msgHelp.getE01PRATCD().trim()%> </td>
								<td nowrap>
									<A HREF="javascript:enter('<%=msgHelp.getE01PRATCD().trim()%>', '<%=msgHelp.getE01PRADSC().trim()%>','<%=msgHelp.getE01PRACNL().trim()%>')"> <%=msgHelp.getE01PRADSC().trim()%></a>
								</td>       		           		
					        </tr>               
<%           	
						}
