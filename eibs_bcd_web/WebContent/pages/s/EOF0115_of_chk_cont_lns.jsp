...
%>
 ...
 /*
       Existen dos posibilidades para las opciones de impresion:
        ...
        - of_a_opt  : Impresion de cheque de gerencia por PDF.
...
%>
function Action() {
	if (document.forms[0].SOURCE.value == "FACT") {
	var fld = "E30OFFAC" + document.getElementById("SEQ").value;
	
		if (document.forms[0].OPTION.value != "P" && document.forms[0].OPTION.value != "C") {
			document.forms[0].SCREEN.value = "4";
			window.opener.document.getElementById(fld).value =
							 document.getElementById("CHECKNUM").value;
			pg = "<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEDL0800?SCREEN=26";
			window.parent.location.href=pg; 
		} else {
			document.forms[0].SCREEN.value = "7";
			//pg = "<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEDL0825?SCREEN=7";
			//window.parent.location.href=pg;
...
<H3 align="center">
	<% if (userPO.getSource().equals("FACT") ) {
		out.println("Emisi&oacute;n Cheques de Gerencia por Desembolso de Descuento de Documentos");
	} else {
		out.println("Emisi&oacute;n Cheques de Gerencia por Desembolso de Préstamos");
	}
	%>
	<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="of_chk_cont_lns.jsp, EOF0115" width="35" height="35"> 
</H3>
<hr size="4">
<form method="post" action="<%=request.getContextPath()%>/pages/background.jsp">
  <INPUT TYPE="HIDDEN" ID="SCREEN" NAME="SCREEN" VALUE="10">
  <INPUT TYPE="HIDDEN" ID="SOURCE" NAME="SOURCE" VALUE="<%= userPO.getSource() %>">
  <INPUT TYPE="HIDDEN" ID="OPTION" NAME="OPTION" VALUE="<%= userPO.getAccOpt() %>">
  <INPUT TYPE="HIDDEN" ID="CHECKNUM" NAME="CHECKNUM" VALUE="<%= userPO.getHeader15() %>">
  <INPUT TYPE="HIDDEN" ID="SEQ" NAME="SEQ" VALUE="<%= userPO.getHeader18() %>"> 
      ...
      <td nowrap> 
        <table cellspacing=0 cellpadding=2 width="100%" border="0" background="<%=request.getContextPath()%>/images/s/dat_check.gif">
          <tr> 
            ...
            <td nowrap width="44%"><%= userPO.getHeader4().trim()%> </td>
          </tr>
          <tr > 
            <td nowrap width="31%"> 
  ...
  <p>&nbsp;</p>
	    ...
	    <div align="center"> 
    	  <input id="EIBSBTN" type=submit onclick="Action()" name="Submit" value="Enviar">
   		</div>
