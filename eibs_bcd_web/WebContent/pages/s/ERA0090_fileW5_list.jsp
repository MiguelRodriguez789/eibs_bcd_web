...
function submitThis(option) {
	if (document.forms[0].REF.value !='') {;
		document.forms[0].E05RW4REF.value=document.forms[0].REF.value;
	}   
	switch (option) {
            //lista registros del archivo seleccionado
    case 5 :{
		showMntW5(document.forms[0].E05RW4REF.value);
       }
		break;
            //
    case 7 :{
...
function showMntW5(ref) {
	var SCREEN = "108";
	var OPECOD = "0005";
	var E05RW4REF = ref;
	page = "<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0090?SCREEN="+SCREEN+"&OPECOD="+OPECOD+"&E05RW4REF="+E05RW4REF;
	CenterNamedWindow(page,'Information',1200,500,2);
}

function goRefresh() {
	if ( isValidObject( getElement("E05NUMREG"))) {
		document.forms[0].Pos.value = document.forms[0].E05NUMREG.value-20;
	}
...
<h3 align="center">Mantenimiento Tabla Garantías FNG para Interface a CREDISMART 
...
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0090">
  <p> 
    ...
    <input type=HIDDEN name="E05RW4REF" id="E05RW4REF" >
...
<TABLE  id="mainTable" class="tableinfo" ALIGN=CENTER height="70%">
    <TR id="trdark"> 
      ...
      <TH ALIGN=CENTER nowrap>Número<br>Operación</TH>
      <TH ALIGN=CENTER nowrap>Número<br>Operación FNG</TH>
    </TR>
    <%
                appList.initRow();
                int k=1;
	...
	<TD WIDTH="50%" ALIGN=LEFT>
  	<%
        if ( appList.getShowPrev() ) {
      			...
      			out.println("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.client.JSERA0090?SCREEN=107&Pos=" + pos +"\"><IMG border=\"0\" src=\""+request.getContextPath()+"/images/s/previous_records.gif\" ></A>");
        }
   %>  
	</TD>
	<TD WIDTH="50%" ALIGN=RIGHT>     
 	<% 
        if ( appList.getShowNext() ) {
      			...
      			out.println("<A HREF=\""+request.getContextPath()+"/servlet/datapro.eibs.client.JSERA0090?SCREEN=107&Pos=" + pos +"\"><IMG border=\"0\" src=\""+request.getContextPath()+"/images/s/next_records.gif\" ></A>");
        }
  	%> 
	</TD>
</TR>
