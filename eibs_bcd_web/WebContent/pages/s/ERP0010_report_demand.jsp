...
function setInfo(table, row) {
	...
	document.getElementById("actRow").value = row;
}

function goProcess() {
    var row = parseInt(document.getElementById("actRow").value);   
	document.forms[0].action = "<%=request.getContextPath()%>/servlet/datapro.eibs.reports.JSERP0010?SCREEN=26&ROW=" + row;	
...
<h3 align="center">Generar Reportes o Ejecutar Procesos<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="report_demand.jsp, ERP0010"></h3>
<hr size="4">
  ...
  <TABLE  id="mainTable" class="tableinfo">
   ...
   <%          
          dvList.initRow();               
               ...
               ERP001000Message msgList = (ERP001000Message) dvList.getRecord();			 
                    
       %>             
                    
		<tr id="dataTable<%= dvList.getCurrentRow() %>">
			<td nowrap align="center" width="5%"><input type="radio" id="ROW" name="ROW" value="<%=dvList.getCurrentRow()%>" <%=chk%> class="highlight" onClick="setInfo('dataTable', <%= dvList.getCurrentRow() %>)"/></td>
