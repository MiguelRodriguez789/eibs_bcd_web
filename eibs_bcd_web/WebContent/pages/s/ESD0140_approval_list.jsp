...
<script type="text/javascript">
  ...
  var nWarn = 0;
     if (op == 'D' || op == 'R') {               
		   for ( var i=0; i < dataT.rows.length; i++ ) {
		       if (getElement("ACCNUM" + actvTb + i).checked) {
		         nChk++;
		         if (nChk == 2) break;
		         acc = getElement("ACCNUM" + actvTb + i).value;
		       }		       
		   }
           ...
           var clName = "";
     	   for ( var i=0; i < dataT.rows.length; i++ ) {
     	       if (getElement("ACCNUM" + actvTb + i).checked) {
		         nChk++;
		         if (getElement("ACCNUM" + actvTb + i).className.trim() != "") {
		         	nWarn++;
		         	if (nWarn > 0 && nChk > 1) break;
		         	clName = getElement("ACCNUM" + actvTb + i).className;
		         	acc = getElement("ACCNUM" + actvTb + i).value;	
		         }	         
		       }		       
		   }
 ...
 function goAction(op) {
    ...
    if (document.forms[0].OFAFL.value.trim() !== "" && op != 'Z' && op != 'D') {
	    ...
	    if (document.forms[0].WRGFL.value.trim() !== "" && op != 'D') {
 ...
 function setSelection(chk){
   ...
   var dataT = getElement("dataTable"+actvTb);
   for ( var i=0; i < dataT.rows.length; i++ ) {
       getElement("ACCNUM" + actvTb + i).checked = chk;
   }
 } 
 ...
 function showNewInqApproval(app, pos, type) {	
	...
	var actvTb = document.forms[0].activeTable.value;
	var account = getElement("ACCNUM" + actvTb + pos).value;
	document.forms[0].SELALL.checked = false; 
	setSelection(false);
	getElement("ACCNUM" + actvTb + pos).checked = true; 
	page = "${pageContext.request.contextPath}/servlet/datapro.eibs.approval.JSESD0140?SCREEN=3&ACCNUM=" + account + "&appCode=" + appcode + "&typeCode=" + type;
	CenterWindow(page,600,500,2);
 }
...
<BODY>
<h3 align="center">Aprobaci&oacute;n General<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="approval_list.jsp,ESD0140">
</h3>
<hr size="4">
    ...
    <%
	                ...
	                while (accList.getNextRow()) {
		            	...
		            	if (accbean.getH01FLGWK2().trim().equals("A")) {
							warnImg= "<IMG border=\"0\" src=\"" + request.getContextPath() +"/images/warning01.gif\" title=\"...\" onClick=\"showWarnings('" + accbean.getE01ACCNUM() + "','ACCNUM"+ j + "" + k + "')\">";
				        ...
				        if (accbean.getH01FLGWK3().trim().equals("3")) {
							ofacImg = "<IMG border=\"0\" src=\"" + request.getContextPath() + "/images/warning_16.jpg\" title=\"OFAC Match List\" onClick=\"showOfac('" + accbean.getE01ACCNUM() + "')\">";
					...
					<TR>
						<TD NOWRAP>
							<input type="checkbox" name="ACCNUM<%=j%><%=k%>" <%if (classN.trim() != "" ) out.print("class=\""+classN+"\"");%> value="<%=accTemp%>">
						</TD>
					<% if ( accbean.getE01LNENUM().equals("0")){ %>
