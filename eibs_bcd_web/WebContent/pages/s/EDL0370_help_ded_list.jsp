...
function enter(code, desc, amount, bank, branch, currency, ledger) {
	var form = top.opener.document.forms[0];
 	if(form[top.opener.fieldName]) 		form[top.opener.fieldName].value = code;
	if(form[top.opener.fieldDesc]) 		form[top.opener.fieldDesc].value = desc;
 	if(form[top.opener.fieldAmount]) 	form[top.opener.fieldAmount].value = amount;
 	if(form[top.opener.fieldBank]) 		form[top.opener.fieldBank].value = bank;
 	if(form[top.opener.fieldBranch]) 	form[top.opener.fieldBranch].value = branch;
 	if(form[top.opener.fieldCurrency]) 	form[top.opener.fieldCurrency].value = currency;
 	if(form[top.opener.fieldLedger]) 	form[top.opener.fieldLedger].value = ledger;
	top.close();
 } 
//-->
</script>
...
<BODY>
<h3 align="center">Lista de Deducciones<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" alt="help_ded_list.jsp,EDL0370"></h3>
<hr size="4">
    ...
    %>
        <TR> 
			<TD ALIGN=center NOWRAP><div nowrap>
				<a href="javascript:enter('<%= brkList.getRecord(4) %>', '<%= brkList.getRecord(9) %>', '<%= brkList.getRecord(5) %>', '<%= brkList.getRecord(1) %>', '<%= brkList.getRecord(2) %>', '<%= brkList.getRecord(3) %>', '<%= brkList.getRecord(7) %>')"> <%= brkList.getRecord(4) %></a></DIV></TD>
			<TD ALIGN=LEFT NOWRAP><div nowrap>
				<a href="javascript:enter('<%= brkList.getRecord(4) %>', '<%= brkList.getRecord(9) %>', '<%= brkList.getRecord(5) %>', '<%= brkList.getRecord(1) %>', '<%= brkList.getRecord(2) %>', '<%= brkList.getRecord(3) %>', '<%= brkList.getRecord(7) %>')"><%= brkList.getRecord(9) %></a></DIV></TD>      
			<TD ALIGN=center NOWRAP><div nowrap>
				<a href="javascript:enter('<%= brkList.getRecord(4) %>', '<%= brkList.getRecord(9) %>', '<%= brkList.getRecord(5) %>', '<%= brkList.getRecord(1) %>', '<%= brkList.getRecord(2) %>', '<%= brkList.getRecord(3) %>', '<%= brkList.getRecord(7) %>')"><%= brkList.getRecord(5) %></a></DIV></TD>
			<TD ALIGN=center NOWRAP><div nowrap>
				<a href="javascript:enter('<%= brkList.getRecord(4) %>', '<%= brkList.getRecord(9) %>', '<%= brkList.getRecord(5) %>', '<%= brkList.getRecord(1) %>', '<%= brkList.getRecord(2) %>', '<%= brkList.getRecord(3) %>', '<%= brkList.getRecord(7) %>')"><%= brkList.getRecord(6) %></a></DIV></TD>
			<TD ALIGN=center NOWRAP><div nowrap>
				<a href="javascript:enter('<%= brkList.getRecord(4) %>', '<%= brkList.getRecord(9) %>', '<%= brkList.getRecord(5) %>', '<%= brkList.getRecord(1) %>', '<%= brkList.getRecord(2) %>', '<%= brkList.getRecord(3) %>', '<%= brkList.getRecord(7) %>')">
