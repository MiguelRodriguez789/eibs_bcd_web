...
<jsp:useBean id="ECO024002List" class="datapro.eibs.beans.JBObjList" scope="session" />
...
<script type="text/javascript">
	...
	function checkvalidatemoney(checks) {
		if (checks.length > 0){//mas de un registro
			var ccy='';
			var ccyant='';
		    for (i = 0; lcheck = checks[i]; i++) {
		        if (lcheck.checked) {
		            ccy = eval('document.forms[0].CCY'+i+'.value');
		            if (ccyant=='') ccyant = ccy;
		            if (ccy!=ccyant){
		            	return true;
		            }
		        }
	...
	function validar(){
	    if (!checkvalidate(document.forms[0].E02PLHSEL)) {
	    	alert('Debe seleccionar al menos una Planilla para continuar.!!!');
	    	return false;
	    }

	    if (checkvalidatemoney(document.forms[0].E02PLHSEL)) {
	    	alert('Debe seleccionar Planillas con la misma Moneda!!!');
	    	return false;
	    }    
		return true;
	}
...
<h3 align="center">Planillas pendientes a pagar<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="pago_planillas_lista.jsp, ECO0240"></h3>
<hr size="4">
...
<!-- 					
 ...
 -->					
		...
		<table id="dataTable">
			<% 
				int ix = 0;
				ECO024002List.initRow();
					while (ECO024002List.getNextRow()) {
						ECO024002Message convObj = (ECO024002Message) ECO024002List.getRecord();
			%>
			<tr>
	            <td nowrap width="5%">
	            	<input type="checkbox" name="E02PLHSEL"  onclick="" value="<%=ix%>"
	            		<%=("Y".equals(convObj.getE02PLHSEL())?"checked='checked'":"")%> >
				...
				<td nowrap align="center"><%=Util.formatCustomDate(currUser.getE01DTF(),
																 convObj.getBigDecimalE02PLHFED().intValue(),
																 convObj.getBigDecimalE02PLHFEM().intValue(),
																 convObj.getBigDecimalE02PLHFEY().intValue())%></td>
