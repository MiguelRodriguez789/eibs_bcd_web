...
function ReportDocVars() {
	...
	if (nCheck == 0)
		document.write(' None of DocAll, DocLay, DocDom is set;',
				' Dynamic Write will fail.'.italics(), '<p>');
	if (DocAll)
		document.write(' DocAll is set. ');
	if (DocLay)
		document.write(' DocLay is set. ');
	if (DocDom)
		document.write(' DocDom is set. ');
}

function TableDocVars() {
	document
			.writeln(
					'<br><br><table summary="Browser class info"',
					...
					'Your browser gives<br>', '<b>DocAll = ', DocAll,
					' ; DocDom = ', DocDom, ' ; DocLay = ', DocLay,
					'.<\/b><br>', 'Check : ', nCheck,
					' browser classification(s) set.', '<\/td><\/tr><\/table>');
}

// Define Function DynWrite(Where, What) to suit browser :
DocStr = '';
if (DocLay)
	DocStr = "return document.layers['NS_'+id]";
if (DocAll)
...
if (DocStr == '') {
	...
	if (DocAll || DocDom) {
		DynWrite = new Function("id", "S",
				"GetRef(id).innerHTML=S; return true");
	}
	// if (DocLay) DynWrite=new Function("id", "S", "var
	// x=GetRef(id).document;"+
	// "x.open('text/html'); x.write(S); x.close(); return true")
	if (DocLay)
		DynWrite = new Function("if (0==DynWarn++)"
...
function LZZ(x) {
	return (x < 0 || x > 99 ? x : "0" + LZ(x));
}

if (!String.substr) {
	String.prototype.substr = new Function("J", "K",
			"return this.substring(J, J+K)");
}
...
function StrT(X, M, N) {
	return Prepend(StrU(X, 1, N), M + N + 2, ' ');
}
...
function StrW(X, M, N) {
	return Prepend(StrS(X, 1, N), M + N + 2, ' ');
}
...
function SigFigExp(X, N) { // N<22
	if (X == 0)
		return SigFigExp(1, N).replace(/\+1/, ' 0');
	var p = Math.floor(Math.log(Math.abs(X)) / Math.LN10);
	if (!isFinite(p))
		return X;
	return (X > 0 ? '+' : '')
			+ String(Math.round(X * Math.pow(10, N - p - 1))).replace(/(\d)/,
					"$1.") + (p >= 0 ? "e+" : "e-") + LZ(Math.abs(p));
} // All OK?
