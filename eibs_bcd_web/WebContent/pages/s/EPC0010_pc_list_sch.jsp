...
function goAction(op) {
 ...
 var page="";
 if (op == 1) {
     page = '${pageContext.request.contextPath}/pages/s/EPC0010_pc_new_sch.jsp';
 } else if (op == 2) {
     page = '${pageContext.request.contextPath}/pages/s/EPC0010_pc_maint_sch.jsp?ROW=' + document.forms[0].ROW.value;
 } else if (op == 4) {     
     page = '${pageContext.request.contextPath}/pages/s/EPC0010_pc_delete_sch.jsp?ROW=' + document.forms[0].ROW.value; 
 }
...
<FORM Method="post" Action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEPC0010">
<INPUT TYPE=HIDDEN NAME="SCREEN" VALUE="2">
<INPUT TYPE=HIDDEN NAME="OPT" VALUE="3">
<INPUT TYPE=HIDDEN NAME="ROW" VALUE="0">

<h3 align="center">Participantes de Pool Asociados a Proyecto<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="pc_list_sch.jsp,EPC0010"> 
</h3>
<hr size="4">
      ...
      <TD> 
        <div align="center">       		
          <p><b>No hay resultados que correspondan a su criterio de búsqueda. 
            Haga Click en la opci&oacute;n Nuevo para asociar un participante
            </b></p>
          </div>
      </TD>
     </TR>
 ...
 <TR> 
   <TD NOWRAP valign="top" width="100%">
    ...
    <TR id="trdark"> 
      ...
      <TH ALIGN=CENTER nowrap>Participante</TH>
      <TH ALIGN=CENTER nowrap> </TH>
      <TH ALIGN=CENTER nowrap>% Participación</TH>            
      <TH ALIGN=CENTER nowrap>Valor Participación</TH>            
    </TR>
    </TABLE>
     ...
     %> 
            <TR> 
              <TD ALIGN=CENTER NOWRAP> 
                <INPUT TYPE="radio" NAME="EXTCHG" VALUE="<%= extList.getCurrentRow() %>" <% if (extList.getCurrentRow() == row) out.print("checked"); %> onClick="showInfo(<%= extList.getCurrentRow() %>,'<%=msgPCA.getE01PCPNUM()%>')">
              </TD>
              <TD ALIGN=CENTER NOWRAP><%= Util.formatCell(msgPCA.getE01PCPNUM()) %></TD>
              <TD ALIGN=LEFT NOWRAP><%= Util.formatCell(msgPCA.getE01PCPNM1()) %></TD>
              <TD ALIGN=LEFT NOWRAP><%= Util.formatCell(msgPCA.getE01PCPNM2()) %></TD>
              <TD ALIGN=RIGHT NOWRAP><%= Util.formatCell(msgPCA.getE01PCPPPR()) %></TD>
              <TD ALIGN=RIGHT NOWRAP><%= Util.formatCell(msgPCA.getE01PCPVPR()) %></TD>
             </TR>
      <%              
         }        
    %>
