...
public class JSUpload extends SuperServlet {
	/**
	...
	/**
	 * Insert the method's description here. Creation date: (9/5/2000 3:47:48
	 * PM)
	 * 
	 * @param out
	 ...
	 */
	public synchronized void saveXMLOutput(OutputStream out)
		...
		root.addContent(new Comment(
				"Documents and FTP Config for eIBSScan Tool"));
		...
		if (!JSEIBSProp.getFtpImgFirewallType().equals(""))
			ftp
					.addAttribute("FirewallType", JSEIBSProp
							.getFtpImgFirewallType());
		if (!JSEIBSProp.getFtpImgFWAuthenticate().equals(""))
			ftp.addAttribute("FWAuthenticate", JSEIBSProp
					.getFtpImgFWAuthenticate());
		...
		try {
			...
			System.out.println("Exception ocurred sending xml. Exception = "
					+ e);
		}
	}
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		if (session == null) {
			...
			try {
				...
				System.out
						.println("Exception ocurred sending xml. Exception = "
								+ e);
			}
		}
	}
