...
public class JSESD0817B extends JSESD0817 {
	...
	protected void procReqSTEnterSearch(ESS0030DSMessage user,
		...
		forward("ESD0817_bg_enter_stat.jsp", req, res);
	}
	...
	protected void procReqList(ESS0030DSMessage user, HttpServletRequest req,
		...
		if (!attIdentifier.trim().equals("")) {
			userPO.setIdentifier(attIdentifier.trim()); 
		}
		...
		try {
			...
			msgHeader = (ESD081702Message) mp.receiveMessageRecord();
			JBObjList list = mp.receiveMessageRecordList("E01INDOPE");
			ELEERRMessage msgError = new ELEERRMessage();
			
			if (mp.hasError(list)) {
				msgError = (ELEERRMessage) mp.getError(list);
				pageName = "ESD0817_bg_enter_stat.jsp";
			} else {
				list = Util.sortByDate(list, user.getE01DTF(), "E01TRABDM", "E01TRABDD", "E01TRABDY", "E01TRAACR");
				list.initRow();
				...
				pageName = "ESD0817_bg_list_fp.jsp";
			}
			flexLog("Putting java beans into the session");
			session.setAttribute("userPO", userPO);
			session.setAttribute("error", msgError);
	...
	protected void procReqPrintList(ESS0030DSMessage user,
		...
		try {
			...
			if (mp.hasError(list)) {
				...
				session.setAttribute("lcBalances", msgHeader);
				session.setAttribute("cifList", beanList);
				pageName = "ESD0817_bg_list_print_fp.jsp";
			}
			flexLog("Putting java beans into the session");
			session.setAttribute("userPO", userPO);
			session.setAttribute("error", msgError);
