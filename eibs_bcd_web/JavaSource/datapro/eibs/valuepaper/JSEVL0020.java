...
public class JSEVL0020 extends datapro.eibs.master.SuperServlet {
	...
	protected static final int R_LIST_UPDATE_R = 3000;
	protected static final int A_UPDATE_R = 6000;
	 ...
	 * This method was created in VisualAge.
	 */
	protected void procReqListUpdateReas(
			MessageContext mc,
			ESS0030DSMessage user,
			HttpServletRequest req,
			HttpServletResponse res,
			...
			MessageRecord newmessage = null;
			ELEERRMessage msgError = null;
			EVL002004Message msgSearch = null;
			EVL002004Message msgList = null;
			...
			try {
				flexLog("Send Initial Data");
				msgSearch = (EVL002004Message) mc.getMessageRecord("EVL002004");
				msgSearch.setH04USERID(user.getH01USR());
				msgSearch.setH04PROGRM("EVL0020");
				msgSearch.setH04TIMSYS(getTimeStamp());
				msgSearch.setH04SCRCOD("01");
				msgSearch.setH04OPECOD("0035");
			    	
				msgSearch.send();
				msgSearch.destroy();
				flexLog("EVL002004 Message Sent");
			
				// Receive Message
			
				newmessage = mc.receiveMessage();
			   
				if (newmessage.getFormatName().equals("EVL002004")) {
					...
					while (true) {
						...
						msgList = (EVL002004Message) newmessage;
					...
					try {
						flexLog("About to call Page: " + LangPath + "EVL0020_value_paper_bal_update_reasigna.jsp");
						callPage(LangPath + "EVL0020_value_paper_bal_update_reasigna.jsp", req, res);						
	 ...
	 * This method was created in VisualAge.
	 */
	protected void procActionUpdateReas(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
		...
		try {
			...
			msgMT = (EVL002004Message) beanList.getRecord();			
			msgMT.setH04OPECOD("0005");
						
			msgMT.setE04BALAQT(req.getParameter("E04BALAQT"));
			msgMT.setE04BALAIN(req.getParameter("E04BALAIN"));
			msgMT.setE04BALAFI(req.getParameter("E04BALAFI"));
			msgMT.setE04BALMOT(req.getParameter("E04BALMOT"));
			msgMT.setE04BALNBR(req.getParameter("E04BALNBR"));
			msgMT.setE04BALLOT(req.getParameter("E04BALLOT"));
			...
			if (newmessage.getFormatName().equals("EVL002004")) {
				...
				msgMT = (EVL002004Message) newmessage;
				
				if (IsNotError) {
					res.sendRedirect(srctx + "/servlet/datapro.eibs.valuepaper.JSEVL0020?SCREEN=3000");
//					String href = req.getContextPath() + "/pages/s/MISC_search_wait.jsp?URL='" + req.getContextPath() + "/servlet/datapro.eibs.valuepaper.JSEVL0020?SCREEN=3000'";
////					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.valuepaper.JSEVL0020?SCREEN=3000");
//					res.setContentType("text/html");
//					PrintWriter out = res.getWriter();
//					out.println("<HTML>");
//					out.println("<HEAD>");
...
//					out.close();				
					...
					ses.setAttribute("appList", beanList);
					try {
						flexLog("About to call Page: " + LangPath + "EVL0020_value_paper_bal_update_detail_reasigna.jsp");
						res.sendRedirect(req.getContextPath() + LangPath + "/EVL0020_value_paper_bal_update_detail_reasigna.jsp?ROW=" + row);								
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EVL0020", req));
					...
					switch (screen) {
						...
						case R_LIST_UPDATE :							
							...
							break;					
						case R_LIST_UPDATE_R :							
							procReqListUpdateReas(mc, msgUser, req, res, session);
							break;
						case A_UPDATE_R :
							procActionUpdateReas(mc,msgUser, req, res, session);
							break;
						default :
