...
public class JSEDL0530 extends datapro.eibs.master.SuperServlet {
	...
	public void destroy() {
		...
		flexLog("free resources used by JSEDL0530");
	...
	protected void procActionEnterMaint(
		...
		EDL053001Message msgClasif = null;
		...
		// Send Initial data
		try {
			msgClasif = (EDL053001Message) mc.getMessageRecord("EDL053001");
			msgClasif.setH01USERID(user.getH01USR());
			msgClasif.setH01PROGRM("EDL0530");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDL053001")) {
				try {
					msgClasif = new datapro.eibs.beans.EDL053001Message();
					flexLog("EDL053001 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgClasif = (EDL053001Message) newmessage;
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EDL0530_client_clasif.jsp");
						callPage(
							LangPath + "EDL0530_client_clasif.jsp",
							req,
							res);
					} catch (Exception e) {
					...
					try {
						flexLog(
							"About to call Page: /pages/"
								+ LangPath
								+ "EDL0530_client_clasif_enter.jsp");
						callPage(
							LangPath + "EDL0530_client_clasif_enter.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
	...
	protected void procActionClasif(
		...
		try {
			...
			//msgClasif = (EDL053001Message)mc.getMessageRecord("EDL053001");
			msgClasif = (EDL053001Message) ses.getAttribute("clasif");
			msgClasif.setH01USERID(user.getH01USR());
			msgClasif.setH01PROGRM("EDL0530");
			msgClasif.setH01TIMSYS(getTimeStamp());
			msgClasif.setH01SCRCOD("01");
			...
			msgClasif.destroy();
			flexLog("EDL053001 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
			throw new RuntimeException("Socket Communication Error");
			...
			if (newmessage.getFormatName().equals("EDL053001")) {
				...
				if (IsNotError) { // There are no errors
					if (userPO.getOption().equals("CLIENT_P")) {
						callPage(
							LangPath + "ESD1080_client_basic.jsp",
							req,
							res);
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EDL0530_client_clasif_enter.jsp");
			callPage(LangPath + "EDL0530_client_clasif_enter.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDL0530", req));
