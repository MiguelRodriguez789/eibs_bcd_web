...
public class JSERA0011 extends datapro.eibs.master.SuperServlet {
	...
	/********************************************/	
	/** Enter to maintain a collateral account **/
	/********************************************/
	...
	/***************************************/	
	/** Maintenance to collateral account **/
	/***************************************/
					...
					if (marker.equals("*")) {
						...
						myRow += "<TD NOWRAP>"
							+ Util.formatCustomDate(user.getE01DTF(),
										msgList.getBigDecimalE04ROCMTM().intValue(),
										msgList.getBigDecimalE04ROCMTD().intValue(),
										msgList.getBigDecimalE04ROCMTY().intValue())
																					+ "</A></TD>";
						myRow += "<TD NOWRAP>"
	...
	*/
	...
	/*********************************************/	
	/** Action to open a new collateral account **/
	/*********************************************/
	...
	/********************************************/	
	/** Enter to open a new collateral account **/
	/********************************************/
	...
	protected void procActionEnterNew(
		...
		try {
			...
			try {
				msgGa.setE01ROCCUN(req.getParameter("CUSNUM"));
			}
			catch (Exception e) {
			}
			try {
				msgGa.setE01ROCSTN(req.getParameter("REFERE"));
			}
			catch (Exception e) {
			}

			try {
				msgGa.setE01ROCPRD(req.getParameter("PRODUCT"));
			}
			catch (Exception e) {
			}
			try {
				msgGa.setE01ROCBNK(req.getParameter("bank"));
			}
			catch (Exception e) {
			}
...
//			}
	...
	*/
	...
	protected void procReqEnterNew(MessageContext mc, ESS0030DSMessage user,
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					msgError = (datapro.eibs.beans.ELEERRMessage) Beans
				...
				int gaColNum = 4;
				try {
					beanList = (datapro.eibs.beans.JBListRec) Beans
							.instantiate(getClass().getClassLoader(),
									"datapro.eibs.beans.JBListRec");
					beanList.init(colNum);
					gcodeList = (datapro.eibs.beans.JBListRec) Beans
							.instantiate(getClass().getClassLoader(),
									"datapro.eibs.beans.JBListRec");
				...
				while (true) {
					...
					if (marker.equals("*")) {
						...
						if (!myFlag.equals(msgGa.getE01CNOSCG())) {
							...
							// { newmessage = mc.receiveMessage();
							// continue; }
							myGaCode[0] = msgGa.getE01CNOATY(); // collateral
																// type
							myGaCode[1] = msgGa.getE01CNODTY(); // description
							myGaCode[2] = msgGa.getE01CNOSCG(); // account
							myGaCode[3] = msgGa.getE01CNOSCY(); // Currency
