	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ERA0010", req));
		...
		if (newmessage.getFormatName().equals("ERA001002")) {
			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					myRow[4] =  msgColl.getE02RODMDM();  // Maturity date MM
					myRow[5] =  msgColl.getE02RODMDD();  // Maturity date DD
					myRow[6] =  msgColl.getE02RODMDY();  // Maturity date YY
	...
	userPO = (datapro.eibs.beans.UserPos)ses.getAttribute("userPO");
	 ...
	 try{
		...
		// Get Parameters here	
			...
			try {
				msgColl.setE02RODMDM(trList.getRecord(4));  // Maturuty Day MM
			}
			catch (Exception e){	
			}
			try {	
				msgColl.setE02RODMDD(trList.getRecord(5));  // Maturuty Day DD
			}
			catch (Exception e){} 
			try {	
				msgColl.setE02RODMDY(trList.getRecord(6));  // Maturuty Day YY
			}
			catch (Exception e){} 	
