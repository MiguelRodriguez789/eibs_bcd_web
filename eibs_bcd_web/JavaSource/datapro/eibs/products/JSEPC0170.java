...
public class JSEPC0170 extends datapro.eibs.master.SuperServlet {
...
public void destroy() {
	...
	flexLog("free resources used by JSEPC0170");
...
protected void procReqSchemePCList(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgPC = (EPC017001Message)mc.getMessageRecord("EPC017001");
	 	msgPC.setH01USERID(user.getH01USR());
	 	msgPC.setH01PROGRM("EPC0170");
	 	msgPC.setH01TIMSYS(getTimeStamp());
	 	msgPC.setH01SCRCOD("01");
	 	msgPC.setH01OPECOD("0015");
	 	// Get Parameters here		
		try	{ 	
	 		msgPC.setE01PCVACC(userPO.getIdentifier());
		}
		catch (Exception e)	{
		} 	
		msgPC.send();	
	...
	try
	{
	  newmessage = mc.receiveMessage();
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog("About to call Page: " + LangPath + "EPC0170_pc_enter_sch.jsp");
					callPage(LangPath + "EPC0170_pc_enter_sch.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
		} 
		else if (newmessage.getFormatName().equals("EPC017001")) {
			...
			while (true) {
				...
				msgPC = (EPC017001Message)newmessage;
			...
			try {
				userPO.setCusNum(msgPC.getE01PCVCUN());
			}
			catch (Exception e)	{
			}

			try {
				userPO.setCusName(msgPC.getD01PCVCUN());
			}
			catch (Exception e)	{
			}
...
//ojo
		  if (userPO.getHeader23() == null) {
			...
			try {
				 	flexLog("About to call Page: " + LangPath + "EPC0170_pc_list_sch.jsp");
				 	callPage(LangPath + "EPC0170_pc_list_sch.jsp", req, res);					
			}
			catch (Exception e) {
					flexLog("Exception calling page " + e);
			}			
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EPC0170", req));
