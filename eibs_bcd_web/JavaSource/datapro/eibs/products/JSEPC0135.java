...
public class JSEPC0135 extends datapro.eibs.master.SuperServlet {
...
public void destroy() {
	...
	flexLog("free resources used by JSEPC0135");
...
protected void procReqSchemePCList(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgPC = (EPC013501Message)mc.getMessageRecord("EPC013501");
	 	msgPC.setH01USERID(user.getH01USR());
	 	msgPC.setH01PROGRM("EPC0135");
	 	msgPC.setH01TIMSYS(getTimeStamp());
	 	msgPC.setH01SCRCOD("01");
	 	msgPC.setH01OPECOD("0015");
	 	// Get Parameters here		
		try	{ 	
	 		msgPC.setE01PDEACC(userPO.getIdentifier());
		}
		catch (Exception e)	{
		} 	
		msgPC.send();	
	...
	try
	{
	  newmessage = mc.receiveMessage();
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog("About to call Page: " + LangPath + "EPC0135_pc_enter_sch.jsp");
					callPage(LangPath + "EPC0135_pc_enter_sch.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
		} 
		else if (newmessage.getFormatName().equals("EPC013501")) {
			...
			while (true) {
				...
				msgPC = (EPC013501Message)newmessage;
			...
			try {
				userPO.setCusNum(msgPC.getE01PDECUN());
			}
			catch (Exception e)	{
			}

			try {
				userPO.setCusName(msgPC.getD01PDECUN());
			}
			catch (Exception e)	{
			}
...
//ojo
		  if (userPO.getHeader23() == null) {
			...
			try {
				 	flexLog("About to call Page: " + LangPath + "EPC0135_pc_list_sch.jsp");
				 	callPage(LangPath + "EPC0135_pc_list_sch.jsp", req, res);					
			}
			catch (Exception e) {
					flexLog("Exception calling page " + e);
			}			
				...
				try {
//						flexLog("About to call Page: " + LangPath + "EPC0135_pc_list_inq_sch.jsp");
//						callPage(LangPath + "EPC0135_pc_list_inq_sch.jsp", req, res);					
					flexLog("About to call Page: " + LangPath + "EPC0135_pc_list_sch.jsp");
					callPage(LangPath + "EPC0135_pc_list_sch.jsp", req, res);					
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EPC0135", req));
