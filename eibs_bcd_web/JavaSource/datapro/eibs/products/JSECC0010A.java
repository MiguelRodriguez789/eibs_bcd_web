...
public class JSECC0010A extends datapro.eibs.master.SuperServlet {

	// credit card 
	protected static final int R_APPROVAL_INQUIRY = 3;
	 ...
	 */
	protected void procReqApprovalInquiry(
		MessageContext mc,
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("ECC001001")) {
					...
					userPO.setHeader9(msgCC.getE01CCMPRO());
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("ESD000002")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "ECC0140_cc_ap_codes.jsp");
						callPage(LangPath + "ECC0140_cc_ap_codes.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected void procReqEspInst(
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog("About to call Page: "	+ LangPath	+ "ECC0140_cc_ap_special_inst.jsp");
					callPage(LangPath + "ECC0140_cc_ap_special_inst.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("ECC0010", req));
				...
				switch (screen) {
					// Request
					case R_APPROVAL_INQUIRY :
						procReqApprovalInquiry(mc, msgUser, req, res, session);
						break;
