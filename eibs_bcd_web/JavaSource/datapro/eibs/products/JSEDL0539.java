...
public class JSEDL0539 extends datapro.eibs.master.SuperServlet {
...
protected void procActionApproval(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		msgList = (EDL053902Message)mc.getMessageRecord("EDL053902");
		msgList.setH02USERID(user.getH01USR());
	 	msgList.setH02PROGRM("EDL0539");
	 	msgList.setH02TIMSYS(getTimeStamp());
  		try {
	  		
		 	msgList.setE02DEAACC(req.getParameter("REFNUM"));
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
			...
			if (IsNotError) { // There is no error
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.products.JSEDL0539?SCREEN=1");
			}
			else {
				try {
					 flexLog("About to call Page: " + LangPath + "EDL0539_approval_list.jsp");
					 callPage(LangPath + "EDL0539_approval_list.jsp", req, res);
					 //flexLog("About to call Page: " + LangPath + "EDL0539_approval_container.jsp");
					 //callPage(LangPath + "EDL0539_approval_container.jsp", req, res);
				}
				catch (Exception e) {
...
protected void procReqApproval(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	MessageRecord newmessage = null;
	ELEERRMessage msgError = null;
	EDL053901Message msgList = null;
	JBList beanList = null;
	UserPos	userPO = null;

	//Grab Search Parameters
	EDL053901Message searchParameters = (EDL053901Message) ses.getAttribute("searchParameters");
	if(null != searchParameters){
		setMessageRecord(req, searchParameters);
	} else {
		searchParameters = new EDL053901Message();
	}		
	...
	{
		flexLog("Send Initial Data");
		msgList = (EDL053901Message)mc.getMessageRecord("EDL053901");
		msgList.setH01USERID(user.getH01USR());
	 	msgList.setH01PROGRM("EDL0539");
	...
	try
	{
	  	...
	  	if (newmessage.getFormatName().equals("ELEERR")) {
	  	...
	  	else if (newmessage.getFormatName().equals("EDL053901")) {
			...
			while (true) {
				...
				msgList = (EDL053901Message)newmessage;
				...
				if (marker.equals("*")) {
					...
					if (firstTime) {
						...
						if (msgList.getE01DEAACC().trim().equals(refNum))
							chk = "checked";
						else 
							chk = "";
					}
					...
					myRow.append("<TD NOWRAP width=\"5%\" ALIGN=\"CENTER\"><input type=\"radio\" name=\"REFNUM\" value=\"" + msgList.getE01DEAACC() + "\" " + chk + " onclick=\"showAddInfo("+indexRow+")\"></TD>");
					myRow.append("<TD NOWRAP width=\"15%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalAdi('" + msgList.getE01DEAACC() + "')\">" + Util.formatCell(msgList.getE01DEAACC()) + "</A>"+ chkWarn+ "</TD>");;
					myRow.append("<TD NOWRAP width=\"25%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalAdi('" + msgList.getE01DEAACC() + "')\">" + Util.formatCell(msgList.getE01CUSCUN()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"25%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalAdi('" + msgList.getE01DEAACC() + "')\">" + Util.formatCell(msgList.getE01CUSNA1()) + "</A></TD>");
					myRow.append("<TD NOWRAP width=\"10%\" ALIGN=\"CENTER\"><A HREF=\"javascript:showInqApprovalAdi('" + msgList.getE01DEAACC() + "')\">" + Util.formatCell(msgList.getE01CUSIDN()) + "</A></TD>");
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EDL0539", req));
...
protected void procReqPassword(HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		...
		userPO = (datapro.eibs.beans.UserPos)ses.getAttribute("userPO");
		userPO.setRedirect("/servlet/datapro.eibs.products.JSEDL0539?SCREEN=" + R_APPROVAL);
		ses.setAttribute("userPO", userPO);
