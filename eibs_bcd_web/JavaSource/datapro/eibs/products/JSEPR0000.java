...
public class JSEPR0000 extends JSEIBSServlet {
	...
	private static final int R_NEW_ACH 		= 7;
	private static final int A_NEW_ACH 		= 8;
	private static final int R_MAINT_ACH 	= 9;
	private static final int A_MAINTENANCE_ACH 	= 10;
		...
		throws ServletException, IOException {
			...
			switch (screen) {
				...
				case R_NEW :
					...
					break;
				case R_NEW_ACH :
					procReqNewACH(user, req, res, session);
					break;	
				case R_MAINTENANCE :
					procReqMaintenance(user, req, res, session);
					break;
				case R_MAINT_ACH :
					procReqMaintACH(user, req, res, session);
					break;
				case R_PRIORITY :
				...
				case A_NEW :
					...
					break;
				case A_NEW_ACH :
					procActionNewACH(user, req, res, session);
					break;
				case A_PRIORITY :
				...
				case A_MAINTENANCE :
					...
					break;
				case A_MAINTENANCE_ACH :
					procActionMaintach(user,	req, res, session);
					break;
				case A_INCOMING :
	 ...
	 * @param session
	 */
	private void procActionMaintach(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		UserPos userPO = (UserPos) session.getAttribute("userPO");
		String PageToCall = "";
		MessageProcessor mp = null;
		try {
			...
			if (mp.hasError(msgError)) {
				PageToCall = "EPR0000_pr_maint_ach.jsp";
			} else {
				PageToCall = "EPR0000_pr_confirm.jsp";
			}
			forward(PageToCall, req, res);
	 ...
	 * @param session
	 */
	private void procActionNewACH(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		UserPos userPO = (UserPos) session.getAttribute("userPO");
		String PageToCall = "";
		MessageProcessor mp = null;
		try {
	...
	private void procReqMaintenance(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		if (!refnum.equals("")){
			session.setAttribute("TrfRef",refnum );
		}
		if(refnum.equals("")){
			try{
				refnum=session.getAttribute("TrfRef").toString();
			}catch(Exception e){}
		}
	 ...
	 * @param session
	 */
	private void procReqMaintACH(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		UserPos userPO = (UserPos) session.getAttribute("userPO");
		String refnum = (req.getParameter("REFNUM") == null) ? "" : req.getParameter("REFNUM");
		if (!refnum.equals("")){
			session.setAttribute("TrfRef",refnum );
		...
		try {
			mp = getMessageProcessor("EPR0100", req);
			EPR010001Message msg = (EPR010001Message) mp.getMessageRecord("EPR010001", user.getH01USR(), "0012");
			msg.setH01SCRCOD("01");
			msg.setE01PRINUM(refnum);
	...
	private void procReqNew(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		session.removeAttribute("prMant");
	...
	private void procReqNewACH(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		forward("EPR0000_pr_maint_ach.jsp", req, res);
	}
