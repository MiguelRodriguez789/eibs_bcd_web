...
public class JSEDL0130I extends datapro.eibs.master.SuperServlet {
	...
	protected void procActionCalcInt(
		...
		try {
			...
			try {
				...
				try {
					msgCD.setE01DEAODM(req.getParameter("E01DEAODM"));
					msgCD.setE01DEAODD(req.getParameter("E01DEAODD"));
					msgCD.setE01DEAODY(req.getParameter("E01DEAODY"));
				} catch (Exception e) {
				...
				try {
					msgCD.setE01DEAMDM(req.getParameter("E01DEAMDM"));
					msgCD.setE01DEAMDD(req.getParameter("E01DEAMDD"));
					msgCD.setE01DEAMDY(req.getParameter("E01DEAMDY"));
				} catch (Exception e) {
				...
				try {
					msgCD.setE01VENCIM(req.getParameter("E01VENCIM"));
					msgCD.setE01VENCID(req.getParameter("E01VENCID"));
					msgCD.setE01VENCIY(req.getParameter("E01VENCIY"));
				} catch (Exception e) {
	...
	protected void procActionEnterPrint(
		...
		try {
			...
			if (newmessage.getFormatName().equals("EFT000010")) {
				...
				userPO.setCurrency(msgCD.getE10DEACCY());
				userPO.setHeader9(msgCD.getE10BANKNM());
				userPO.setHeader10(msgCD.getE10CUSIDN());
				userPO.setHeader11(msgCD.getE10CNTRDM());
				userPO.setHeader12(msgCD.getE10CNTRDD());
				userPO.setHeader13(msgCD.getE10CNTRDY());
	...
	protected void procReqProductCD(
		...
		MessageRecord newmessage = null;
		ESD070001Message msgProdCD = null;
		ELEERRMessage msgError = null;
		UserPos userPO = null;
		boolean IsNotError = false;
		...
		String prodCode = userPO.getProdCode();

				// Send Initial data
		try {
			msgProdCD = (ESD070001Message) mc.getMessageRecord("ESD070001");
		 	msgProdCD.setH01USERID(user.getH01USR());
		 	msgProdCD.setH01PROGRM("ESD0700");
		 	msgProdCD.setH01TIMSYS(getTimeStamp());
		 	msgProdCD.setH01SCRCOD("01");
		 	msgProdCD.setH01OPECOD("0002");
		 	msgProdCD.setE01APCCDE(prodCode);
		 	msgProdCD.setE01APCBNK(bank);
			msgProdCD.send();
			msgProdCD.destroy();
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
			...
			} else if (newmessage.getFormatName().equals("ESD070001")) {
				try {
					msgProdCD = new datapro.eibs.beans.ESD070001Message();
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgProdCD = (ESD070001Message) newmessage;

				flexLog("Putting java beans into the session");
				ses.setAttribute("prd", msgProdCD);
				ses.setAttribute("error", msgError);
				...
				try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "ESD0700_products_tds.jsp");
					callPage(
						LangPath + "ESD0700_products_tds.jsp",
						req,
						res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
	...
	protected void procReqRel(
		...
		try {
			...
			msgRel.setE02PRIACC(userPO.getAccNum());
				...
				try {
					...
					switch (screen) {
						...
						case R_ENTER_INQUIRY_BONO :
							procReqEnterInquiryBono(msgUser, req, res, session);
							break;	
						case R_ENTER_PRINT :
	...
	protected void procActionCalcPty(
		...
		try {
			...
			try {
				...
				try {
					msgCalc.setE07TRNVDM(req.getParameter("E07TRNVD1"));
				} catch (Exception e) {
					flexLog("E07TRNVD1");
				}
				try {
					msgCalc.setE07TRNVDD(req.getParameter("E07TRNVD2"));
				} catch (Exception e) {
					flexLog("E07TRNVD2");
				}
				try {
					msgCalc.setE07TRNVDY(req.getParameter("E07TRNVD3"));
				} catch (Exception e) {
					flexLog("E07TRNVD3");
				}
			} catch (Exception e) {
				...
				if (newmessage.getFormatName().equals("ELEERR")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow[1] =
									Util.formatCustomDate(user.getE01DTF(),
											msgPmnt.getE04DLPPDM(),
											msgPmnt.getE04DLPPDD(),
											msgPmnt.getE04DLPPDY());
							// Date
