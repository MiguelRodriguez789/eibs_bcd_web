...
public class JSEPC0145 extends datapro.eibs.master.SuperServlet {
...
public void destroy() {
	...
	flexLog("free resources used by JSEPC0145");
...
protected void procReqSchemePCList(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgPC = (EPC014501Message)mc.getMessageRecord("EPC014501");
	 	msgPC.setH01USERID(user.getH01USR());
	 	msgPC.setH01PROGRM("EPC0145");
	...
	try
	{
	  newmessage = mc.receiveMessage();
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
				...
				try {
					flexLog("About to call Page: " + LangPath + "EPC0145_pc_enter_sch.jsp");
					callPage(LangPath + "EPC0145_pc_enter_sch.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
		} 
		else if (newmessage.getFormatName().equals("EPC014501")) {
			...
			while (true) {
				...
				msgPC = (EPC014501Message)newmessage;
...
//ojo
		  if (userPO.getHeader23() == null) {
			...
			try {
				 	flexLog("About to call Page: " + LangPath + "EPC0145_pc_list_sch.jsp");
				 	callPage(LangPath + "EPC0145_pc_list_sch.jsp", req, res);					
			}
			catch (Exception e) {
					flexLog("Exception calling page " + e);
			}			
				...
				try {
//						flexLog("About to call Page: " + LangPath + "EPC0145_pc_list_inq_sch.jsp");
//						callPage(LangPath + "EPC0145_pc_list_inq_sch.jsp", req, res);					
					flexLog("About to call Page: " + LangPath + "EPC0145_pc_list_sch.jsp");
					callPage(LangPath + "EPC0145_pc_list_sch.jsp", req, res);					
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EPC0145", req));
