...
public class JSEGL0035 extends JSEIBSServlet {
	...
	private void procActionEnter(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		if (transData.getBranch() == null) {
			transData.setBranch(user.getE01UBR());		}
	...
	private void procReqEnter(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		try {
			...
			msg.setE01WRKFIL("2");
			if (null != option  && option.equals("FACT")) {
				msg.setE01OPTION("F");
			}
			mp.sendMessage(msg);
	...
	private void procActionTr(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		for (int row = 0; row < maxRow; row++) {
			...
			if ((opt == 1) & (checked.equals("checked"))) {
				...
				String fromFile = "N";
				try {
					fromFile = req.getParameter("E01PROFIL");
				} catch (Exception e) {
					
				}
				if (null != fromFile && !fromFile.equals("Y")) {
				...
				try {
					...
					myRow[47] = req.getParameter("E01WRKIVP_" + row).toUpperCase();
					myRow[49] = req.getParameter("E01WRKIVB_" + row).toUpperCase();
					myRow[51] = req.getParameter("E01WRKCDT_" + row).toUpperCase();
					myRow[52] = req.getParameter("E01WRKRCL_" + row).toUpperCase();
				} catch (Exception e) {
				}
				...
				try {
					...
					myRow[47] = req.getParameter("E01WRKIVP").toUpperCase();
				}	
				try {
					myRow[49] = req.getParameter("E01WRKIVB_" + row).toUpperCase();
				} catch (Exception e) {
					myRow[49] = req.getParameter("E01WRKIVB").toUpperCase();
				}	
				try {
					myRow[51] = req.getParameter("E01WRKCDT_" + row).toUpperCase();
				} catch (Exception e) {
					myRow[51] = req.getParameter("E01WRKCDT").toUpperCase();
				}	
				try {
					myRow[52] = req.getParameter("E01WRKRCL_" + row).toUpperCase();
				} catch (Exception e) {
					myRow[52] = req.getParameter("E01WRKRCL").toUpperCase();
				}	

				// Source and Destinations only on atlantida
				try {
					myRow[50] = req.getParameter("E01WRKCOD_" + row);
				} catch (Exception e) {
					myRow[50] = req.getParameter("E01WRKCOD");
				}	
				try {
					String[] myfield;
				...
				if (null != userPO.getType() && userPO.getType().equals("FACT")) {
					try {
						myRow[23] = req.getParameter("E01WRKDS9_" + row);
					} catch (Exception e) {
						myRow[23] = " ";
					}
				}
		...
		transData.setDescription(req.getParameter("E01WRKTDS") == null ? "" : req.getParameter("E01WRKTDS"));
		transData.setDate1(req.getParameter("E01WRKVD1") == null ? "" : req.getParameter("E01WRKVD1"));
		transData.setDate2(req.getParameter("E01WRKVD2") == null ? "" : req.getParameter("E01WRKVD2"));
		transData.setDate3(req.getParameter("E01WRKVD3") == null ? "" : req.getParameter("E01WRKVD3"));
	...
	private void procActionTrBeansToSck(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session, int opt) throws ServletException, IOException {
		...
		String fileName = "";
		try {
			directorio = req.getParameter("E01FILDIR");
		} catch (Exception e) {
			
		}
		try {
			fileName = req.getParameter("E01FILENA");
		} catch (Exception e) {
		...
		try {
			...
			while (trList.getNextRow()) {
				...
				if (fromFile.equals("Y")) {
					try {
						msg.setE01FILDIR(directorio);
					} catch (Exception e) {
						
					}
					try {
						msg.setE01FILENA(fileName);
					} catch (Exception e) {
				...
				try {
					msg.setE01WRKIVP(trList.getRecord(47)); 
				} catch (Exception e) {
				}
				try {
					msg.setE01WRKIVB(trList.getRecord(49)); 
				} catch (Exception e) {
				}
				try {
					msg.setE01WRKCDT(trList.getRecord(51)); 
				} catch (Exception e) {
				}
				try {
					msg.setE01WRKRCL(trList.getRecord(52)); 
				} catch (Exception e) {
				}
				...
				try {
					msg.setE01WRKCOD(trList.getRecord(50)); 
				} catch (Exception e) {
				}
			...
			while (jbList.getNextRow()) {
				...
				myRow[47] = msg.getE01WRKIVP(); // Taxes
				myRow[49] = msg.getE01WRKIVB(); // Taxes
				myRow[50] = msg.getE01WRKCOD(); // Acct holder Name
				myRow[51] = msg.getE01WRKCDT();
				myRow[52] = msg.getE01WRKRCL().trim();
				if (!myFirstRow) {
					String myDesc = "";
					if (null != userPO.getType() && userPO.getType().equals("FACT")) {
						myDesc = myRow[9]
			...
			if (IsNotError && !fromFile.equals("Y") ) { // There are no errors and don't came from XML File
				userPO.setAccOpt("");
				session.setAttribute("userPO", userPO);
				session.setAttribute("transData", transData);
				if (transData.isFlagPrint()) {
					...
					if (userPO.getOption().equals("TRANSACTION")) {
						...
						forward("EP0150_ln_basic.jsp",	req, res);
	...
	private void procReqTr(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws ServletException, IOException {
		...
		try {
			...
			while (list.getNextRow()) {
				...
				myRow[47] = msg.getE01WRKIVP().trim();  //Taxes
				myRow[49] = msg.getE01WRKIVB().trim();  //Taxes
				myRow[50] = msg.getE01WRKCOD().trim();  //source/destination only on Atlantida
				myRow[51] = msg.getE01WRKCDT().trim(); //
				myRow[52] = msg.getE01WRKRCL().trim(); //
				if (!myFirstRow) {
					String myDesc = "";
					if (null != userPO.getType() && userPO.getType().equals("FACT")) {
