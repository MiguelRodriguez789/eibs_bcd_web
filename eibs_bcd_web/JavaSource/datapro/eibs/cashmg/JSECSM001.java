...
/**
 * Mantenimiento de la tabla de Comisiones Motor de pagos
...
public class JSECSM001 extends JSEIBSServlet { 
	...
	protected void procReqList(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		try {
			mp = new MessageProcessor("ECSM00101");
			ECSM00101Message msg = (ECSM00101Message) mp.getMessageRecord("ECSM00101");
			userPO = (datapro.eibs.beans.UserPos) ses.getAttribute("userPO");
			userPO.setPurpose("MAINTENANCE");
			// Send Initial data
			msg.setH01USERID(user.getH01USR());
			msg.setH01PROGRM("ECSM001");
		...
		forward("ECSM001_commissions_cash_management_list.jsp", req, res);
		} finally {
			if (mp != null)
				mp.close();
		}
		...
		try {
			...
			if (mp.hasError(newmessage)) {
				...
				if (screen == 2 || screen == 3 || screen == 4) {
					...
					forward("ECSM001_commissions_cash_management.jsp", req, res);
				}

			} else {
				ses.setAttribute("msgRcd", newmessage);
				if (screen == 5 || screen == 4) {     // Update or Delete
					res.sendRedirect(srctx + "/servlet/datapro.eibs.cashmg.JSECSM001?SCREEN=1&SEARCHCDE=" + userPO.getHeader2());	
				} else {
					forward("ECSM001_commissions_cash_management.jsp", req, res);
				}
			}
