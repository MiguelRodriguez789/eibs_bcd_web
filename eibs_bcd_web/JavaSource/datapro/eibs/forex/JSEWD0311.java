...
public class JSEWD0311 extends datapro.eibs.master.SuperServlet {
...
private void procReqPos(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		msgList = (EWD0311DSMessage)mc.getMessageRecord("EWD0311DS");
		...
		try{
	 	...
	 	msgList.send();	
	 	msgList.destroy();
	 	flexLog("EWD0311DS Message Sent");
	}		
	catch (Exception e)
	{
		e.printStackTrace();
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("ELEERR")) {
	  	...
	  	else if (newmessage.getFormatName().equals("EWD0311DS")) {
			...
			while (true) {
				...
				msgList = (EWD0311DSMessage)newmessage;
			...
			flexLog("Putting java beans into the session");
			ses.setAttribute("EWD0311help", beanList);
			ses.setAttribute("userPO", userPO);

			try {
				flexLog("About to call Page: " + LangPath + "EWD0311_payments_list.jsp");
				callPage(LangPath + "EWD0311_payments_list.jsp", req, res);
			}
			catch (Exception e) {
	...
	String ACCOUNT = req.getParameter("ACCOUNT");
			...
			switch (inptOPT) {
				...
				case 3  ://Inquiry
					...
					break;	
				case 5  ://Search
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.forex.JSEWD0311?SCREEN=1&AGREEMENT=" + AGREEMENT +"&CUSTOMER=" + CUSTOMER +"&ACCOUNT=" + ACCOUNT);
					break;																					
				default :
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EWD0311", req));
