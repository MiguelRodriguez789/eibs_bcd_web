	...
	if (session == null) {
		...
		try {
			...
			try {
				...
				mc = new MessageContext(super.getMessageHandler("EDL0130", req));
			switch (screen) {
				// Request
				case R_NEW :
			...
			case 1 :
				{
					...
					try {
						...
						while (pmntList.getNextRow()) {
							...
							try {
								msgPmnt.setE04DLPPDD(pmntList.getRecord(1)); //Date
							} catch (Exception e) {
							}

							try {
								msgPmnt.setE04DLPPDM(pmntList.getRecord(2)); //Date
							} catch (Exception e) {
							}

							try {
								msgPmnt.setE04DLPPDY(pmntList.getRecord(3)); //Date
							} catch (Exception e) {
							}
