...
protected void procActionEnterNDF(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		try {
		 	msgNDF.setE01FEMACC(req.getParameter("E01FEMACC"));
		}
		catch (Exception e)	{
	 	  msgNDF.setE01FEMACC("");
		}
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EFE0150DS")) {
			...
			userPO.setIdentifier(msgNDF.getE01FEMACC());
			userPO.setCusNum(msgNDF.getE01FEMCUN());
			userPO.setCusName(msgNDF.getD01FEMCP1());
			userPO.setCurrency(msgNDF.getE01FEMCCY());
			userPO.setProdCode(msgNDF.getD01APCDSC());
...
protected void procActionNDFSettlement(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		try {
		     msgNDF.setE01FEMNRT(req.getParameter("E01FEMNRT"));
		}
		catch (Exception e) {  flexLog("Error: " + e);}	
		
		flexLog("E01FEMNRT"	+ req.getParameter("E01FEMNRT") );		
...
protected void procActionNDFTck(ESS0030DSMessage user,
	...
	//		msgNDF.setH01FLGWK2("2");
			...
			try {                                     
				...
				try {   
					msgNDF.setE01FEMACC(userPO.getAccOpt());      
				}
            		
				catch (Exception e1)	{ msgNDF.setE01FEMACC(""); }
			}
		...
		try
		{
			...
			if (newmessage.getFormatName().equals("EFE0150DS")) {
				...
				ses.setAttribute("error", msgError);
				ses.setAttribute("ndf", msgNDF);
				if (!msgNDF.getE01FEMACC().equals(""))	
					userPO.setAccOpt(msgNDF.getE01FEMACC());
				//	userPO.setE01FEMACC(msgNDF.getE01FEMACC());
				ses.setAttribute("userPO", userPO);			 
				if (IsNotError) {  // There are no errors
						try {					
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EFE0150",req));
