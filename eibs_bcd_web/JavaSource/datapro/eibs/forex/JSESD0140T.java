			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					if (msgList.getH01FLGWK2().trim().equals("A")) {
						warnImg= "<IMG border=\"0\" src=\"../images/warning01.gif\" title=\""+txtAlt+"\" onClick=\"showWarnings('" + msgList.getE01ACCNUM() + "','ACCNUM"+indexCustRow+"')\">";
			        ...
			        if (msgList.getH01FLGWK3().trim().equals("3")) {
						ofacImg= "<IMG border=\"0\" src=\"../images/warning_16.jpg\" title=\"OFAC Match List\" onClick=\"showOfac('" + msgList.getE01ACCNUM() + "')\">";
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ESD0140", req));
