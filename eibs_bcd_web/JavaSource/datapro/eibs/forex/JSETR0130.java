...
public class JSETR0130 extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		try {
		 	if (req.getParameter("E01DLSREF") != null)
		 	  msgFRA.setE01WFRREF(req.getParameter("E01DLSREF"));
		}
		catch (Exception e)	{
	 	  msgFRA.setE01WFRREF("0");
		}
		try {
		 	if (req.getParameter("E01DLSDID") != null)
		 	  msgFRA.setE01WFRDID(req.getParameter("E01DLSDID"));
		}
		catch (Exception e)	{
	 	  msgFRA.setE01WFRDID("");
		}
		try {
		 	if (userPO.getPurpose().equals("MANTFRA"))
...
protected void procActionFRA(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	protected synchronized void procReqFRAMaint(ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
		HttpSession ses)
		throws ServletException, IOException {
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("ETR0130", req));
