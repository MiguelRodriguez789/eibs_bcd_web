			...
			while (true) {
				...
				if (marker.equals("*")) {
					...
					if (firstTime) {
						...
						myRow.append("<TD NOWRAP ALIGN=CENTER>"+ Util.formatCustomDate(user.getE01DTF(),msgList.getSWDOPM(),msgList.getSWDOPD(),msgList.getSWDOPY()) + "</TD>");						
						myRow.append("</TR>");
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EWD0301", req));
