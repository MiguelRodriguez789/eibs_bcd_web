 ...
 * 	// List of Options
 * 	//
 *	// SCREEN =    1	Request Planilla File Name          
 *	// SCREEN =    2    Validate Planilla Incoming File 
 **	// SCREEN =    5    Process  Planilla Incoming File 
 */
...
public class JSEIMP400 extends JSEIBSServlet { 
	...
	protected void processRequest(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, 
		...
		switch (screen) {
			//  
			case 1 : // Request Planilla File Name 
				procReqFile(user, req, res, session);
				break;
			case 2 : // Valide Plainilla File
			case 5 : // Proccess Planilla File
	...
	protected void procReqFile(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		forwardOnSuccess("EIMP400_incoming_file.jsp", req, res);
	...
	protected void procActionFile(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			try {
				...
				String line = "";
				String linetosend = "";
				mp = new MessageProcessor("EIMP400");
				msg = (EIMP400DSMessage) mp.getMessageRecord("EIMP400DS");
				...
				while (true) {
					...
					if (line.length() > 0 ) {
						...
						boolean find = true;
						while (find == true) {
							if (line.substring(posIni, posIni + 93).length() > 93) {
								linetosend = line.substring(posIni, posIni + 93);
								posIni = posIni + 93;					
							} else {
				...
				if (mp.hasError(newmessage)) {
					...
					forward("EIMP400_incoming_file.jsp", req, res);
				} else {
					redirectToPage("/servlet/datapro.eibs.misc.JSEIMP400?SCREEN=1", res);
				}
			} catch (Exception e) {
				e.printStackTrace();
