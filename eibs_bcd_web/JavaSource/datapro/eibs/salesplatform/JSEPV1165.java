...
public class JSEPV1165 extends JSEIBSServlet {
	...
	protected void procActionTablaCargosList(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session)
		...
		try {
			...
			userPO.setCusNum(req.getParameter("E01PVHCUN") != null ? req.getParameter("E01PVHCUN").trim() : userPO.getCusNum());
			userPO.setPorfNum(req.getParameter("E01PVHNUM") != null ? req.getParameter("E01PVHNUM").trim() : userPO.getPorfNum());
			userPO.setHeader1(req.getParameter("E01PVHRF1") != null ? req.getParameter("E01PVHRF1").trim() : userPO.getHeader1());
			
			mp = getMessageProcessor("EPV1165", req);

			EPV116501Message msg = (EPV116501Message) mp.getMessageRecord("EPV116501");
			msg.setH01USERID(user.getH01USR());
			msg.setH01PROGRM("EPV1165");
			msg.setH01TIMSYS(getTimeStamp());
			msg.setH01SCRCOD(appType);
			msg.setH01OPECOD("0015");
			
			msg.setE01PVHCUN(userPO.getCusNum());
			msg.setE01PVHNUM(userPO.getPorfNum());
			msg.setE01PVHRF1(userPO.getHeader1());
	...
	private void procReqCargosNew(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			...
			mp = getMessageProcessor("EPV1165", req);
			EPV116501Message msgNew = (EPV116501Message) mp.getMessageRecord("EPV116501");
			msgNew.setH01USERID(user.getH01USR());
			msgNew.setH01PROGRM("EPV1165");
			msgNew.setH01TIMSYS(getTimeStamp());
			msgNew.setH01SCRCOD(appType);
			msgNew.setH01OPECOD("0001");
			
			msgNew.setE01PVHCUN(userPO.getCusNum());
			msgNew.setE01PVHNUM(userPO.getPorfNum());
			msgNew.setE01PVHRF1(userPO.getHeader1());
			
			mp.sendMessage(msgNew);
			
			ELEERRMessage msgError = (ELEERRMessage) mp.receiveMessageRecord("ELEERR");
			msgNew = (EPV116501Message) mp.receiveMessageRecord("EPV116501");
	...
	private void procReqCargosMaint(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		UserPos userPO = getUserPos(req);
		MessageProcessor mp = null;
		EPV116501Message listMessage = null;
		...
		try {
			...
			if (req.getParameter("key") != null) {
				JBObjList list = (JBObjList)session.getAttribute("EPV116501List");
				int index = Integer.parseInt(req.getParameter("key").trim());
				listMessage = (EPV116501Message)list.get(index);
			}

			EPV116501Message msgMaint = (EPV116501Message) mp.getMessageRecord("EPV116501");
			msgMaint.setH01USERID(user.getH01USR());
			msgMaint.setH01PROGRM("EPV1165");
			msgMaint.setH01TIMSYS(getTimeStamp());
			msgMaint.setH01SCRCOD(appType);
			msgMaint.setH01OPECOD("0002");
			
			if (listMessage == null) {
				msgMaint.setE01PVHCUN(req.getParameter("cusNum").trim());
				msgMaint.setE01PVHNUM(req.getParameter("PorfNum").trim());
				msgMaint.setE01PVHSEQ(req.getParameter("E01PVHSEQ").trim());
			} else {
				msgMaint.setE01PVHCUN(listMessage.getE01PVHCUN());
				msgMaint.setE01PVHNUM(listMessage.getE01PVHNUM());
				msgMaint.setE01PVHSEQ(listMessage.getE01PVHSEQ());
			}

			mp.sendMessage(msgMaint);
			
			ELEERRMessage msgError = (ELEERRMessage) mp.receiveMessageRecord("ELEERR");
			msgMaint = (EPV116501Message) mp.receiveMessageRecord("EPV116501");
	...
	protected void procActionMaintenance(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session)
		...
		try {
			...
			ELEERRMessage msgError = (ELEERRMessage) mp.receiveMessageRecord("ELEERR");
			msg = (EPV116501Message) mp.receiveMessageRecord("EPV116501");
	...
	protected void procReqDelete(ESS0030DSMessage user, HttpServletRequest req,
		...
		try {
			...
			JBObjList list = (JBObjList)session.getAttribute("EPV116501List");
			int index = Integer.parseInt(req.getParameter("key").trim());
			EPV116501Message listMessage = (EPV116501Message)list.get(index);
			
			EPV116501Message msgDelete = (EPV116501Message) mp.getMessageRecord("EPV116501");
			msgDelete.setH01USERID(user.getH01USR());
			msgDelete.setH01PROGRM("EPV1165");
			msgDelete.setH01TIMSYS(getTimeStamp());
			msgDelete.setH01SCRCOD(appType);
			msgDelete.setH01OPECOD("0009");
			
			msgDelete.setE01PVHCUN(listMessage.getE01PVHCUN());
			msgDelete.setE01PVHNUM(listMessage.getE01PVHNUM());
			msgDelete.setE01PVHSEQ(listMessage.getE01PVHSEQ());
