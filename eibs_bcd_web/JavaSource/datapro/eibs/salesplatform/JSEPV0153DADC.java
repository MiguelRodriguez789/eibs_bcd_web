...
//DADC = EPV0153 Desembolso; Aumento/Decremento; Cambio de Status
...
import datapro.eibs.beans.EPV015304Message;
import datapro.eibs.beans.EPV015305Message;
import datapro.eibs.beans.EPV015306Message;
import datapro.eibs.beans.EPV015307Message;
...
public class JSEPV0153DADC extends JSEIBSServlet {
	...
	private void procReqEventReason(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session)
		...
		EPV015307Message msgEvent = (EPV015307Message) mp.getMessageRecord("EPV015307");	
		...
		forward ("EPV0153_cuplus_evento_reason.jsp",req,res );
	}	
	...
	private void procReqEventEnter(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		mp = getMessageProcessor("EPV0153", req);
		EPV015301Message msgEvent = (EPV015301Message) mp.getMessageRecord("EPV015301");		
		...
		forward("EPV0153_cuplus_eventos_enter.jsp", req, res);
	}	
	...
	private void procActDes(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session)
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015304Message msgDesem = (EPV015304Message) session.getAttribute("RecData");
			msgDesem.setH04USERID(user.getH01USR());
			msgDesem.setH04PROGRM("EPV0153");
			...
			msgDesem = (EPV015304Message) mp.receiveMessageRecord();
			...
			if (!mp.hasError(msgError)) {
				...
				session.setAttribute("userPO", userPO);
				session.setAttribute("error", msgError);
				forward("EPV0153_cuplus_desmb_info.jsp", req, res);
			}
	...
	private void procActIncre(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015305Message msgIncr = (EPV015305Message) session.getAttribute("RecData");
			msgIncr.setH05USERID(user.getH01USR());
			msgIncr.setH05PROGRM("EPV0153");
			...
			ELEERRMessage msgError = (ELEERRMessage) mp.receiveMessageRecord();
			msgIncr = (EPV015305Message) mp.receiveMessageRecord();
			...
			if (!mp.hasError(msgError)) {
				...
				session.setAttribute("userPO", userPO);
				session.setAttribute("error", msgError);
				forward("EPV0153_cuplus_incre_info.jsp", req, res);
			}
	...
	private void procActStatus(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015306Message msgStatus = (EPV015306Message) session.getAttribute("RecData");
			msgStatus.setH06USERID(user.getH01USR());
			msgStatus.setH06PROGRM("EPV0153");
			...
			msgStatus = (EPV015306Message) mp.receiveMessageRecord();
			...
			if (!mp.hasError(msgError)) {
				...
				session.setAttribute("userPO", userPO);
				session.setAttribute("error", msgError);
				forward("EPV0153_cuplus_cambio_status_info.jsp", req, res);
			}
	...
	private void procActEventList(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015307Message msgList = (EPV015307Message) mp.getMessageRecord("EPV015307");		
			msgList.setH07USERID(user.getH01USR());
			msgList.setH07PROGRM("EPV0153");
			...
			if (mp.hasError(msgError)) {
				...
				EPV015301Message HeadInfo = (EPV015301Message) mp.getMessageRecord("EPV015301");
				...
				session.setAttribute("EPV015307List", list);
				session.setAttribute("userPO", userPO);
				forward("EPV0153_cuplus_eventos_list.jsp", req, res);
			}
	...
	private void procReqDesmbolso(ESS0030DSMessage user,HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015304Message msgDesem = (EPV015304Message) mp.getMessageRecord("EPV015304");
			msgDesem.setH04USERID(user.getH01USR());
			msgDesem.setH04PROGRM("EPV0153");
			msgDesem.setH04TIMSYS(getTimeStamp());
	    	msgDesem.setH04SCRCOD("01");
			...
			if (!mp.hasError(msgError)) {
				...
				if(!userPO.getHeader1().equals("APPROVAL")){	
					...
					forward("EPV0153_cuplus_desmb_info.jsp?readOnly=true", req, res);
				}
			} else {
				session.setAttribute("RecData", msgDesem);
				session.setAttribute("userPO", userPO);
	...
	private void procReqAumentoDism(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015305Message msgIncr = (EPV015305Message) mp.getMessageRecord("EPV015305");
			msgIncr.setH05USERID(user.getH01USR());
			msgIncr.setH05PROGRM("EPV0153");
			msgIncr.setH05TIMSYS(getTimeStamp());
	    	msgIncr.setH05SCRCOD("01");
			...
			if (!mp.hasError(msgError)) {
				...
				if(!userPO.getHeader1().equals("APPROVAL")){				
					...
					forward("EPV0153_cuplus_incre_info.jsp?readOnly=true", req, res);
				}
			}else{
				session.setAttribute("userPO", userPO);
				session.setAttribute("error", msgError);
	...
	private void procReqCambioEstatus(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession session) 
		...
		try {
			mp = getMessageProcessor("EPV0153", req);
			EPV015306Message msgStatus = (EPV015306Message) mp.getMessageRecord("EPV015306");
			msgStatus.setH06USERID(user.getH01USR());
			msgStatus.setH06PROGRM("EPV0153");
			msgStatus.setH06TIMSYS(getTimeStamp());
	    	msgStatus.setH06SCRCOD("01");
			...
			if (!mp.hasError(msgError)) {
				...
				if(!userPO.getHeader1().equals("APPROVAL")){				
					...
					forward("EPV0153_cuplus_cambio_status_info.jsp?readOnly=true", req, res);
				}
			}else{
				session.setAttribute("userPO", userPO);
				session.setAttribute("error", msgError);
