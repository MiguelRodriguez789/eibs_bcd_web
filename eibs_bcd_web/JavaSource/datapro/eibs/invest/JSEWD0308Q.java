...
public class JSEWD0308Q extends datapro.eibs.master.SuperServlet {
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("EWD0308DS")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=CENTER><input type=\"radio\" name=\"index\" value=\""
								...
								+ "\" "
								+ chk
								+ " onclick=\"getParams('" + msgList.getSWDIIC() + "')\"></TD>");
					...
					try {
						flexLog("About to call Page: " + LangPath + "EWD0308Q_sel_instruments.jsp");
						callPage(LangPath + "EWD0308Q_sel_instruments.jsp", req, res);
					} catch (Exception e) {
	...
	protected void procReqEnterSearch(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
		...
		try {
			flexLog("About to call Page: " + LangPath + "EIE0060_inv_enter_inst.jsp");
			callPage(LangPath + "EIE0060_inv_enter_inst.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
	...
	protected void procReqEnterAdvSearch(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
		...
		try {
			flexLog("About to call Page: " + LangPath + "EIE0060_inv_inst_adv_search.jsp");
			callPage(LangPath + "EIE0060_inv_inst_adv_search.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
		...
		switch (inptOPT) {
			case 1 : //Get Quote
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0060?SCREEN=4" + "&CODE=" + CODE);		    
				break;
			case 5 : //Search
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEWD0308Q?SCREEN=100" + "&SYMBOL=" + SYMBOL
					+ "&ISIN=" + ISIN + "&SERIAL=" + SERIAL + "&CUSIP=" + CUSIP);
				break;
			case 6 : //Advance Search
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEWD0308Q?SCREEN=300");
				break;
