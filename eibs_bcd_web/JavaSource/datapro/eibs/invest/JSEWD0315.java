...
public class JSEWD0315 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqList(
		...
		try {
			...
			try { //ISIN
				...
				msgList.setSWDISI(req.getParameter("ISIN"));
			...
			try { //Currency
				...
				msgList.setSWDCCY(req.getParameter("CURRENCY"));
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEWD0315?SCREEN=100");
				...
				if (newmessage.getFormatName().equals("EWD0315DS")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							String PAYTYP = msgList.getSWDTYP();
							...
							if(PAYTYP == "C"){
								...
								PAYTYP = "Input Adjustment";
							} else if(PAYTYP == "O"){
								PAYTYP = "Output Adjustment";
							}
							...
							myRow.append("<TD NOWRAP  ALIGN=LEFT>" + msgList.getSWDCCY() + "</td>");
							myRow.append("<TD NOWRAP  ALIGN=CENTER>" + msgList.getSWDISI() + "</td>");
							myRow.append("<TD NOWRAP  ALIGN=CENTER>" + msgList.getSWDNME() + "</td>");														
							myRow.append("<TD NOWRAP  ALIGN=CENTER>" + msgList.getSWDCNM() + "</td>");
							myRow.append("<TD NOWRAP  ALIGN=CENTER>" + msgList.getSWDPDS() + "</td>");							
							myRow.append("<TD NOWRAP  ALIGN=RIGHT>" + Util.formatDate(msgList.getSWDPM1(),msgList.getSWDPM2(),msgList.getSWDPM3()) + "</td>");							
							...
							myRow.append("<TD NOWRAP  ALIGN=RIGHT>" + msgList.getSWDADS() + "</td>");
							myRow.append("</TR>");
					...
					try {
						flexLog("About to call Page: " + LangPath + "EWD0315_sel_coupon.jsp");
						callPage(LangPath + "EWD0315_sel_coupon.jsp", req, res);
					} catch (Exception e) {
		...
		switch (inptOPT) {
			...
			case 2 : //Approve
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0210?SCREEN=400" + "&CODE=" + CODE
