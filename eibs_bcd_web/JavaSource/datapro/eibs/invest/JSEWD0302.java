				...
				if (newmessage.getFormatName().equals("EWD0302DS")) {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=RIGHT><img src=\"../images/1bori.gif\" title=\"Pending Approval\" align=\"absmiddle\" border=\"0\" ><input type=\"radio\" name=\"PORTNUM\" value=\""
								+ msgList.getSWDNUM()
								+ "\" "
								+ chk
								+ " onclick=\"getParams('"
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EWD0302", req));
