...
public class JSEIE0130I extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE013001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130I_inv_trade_tickets.jsp");
						callPage(LangPath + "EIE0130I_inv_trade_tickets.jsp", req, res);	
					}
					catch (Exception e) {
...
protected  void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					...
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=900" + "&CODE=" + ORDNUM);
				
					}
					else {
					try {
						    flexLog("About to call Page: " + LangPath + "EIE0130I_inv_trade_tickets_confirm.jsp");
							callPage(LangPath + "EIE0130I_inv_trade_tickets_confirm.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("EIE013002")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130I_sel_list_purchase.jsp");
						callPage(LangPath + "EIE0130I_sel_list_purchase.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected  void procActionListMultiple(
		...
		switch (inptOPT) {
			case 1 : //New
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=1200" + "&ORDMAST=" + ORDMAST);		    
				break;
			case 2 : //Maintenance
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=1400" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM);
				break;
				
			case 3 : //Delete
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=1600" + "&ORDMAST=" + ORDMAST + "&ORDNUM=" + ORDNUM );
				break;
			case 10 : //Ticket Confirm
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=1800");
				break;	
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0130I?SCREEN=1200" + "&ORDMAST=" + ORDMAST);
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE013002")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0130I_inv_trade_tickets_mp.jsp");
						callPage(LangPath + "EIE0130I_inv_trade_tickets_mp.jsp", req, res);	
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
