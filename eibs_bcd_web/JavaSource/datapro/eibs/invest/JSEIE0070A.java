...
public class JSEIE0070A extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE007001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0070A_inv_app_payment.jsp");
						callPage(LangPath + "EIE0070A_inv_app_payment.jsp", req, res);	
					}
					catch (Exception e) {
				...
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0070A?SCREEN=900"+ "&CODE=" + CODE);		
			}
		}
		else
			flexLog("Message " + newmessage.getFormatName() + " received.");
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("EIE007001")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0070A_sel_list_payments.jsp");
						callPage(LangPath + "EIE0070A_sel_list_payments.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
		...
		switch (inptOPT) {
			case 4 : //Inquiry
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0070A?SCREEN=800" + "&CODE=" + CODE + "&DATE1=" + DATE1
					+ "&DATE2=" + DATE2 + "&DATE3=" + DATE3 + "&PAYMENT=" + PAYMENT);
				break;	
			
			case 10 : //Action Main List
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0050?SCREEN=1000" + "&CODE=" + userPO.getIdentifier());		    
				break;				
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0070A?SCREEN=200" + "&CODE=" + CODE);	
		}
	}
