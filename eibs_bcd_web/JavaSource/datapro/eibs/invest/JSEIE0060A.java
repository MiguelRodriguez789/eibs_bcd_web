...
public class JSEIE0060A extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE006001")) {
			...
			if (IsNotError) {  // There are no errors
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0060A_inv_app_quote_basic.jsp");
						callPage(LangPath + "EIE0060A_inv_app_quote_basic.jsp", req, res);	
					}
					catch (Exception e) {
				...
				res.sendRedirect(super.srctx + "/servlet/datapro.eibs.invest.JSEIE0060A?SCREEN=4");			}
		}
		else
			flexLog("Message " + newmessage.getFormatName() + " received.");
		...
		switch (inptOPT) {
			...
			case 4 : //Inquiry
			    res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0060A?SCREEN=800" + "&CODE=" + CODE 
			...
			default :
				res.sendRedirect(super.srctx + 
					"/servlet/datapro.eibs.invest.JSEIE0060A?SCREEN=200" + "&CODE=" + CODE);
		}
	}
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (newmessage.getFormatName().equals("EIE006001")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EIE0060A_sel_list_quotes.jsp");
						callPage(LangPath + "EIE0060A_sel_list_quotes.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
