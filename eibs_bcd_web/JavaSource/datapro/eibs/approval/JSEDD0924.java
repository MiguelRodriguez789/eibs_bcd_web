...
public class JSEDD0924 extends datapro.eibs.master.SuperServlet {
	...
	protected static final int A_CONFIRM_ITEMS = 20;	
		...
		try {
			...
			beanList.getNextRow();
				...
				try {
					...
					msgSearch.setH03TIMSYS(getTimeStamp());
					msgSearch.setH03SCRCOD("01");
					msgSearch.setE03DEVOF1(dataCR.getOfficer());
					msgSearch.setE03NEWCHK(dataCR.getBranch());
					msgSearch.setH03OPECOD("1000");					
	...
	protected void procReqList(
		...
		try {
			...
			if (req.getParameter("E01FILACC") != null && !req.getParameter("E01FILACC").trim().equals("")){
				msgSearch.setE01FILACC(req.getParameter("E01FILACC"));
			}
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				while (true) {
					...
					if (marker.equals("*")) {
						...
						myRow[2] = msgSearch.getE01DEVCHK(); // Check Number
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				while (true) {
					...
					if (marker.equals("*")) {
						...
						myRow[2] = msgSearch.getE02DEVCHK(); // Check Number
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDD0924", req));
