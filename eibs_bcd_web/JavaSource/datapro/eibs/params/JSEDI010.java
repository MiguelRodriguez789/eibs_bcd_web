...
public class JSEDI010 extends datapro.eibs.master.SuperServlet {
					...
					while (true) {
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=CENTER width=\"30%\">" + 
									(msgList.getE01DCIACD().equals("AC") ? "Cuenta" : 
										(msgList.getE01DCIACD().equals("CC") ? "Cliente Corporativo" : 
											(msgList.getE01DCIACD().equals("CP") ? "Cliente Personal" : 
												msgList.getE01DCIACD().equals("PC") ? "Propuesta de Crédito" : 
													("")))).toUpperCase()
									+ "</td>");
	...
	protected void procReqNew(
		...
		try {
			...
			msgDoc.setE02DCIACD(req.getParameter("CODE"));
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDI0101", req));
