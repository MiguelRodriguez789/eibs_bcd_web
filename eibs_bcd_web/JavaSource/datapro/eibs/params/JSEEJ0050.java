...
public class JSEEJ0050 extends datapro.eibs.master.SuperServlet {

	static final int R_LIST  	= 1;
	static final int R_DETAIL  	= 100;
	static final int R_CHANGE  	= 110;
	static final int A_ACTION  	= 200;
	static final int A_CHANGE  	= 210;
	...
	public void destroy() {
		...
		flexLog("free resources used by JSEEJ0050");
	...
	protected void procReqList(
		...
		try {
			...
			msgList = (EEJ005001Message) mc.getMessageRecord("EEJ005001");
			...
			msgList.send();
			msgList.destroy();
			flexLog("EEJ005001 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
		...
		try {
			  newmessage = mc.receiveMessage();
				...
				if (newmessage.getFormatName().equals("EEJ005001")) {
					...
					while (true) {
						...
						msgList = (EEJ005001Message) newmessage;
						...
						if (marker.equals("*")) {
							...
							myRow.append("<TD NOWRAP  ALIGN=CENTER >" + msgList.getE01EJACD1() + "</td>");
						//	myRow.append("<TD NOWRAP  ALIGN=CENTER >" + Util.formatDate(msgList.getE01EJARD1(),msgList.getE01EJARD2(),msgList.getE01EJARD3())  + " - " + msgList.getE01EJARTI() + "</td>");
					...
					try {
						flexLog("About to call Page: " + LangPath + "EEJ0050_special_autoriz_list.jsp");
						callPage(LangPath + "EEJ0050_special_autoriz_list.jsp", req, res);
					} catch (Exception e) {
	...
	protected void procReqDetail(
		...
		// Send Initial data
		try {
			msgAut = (EEJ005001Message) mc.getMessageRecord("EEJ005001");
			msgAut.setH01USERID(user.getH01USR());
			msgAut.setH01PROGRM("EEJ0050");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EEJ005001")) {
				try {
					msgAut = new EEJ005001Message();
					flexLog("EEJ005001 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}
	
				msgAut = (EEJ005001Message) newmessage;
				...
				try {
					flexLog("About to call Page: " + LangPath + "EEJ0050_special_autoriz_detail.jsp");
					callPage(LangPath + "EEJ0050_special_autoriz_detail.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
	...
	protected void procReqChange(
		...
		try {
			...
			if (newmessage.getFormatName().equals("EEJ005001")) {
				...
				try {
					flexLog("About to call Page: " + LangPath + "EEJ0050_special_autoriz_change.jsp");
					callPage(LangPath + "EEJ0050_special_autoriz_change.jsp", req, res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				}
	...
	protected void procReqHistory(
		...
		// Send Initial data
		try {
			msgList = (EEJ005002Message) mc.getMessageRecord("EEJ005002");
			msgList.setH02USERID(user.getH01USR());
			msgList.setH02PROGRM("EEJ0050");
			...
			msgList.send();
			msgList.destroy();
			flexLog("EEJ005002 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
			throw new RuntimeException("Socket Communication Error");
		...
		try {
				...
				if (newmessage.getFormatName().equals("EEJ005002")) {
					...
					while (true) {
						...
						msgList = (EEJ005002Message) newmessage;
					...
					try {
						flexLog("About to call Page: " + LangPath + "EEJ0050_special_autoriz_history.jsp");
						callPage(LangPath + "EEJ0050_special_autoriz_history.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
	...
	protected void procActionApproval(
		...
		try {
			...
			if (option.equals("R")) {
				msgAut.setE01EJARMK(req.getParameter("E01EJARM1")+" "+req.getParameter("E01EJARM2")+" "+req.getParameter("E01EJARM3"));
			}
		...
		try {
			...
			if (newmessage.getFormatName().equals("EEJ005001")) {
				try {
					msgAut = new datapro.eibs.beans.EEJ005001Message();
					flexLog("EEJ005001 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}
				...
				if ((IsNotError)|| (msgError.getERNU01().equals("0273"))|| (msgError.getERNU01().equals("0834"))) {
					...
					try {
						...
						"/servlet/datapro.eibs.params.JSEEJ0050?SCREEN=1" );
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				} else {
	...
	protected void procActionChange(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
		...
		try {
			...
			try{
				msgAut.setE01EJASUP(req.getParameter("E01EJASUP"));
			} catch (Exception e) {
			}	
	
			try{
				msgAut.setE01EJAREF(req.getParameter("E01EJAREF"));
			} catch (Exception e) {
			}
	
			try{
					msgAut.setE01NEWRTE(req.getParameter("E01NEWRTE"));
				} catch (Exception e) {
				}

			try{
					msgAut.setE01EJARTE(req.getParameter("E01EJARTE"));
				} catch (Exception e) {
				}
	
			
			if (option.equals("R")) {
				msgAut.setE01EJARMK(req.getParameter("E01EJARM1")+" "+req.getParameter("E01EJARM1")+" "+req.getParameter("E01EJARM3"));
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EEJ0050", req));
				...
				switch (screen) {
					...
					case R_DETAIL :
						...
						break;
					case R_CHANGE :
						procReqChange(mc, msgUser, req, res, session);
						break;
					...
					case A_ACTION :
						...
						break;
					case A_CHANGE :
						procActionChange(mc, msgUser, req, res, session);
						break;
