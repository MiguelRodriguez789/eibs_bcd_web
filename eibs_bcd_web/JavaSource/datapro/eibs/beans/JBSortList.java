...
public class JBSortList {
	...
	public JBSortList(List list) {
		...
		while (it.hasNext()) {
			...
			if (type instanceof MessageRecord) {
				add((MessageRecord) type);
			}
		}
	}
	
	public JBSortList(List list, int orden) {
		this(list);
		this.orden = orden;
	}
	
	public JBSortList(List list, int orden, String field) {
		this(list, orden);
		setSortKey(new String[]{field});
	}
	...
	public MessageRecord getData(int index) {
		return ((sortElem) this.data.get(index)).getData();
	}
	...
	public void setSortKey(int[] colNum) {
		for (int i = 0; i < this.data.size(); i++) {
			sortElem element = (sortElem) this.data.get(i);
	...
	public void sort() {
		if (orden == ASCENDENTE)
			Collections.sort(this.data, newAscOrden);
		else
			Collections.sort(this.data, newDescOrden);
		this.baseIdx = 0;
	}
