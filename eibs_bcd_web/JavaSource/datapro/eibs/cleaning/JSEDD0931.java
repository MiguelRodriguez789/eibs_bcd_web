...
public class JSEDD0931 extends datapro.eibs.master.SuperServlet {
	...
	if (session == null) {
		...
		try {
			...
			if (screen == R_ENTER) {
				...
				mc = new MessageContext(super.getMessageHandler("EDD0931", req));
...
protected void  procReqEnter(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try {
		flexLog("About to call Page: " + LangPath + "EDD0931_enter_cleaning_load.jsp");
		callPage(LangPath + "EDD0931_enter_cleaning_load.jsp", req, res);	
	}
	catch (Exception e) {
...
protected void procActionEnter(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	EDD093101Message msgLOAD = null;
	EDD093102Message msgList = null;
	...
	try
	{
		...
		flexLog("Send Initial Data");
	 	msgLOAD = (EDD093101Message)mc.getMessageRecord("EDD093101");
		msgLOAD.setH01USERID(user.getH01USR());
	 	msgLOAD.setH01PROGRM("EDD0931");
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD093102")) {
			
			beanList =  new JBObjList(); 
			while (true) {
	
				msgList = (EDD093102Message)newmessage;
			...
			if (IsNotError) {  // There are no errors
				...
				try {
					flexLog("About to call Page: " + LangPath + "EDD0931_cleaning_load_list.jsp");
					callPage(LangPath + "EDD0931_cleaning_load_list.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
...
protected void procActionLoad(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		flexLog("Send Initial Data");
	 	msgLOAD = (EDD093103Message)mc.getMessageRecord("EDD093103");
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD093103")) {
			
			msgLOAD = (EDD093103Message)newmessage;
			

			if (IsNotError) {  // There are no errors
				try {
					flexLog("About to call Page: " + LangPath + "EDD0931_cleaning_load_confirm.jsp");
					callPage(LangPath + "EDD0931_cleaning_load_confirm.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
