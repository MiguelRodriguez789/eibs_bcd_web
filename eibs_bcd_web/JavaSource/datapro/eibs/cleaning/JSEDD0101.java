...
public class JSEDD0101 extends datapro.eibs.master.SuperServlet {
...
public void destroy() {
	...
	flexLog("free resources used by JSEDD0101");
...
protected void procReqControl(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		flexLog("Send Initial Data");
	 	msgCTRL = (EDD010101Message)mc.getMessageRecord("EDD010101");
		msgCTRL.setH01USERID(user.getH01USR());
	 	msgCTRL.setH01PROGRM("EDD0101");
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD010101")) {
			
			msgCTRL = (EDD010101Message)newmessage;
			
			flexLog("Putting java beans into the session");

			if (IsNotError) {  // There are no errors
				ses.setAttribute("error", msgError);
				ses.setAttribute("uncollRLS", msgCTRL);
				ses.setAttribute("userPO", userPO);
				
				try {
					flexLog("About to call Page: " + LangPath + "EDD0101_uncoll_release.jsp");
					callPage(LangPath + "EDD0101_uncoll_release.jsp", req, res);		
				}
				catch (Exception e) {
	...
	if (session == null) {
		...
		try {
			...
			try
			{
				...
				mc = new MessageContext(super.getMessageHandler("EDD0101", req));
...
protected void procActionControl(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDD010101")) {
			...
			if (IsNotError) {  // There are no errors
				try {
					flexLog("About to call Page: " + LangPath + "EDD0101_uncoll_release_confirm.jsp");
					callPage(LangPath + "EDD0101_uncoll_release_confirm.jsp", req, res);	
					}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
