	...
	if (session == null) {
		...
		try {
			...
			if (screen == R_ENTER) {
				...
				mc = new MessageContext(super.getMessageHandler("EDD0930", req));
...
protected void procActionEnter(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		// all the fields here
	 	try {
				msgAPP.setE01CNTFEM(req.getParameter("E01CNTFEM"));
		} catch (Exception e) {
				msgAPP.setE01CNTFEM("0");
		}
		try {
				msgAPP.setE01CNTFED(req.getParameter("E01CNTFED"));
		} catch (Exception e) {
				msgAPP.setE01CNTFED("0");
		}
		try {
				msgAPP.setE01CNTFEY(req.getParameter("E01CNTFEY"));
		} catch (Exception e) {
...
protected ELEERRMessage procSendData(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, EDD093002Message msgList)
	...
	try {
			...
			try {
				msgAPP.setE03CNTFEM(req.getParameter("E03CNTFEM"));
			} catch (Exception e) {
				msgAPP.setE03CNTFEM("0");
			}
			try {
				msgAPP.setE03CNTFED(req.getParameter("E03CNTFED"));
			} catch (Exception e) {
				msgAPP.setE03CNTFED("0");
			}
			try {
				msgAPP.setE03CNTFEY(req.getParameter("E03CNTFEY"));
			} catch (Exception e) {
				msgAPP.setE03CNTFEY("0");
			}
	 		try {
