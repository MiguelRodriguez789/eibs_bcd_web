 ...
 * 	// List of Options
 * 	//
 *	// SCREEN =    3	Request FEP File Name          
 *	// SCREEN =    5    Process  FEP Incoming File 
 */
...
public class JSEACH401 extends JSEIBSServlet { 
	...
	protected void processRequest(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, 
		...
		switch (screen) {
			//  
			case 3 : // Request File Name 
				procReqFile(user, req, res, session);
				break;
			case 5 : // Proccess FEP File
	...
	protected void procReqFile(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		forward("EACH401_incoming_file.jsp", req, res);
	...
	protected void procActionFile(ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			try {
				...
				msg = (EACH401DSMessage) mp.getMessageRecord("EACH401DS");
				...
				while (true) {
					...
					if (line.length() > 0 ) {
						...
						while (find == true) {
							if (line.length() > 1024) {
								linetosend = line.substring(posIni, posIni + 1024);
								posIni = posIni + 1024;					
							} else {
				...
				if (mp.hasError(newmessage)) {
					...
					redirectToPage("/servlet/datapro.eibs.ach.JSEACH401?SCREEN=3", res);
				}
			} catch (Exception e) {
				e.printStackTrace();
