...
public class JSESS0041 extends datapro.eibs.master.SuperServlet {
...
public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
	...
	if (session == null) {
		...
		try
		{
			...
			mc = new MessageContext(super.getMessageHandler("ESS0041", req));
		}
		catch (Exception e)
		{
			e.printStackTrace();
		...
		flexLog("Sending Request. UserID = " + msgUser.getH01USR());
	  	try {
			msgMenu = (ESS0041DSMessage)mc.getMessageRecord("ESS0041DS");
		...
		try
		{
		  ...
		  if (newmessage.getFormatName().equals("ESS0041DS")) {
			msgMenu = (ESS0041DSMessage)newmessage; 
