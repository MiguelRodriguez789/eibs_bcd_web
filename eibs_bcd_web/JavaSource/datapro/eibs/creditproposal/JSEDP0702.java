...
public class JSEDP0702 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqEnterBranch(
		...
		try {
			...
			try {
				...
				callPage(LangPath + "EDP0702_changeprod_enter.jsp", req, res);
			} catch (Exception e) {
	...
	protected void procActionPos(
		...
		switch (inptOPT) {
			...
			res.sendRedirect(
				super.srctx
					+ "/servlet/datapro.eibs.creditproposal.JSEDP0702?SCREEN=300");
				break;
			case 2 : //Maintenance
			...
			default :
				res.sendRedirect(
					super.srctx
						+ "/servlet/datapro.eibs.creditproposal.JSEDP0702?SCREEN=500") ;
				break;
		}
	}
				...
				try {
						...
						if (msgError.getERRNUM().equals("0")) {
							flexLog(
								"About to call Page: "
									+ LangPath
									+ "EDP0702_format_list.jsp");
							callPage(
								LangPath + "EDP0702_format_list.jsp",
								req,
								res);
						} else {
							flexLog(
...
//			ses.setAttribute("brnDetails", msgList);
   ...
   protected void procReqNew(
	   ...
	   MessageRecord newmessage = null;
	   ELEERRMessage msgError = null;
	   EDP070201Message brnDetails = null;
	   UserPos userPO = null;
	   boolean IsNotError = false;
	   ...
	   ses.setAttribute("brnDetails", brnDetails);

	   try {
		   flexLog(
			   "About to call Page: "
				   + LangPath
				   + "EDP0702_format_maintenance.jsp");
		   callPage(LangPath + "EDP0702_format_maintenance.jsp", req, res);
	   } catch (Exception e) {
		   flexLog("Exception calling page " + e);
	   }
	...
	protected void procActionMaintenance(
		...
		try {
			...
			msgRT = (EDP070201Message) ses.getAttribute("brnDetails");
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDP070201")) {
				try {
					msgRT = new datapro.eibs.beans.EDP070201Message();
					flexLog("EDP070201 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgRT = (EDP070201Message) newmessage;
				...
				//
					if (IsNotError) { // There are no errors
						res.sendRedirect(
							super.srctx
								+ "/servlet/datapro.eibs.creditproposal.JSEDP0702?SCREEN=100");
						...
						try {
							flexLog(
								"About to call Page: "
									...
									+ "EDP0702_format_maintenance.jsp");
							callPage(
								LangPath + "EDP0702_format_maintenance.jsp",
								req,
								res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
	...
	protected void procReqMaintenance(
		...
		// Receive Data
		try {
			JBObjList bl = (JBObjList) ses.getAttribute("EDP070201Help");
			int idx = Integer.parseInt(req.getParameter("CURRCODE"));
			bl.setCurrentRow(idx);

			msgDoc = (EDP070201Message) bl.getRecord();
		...
		if (session == null) {
			...
			try {
				...
				try {
					...
					mc = new MessageContext(super.getMessageHandler("EDP0702", req));
