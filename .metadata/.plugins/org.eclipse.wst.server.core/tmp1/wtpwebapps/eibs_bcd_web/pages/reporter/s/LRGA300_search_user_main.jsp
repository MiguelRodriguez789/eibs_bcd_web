...
%>
<h3 class="h3title">B&uacute;squeda de Usuarios<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="search_user_main, LRGA300"></h3>
<hr class="raya">
...
<table class="tbenter"  width="100%" border="0">
    ...
    <tr> 
      <td nowrap ALIGN=left> 
		<input type="radio" name="usersWithReportsOnly" value="1" <%= (msg.getUsersWithReportsOnly().equals("1") ? "checked" : "") %>>Usuarios con reportes solamente
		<br>
		<input type="radio" name="usersWithReportsOnly" value="0" <%= (!msg.getUsersWithReportsOnly().equals("1") ? "checked" : "") %>>Todos los Usuarios con autoridad de creaci&oacute;n
      </td>
    </tr>                  
  </table>
<br><br>
...
<%
	...
	<div style="width: 100%;" align="center" class="dataContent">
		...
		requestURI="" style="width:100%;" class="dataTable" export="false" >
			...
			<display:column title="Reportes" class="r">
			<a href="#" onclick="assign('${result.userCode}', '${result.totalReports}');">
				<c:out value="${result.totalReports}"/>			
			</a>
			</display:column>
			<% } %>			
	</display:table></div>
