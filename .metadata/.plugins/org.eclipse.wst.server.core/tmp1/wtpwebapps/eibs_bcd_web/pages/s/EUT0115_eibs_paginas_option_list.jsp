...
<%@ page import="datapro.eibs.master.Util,datapro.eibs.beans.EUT011502Message"%>

<!doctype html public "-//w3c//dtd html 4.0 transitional//en">
<%@page import="com.datapro.constants.EibsFields"%>
<html>
<head>
<title>Tablas de Opciones</title>
...
function getParams(bnk,acd,tar) {
    ...
    getElement("BANK").value = bnk;
	getElement("ACD").value = acd;
	getElement("TABLEN").value = tar;
...
<h3 align="center">Tabla de Opciones
	<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="eibs_paginas_option_list.jsp,EUT0115"></h3>
<hr size="4">
<form method="POST"
	...
	width: 250px">
      <tr id="trdark"> 
        ...
        <td align=CENTER width="34%"> 
          <div align="left"> 
            <input type="text" id="E02COIDEN" name="E02COIDEN" size="5" maxlength="3"  onKeypress="enterInteger(event)" >
          </div>
        </td>
      </tr>
      <tr id="trclear"> 
      ...
      <td NOWRAP valign="top" width="100%"> 
		<table id="headTable" width="98%">
			<tr id="trdark">
				...
				<th align="left" nowrap width="45%">
						Descripci&oacute;n
				</th>
				<th align="left" nowrap width="30%">
						Tipo
				</th>
			</tr>
        </table>
		...
		<%
			msglst.initRow();
				...
				while (msglst.getNextRow()) {
					...
					EUT011502Message menu = (EUT011502Message) msglst.getRecord();
		%>
		<tr id="dataTable<%= msglst.getCurrentRow() %>">
			<td nowrap align="center" width="5%"><input type="radio" id="key1" name="key1"	value="<%=msglst.getCurrentRow()%>" <%=chk%> class="highlight" onClick="highlightRow('dataTable', <%= msglst.getCurrentRow() %>)"/></td>
			<td nowrap align="center" width="20%"><a href="javascript:goAction('12',<%= msglst.getCurrentRow()%>);"><%=menu.getE02COIDEN()%></a></td>
			<td nowrap align="left" width="45%"><a href="javascript:goAction('12',<%= msglst.getCurrentRow()%>);"><%=menu.getE02COENDS()%></a></td>
			<td nowrap align="left" width="30%"><a href="javascript:goAction('12',<%= msglst.getCurrentRow()%>);">
				<% if (menu.getE02COIMFL().equals("I")) out.print("Consulta");
    	           if (menu.getE02COIMFL().equals("M")) out.print("Mantenimiento");
    	           if (menu.getE02COIMFL().equals("N")) out.print("Creación");
    	           if (menu.getE02COIMFL().equals("A")) out.print("Aprobación");
    	           if (menu.getE02COIMFL().equals("R")) out.print("Reporte");
    	           if (menu.getE02COIMFL().equals("P")) out.print("Proceso");
    	           if (menu.getE02COIMFL().equals("D")) out.print("Administración");
    	        %>    
				</a>
			</td>
		</tr>
