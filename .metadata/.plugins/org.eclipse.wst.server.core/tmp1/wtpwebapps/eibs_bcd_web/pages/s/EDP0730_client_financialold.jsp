...
function showOption(obj){
 var i= obj[obj.selectedIndex].value; 
   getElement("dataGrpTable"+ document.forms[0].ActOpt.value).style.display="none"
...
function showGrpAcc(flg,idxRow){
  ...
  try {
  	getElement("dataAccTable"+flg).style.display="";
  	if ( getElement("dataAccTable"+flg).rows.length > 0) {
  	ACCINFO.style.display="";
  	adjustEquTables( getElement("headTable2"), getElement("dataAccTable"+flg), getElement("dataDiv2"),1,false);
...
// alert(document.forms[0].E01DPXLID.text);
  ...
  var dataTable= getElement("dataAccTable"+document.forms[0].ActAccTb.value);
...
// alert(document.forms[0].E01DPXLID.value);
 ...
 function goAction(op) {
   ...
   switch (op) {
    ...
    case 3 :{
            page = "${pageContext.request.contextPath}/servlet/datapro.eibs.creditproposal.JSEDP0730?SCREEN=3";
