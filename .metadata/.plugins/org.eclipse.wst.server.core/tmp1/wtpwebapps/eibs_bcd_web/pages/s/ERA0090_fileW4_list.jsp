...
function showAddInfo(ref) {
    document.forms[0].E04RW2REF.value = ref;
}

function submitThis(option) {
	if (document.forms[0].REF.value !='') {;
		document.forms[0].E04RW2REF.value=document.forms[0].REF.value;
	}   
	switch (option) {
            //lista registros del archivo seleccionado
    case 5 :{
		showMntW4(document.forms[0].E04RW2REF.value);
       }
		break;
            //
    case 7 :{
...
function showMntW4(ref) {
	var SCREEN = "104";
	var OPECOD = "0005";
	var E04RW2REF = ref;
	page = "<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0090?SCREEN="+SCREEN+"&OPECOD="+OPECOD+"&E04RW2REF="+E04RW2REF;
	CenterNamedWindow(page,'Information',1200,500,2);
}

function goRefresh() {
	if ( isValidObject( getElement("E04NUMREG"))) {
		document.forms[0].Pos.value = document.forms[0].E04NUMREG.value-20;
	}
...
<h3 align="center">Mantenimiento Tabla Garantías Obligación para Interface a CREDISMART 
...
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0090">
  <p> 
    ...
    <input type=HIDDEN name="E04RW2REF" id="E04RW2REF" >
...
<TABLE  id="mainTable" class="tableinfo" ALIGN=CENTER height="70%">
    <TR id="trdark"> 
      ...
      <TH ALIGN=CENTER nowrap>Número<br>Obligación</TH>
      <TH ALIGN=CENTER nowrap>Número<br>Cliente</TH>
      <TH ALIGN=CENTER nowrap>Tipo<br>Documento</TH>
      <TH ALIGN=CENTER nowrap>Identificación</TH>
      <TH ALIGN=CENTER nowrap>Razón Social</TH>
    </TR>
    <%
                appList.initRow();
                int k=1;
