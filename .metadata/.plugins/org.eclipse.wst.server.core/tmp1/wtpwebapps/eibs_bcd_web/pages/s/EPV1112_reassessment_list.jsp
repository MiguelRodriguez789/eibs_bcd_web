...
<%@ page import="datapro.eibs.master.Util,datapro.eibs.beans.EPV111201Message"%>
...
<jsp:useBean id="EPV1112List" class="datapro.eibs.beans.JBObjList" scope="session" />
...
function goNew(op,row) {
	...
	var prlAcr = getElement('E01PCRACR').value;
	var url = "${pageContext.request.contextPath}/servlet/datapro.eibs.salesplatform.JSEPV1112?SCREEN="+ op + "&ajaxRequest=Y&E01PCRACR=" + prlAcr;
	
	$.ajax({
		  type: 'POST',
		  url: "${pageContext.request.contextPath}/servlet/datapro.eibs.salesplatform.JSEPV1112?SCREEN="+ op + "&ajaxRequest=Y&E01PCRACR=" + prlAcr,
          cache: false,
          data: serialize,
...
function goAction(op ,row) {
	...
	if (op == '400') {
		...
		var url = "<%=request.getContextPath()%>/servlet/datapro.eibs.salesplatform.JSEPV1112?SCREEN="+op;		
		url += "&E01PCRACR=" + prlAcr;
		if (op =='300' || op == '301') {				
...
function hideMenu() {
	if (!(document.getElementById("menuRQ") == null)) {
		if (document.getElementById("menuRQ").style.display == 'none') {
			document.getElementById("menuRQ").style.display = '';
			resizeIFrame(true);
		} else {
			document.getElementById("menuRQ").style.display = 'none';
			resizeIFrame(false);
		}
	}
}
...
function showMenu(firstTime) {
	if(isValidObject(document.getElementById("menuRQ") && !firstTime)){
		if (document.getElementById("menuRQ").style.display == 'none') {
			document.getElementById("menuRQ").style.display = '';
			resizeIFrame(true);
		} 
...
function resizeIFrame(lMenu) {
	var nHeight = 0;	
	var nMenu = 60;
	if (isValidObject(parent.document.getElementById("ifremprq"))) {
		nHeight = parseInt(document.getElementById("IFRHEIGHT").value);
	}
	if (nHeight > 0) {
		nHeight = lMenu ? nHeight + nMenu : nHeight;
		parent.document.getElementById("ifremprq").height = nHeight;
	}	
}
...
<% 
  ...
  String emp = (String)session.getAttribute("EMPRQ");
  emp = (emp==null)?"":emp; 
%>

<%if ("".equals(emp)){ %>
<h3 align="center">Listado de Utilizaciones<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="reassessment_list.jsp,EPV1112"></h3>
<hr size="4">
<%}%>
	...
	filter:progid:DXImageTransform.Microsoft.dropshadow(OffX=3, OffY=3, Color='gray', Positive='true');">
      <tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>">  
        ...
        <td align=CENTER width="34%"> 
          <div align="left"> 
            <eibsinput:help property="E01PCRACR" fn_param_one="E01PCRACR" fn_param_two="" fn_param_three="L3" fn_param_five="<%=userPO.getCusNum()%>"  eibsType="<%=EibsFields.EIBS_FIELD_TYPE_ACCOUNT %>" />
          </div>
        </td>
      </tr>
      <tr id="<%=(row % 2 == 1) ? "trdark" : "trclear"%><%row++;%>">  
...
<%}else{%>
    ...
    <br><br>
    <table id="menuRQ" class="tbenter" width="100%" >
      <tr>
        ...
        <td align="center" class="tdbkg" width="33%" style="display: <%= (EPV1112List.isEmpty()) ? "none": "" %> "><a href="javascript:goAction('400')"> <b>Borrar</b> </a></td>
<!--  
			...
			<%	
				...
				EPV1112List.initRow();
					...
					while (EPV1112List.getNextRow()) {
						...
						EPV111201Message datarec = (EPV111201Message) EPV1112List.getRecord();
						suma += datarec.getBigDecimalE01PCRMTO().doubleValue();
			%>
			<tr id="dataTable<%= EPV1112List.getCurrentRow() %>">
				<td nowrap align="center" width="2%" ><input type="radio" name="keys" value="<%=EPV1112List.getCurrentRow()%>" <%=chk%> class="highlight" onClick="showMenu();highlightRow('dataTable',<%=EPV1112List.getCurrentRow()%>)"/></td>
				<td nowrap align="center" width="5%" ><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRSEQ()%></a></td>
				<td nowrap align="right"  width="10%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRACR()%></a></td>
				<td nowrap align="left"   width="30%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01DSCPRD()%></a></td>																		
				<td nowrap align="right"  width="11%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRSTO()%></a></td>
				<td nowrap align="right"  width="11%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRCTO()%></a></td>
				<td nowrap align="right"  width="11%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRPTO()%></a></td>
				<td nowrap align="right"  width="11%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRDTO()%></a></td>
				<td nowrap align="right"  width="12%"><a href="javascript:goAction('301',<%=EPV1112List.getCurrentRow()%>);" onclick="hideMenu();"><%=datarec.getE01PCRMTO()%></a></td>
			</tr>
			<% } %>
           </table> 
         </div>
...
<SCRIPT type="text/javascript">
	...
	if (isValidObject(parent.document.getElementById("ifremprq"))) {
		document.getElementById("IFRHEIGHT").value = parent.document.getElementById("ifremprq").height;
	}	
</SCRIPT>
