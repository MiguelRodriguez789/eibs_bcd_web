...
location= "${pageContext.request.contextPath}/pages/s/EDD0924_rejection_chk_search.jsp";
} 
...
function OkComments(){
	var idxchk =document.getElementById("ActRow").value;
	getElement("REMDATA"+idxRow).value = trim(document.getElementById("txtComments").value);
	if (getElement("REMDATA"+idxRow).value == ""){
...
function addComments() {
     ...
     Comments.filters[0].Play();
     document.getElementById("txtComments").value=getElement("REMDATA"+idxRow).value;
...
function ShowComments(idxchk) {	 
	 ...
	 document.getElementsByName("RECNUM")[idxchk].click();
	 cancelBub();
	 hideDiv(schCause);
  	 hideDiv(schACC);
	 eval('Comments.style.pixelTop=' + y);
...
function showACC(){
 document.getElementById("FlagMov").value="0";
 document.forms[0].submit();
}

function SendInfo(){
  document.getElementById("FlagMov").value="1";
  document.forms[0].submit();
}

function checkCause(){
 var valc=document.getElementById("CAUSE").value;
...
function showAddInfo(idxRow){
  var i= parseInt(document.getElementById("ActRow").value);
  dataTable.rows[i].className="trnormal";
  dataTable.rows[idxRow].className="trhighlight";
  document.getElementById("ActRow").value=""+idxRow;
}    
...
function GetInfo(currentrow){
  ...
  if (form["OPTION_"+currentrow].value == "0004") {
	...
	page= "${pageContext.request.contextPath}/pages/s/EDD0924_rejection_chk_change.jsp" + Params; 
		...
		if (form["OPTION_"+currentrow].value == "0009") {
	  		...
	  		if (form["TYPE_"+currentrow].value == ""){ 
				if (form["ESTADO_"+currentrow].value == "X"){ 
				  	...
				  	var type = form["TYPE_"+currentrow].value;
				  	var Params = "?CurrRow="+ currentrow + "&Acc=" + acc + "&Chk=" + chk + "&Rs1=" + rs1 + "&Rs2=" + rs2 + "&Rs3=" + rs3 + "&Rs4=" + rs4 + "&GL=" + gl + "&TYPE=" + type;
			  		page= "${pageContext.request.contextPath}/pages/s/EDD0924_rejection_chk_change_print.jsp" + Params; 
...
function SetSelection(value){
  var form = document.forms[0];
  var maxrow = getElement("dataTable").rows.length;
...
function ShowPrev(){
  document.getElementById("FlagMov").value="-";
  document.forms[0].submit();
}
...
<h3 align="center">Resoluci&oacute;n Protesto Central de Canje<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="rejection_chk_protesto_list.jsp,EDD0924"></h3>
<hr size="4">
...
<INPUT TYPE=HIDDEN NAME="FlagMov" id="FlagMov" VALUE="0">
<%
	int recnum = 0;
 ...
 <TABLE id=tbhelp style="border-top-width : 1px;border-right-width : 1px;border-bottom-width : 1px;border-left-width : 1px;
	...
	<TR>
	  <TD ALIGN=right nowrap>
          Buscar Cuenta : 
          <input type="text" maxlength=13 size=12 name="ACCNUM" id="ACCNUM" onKeyPress="enterInteger(event)"
...
<div id="schCause" style="position:absolute; visibility:hidden; left:25px; top:-100px; z-index:3; filter:progid:DXImageTransform.Microsoft.Fade(duration=1.0,overlap=1.0)" onclick="cancelBub()">
 <TABLE id=tbhelp style="border-top-width : 1px;border-right-width : 1px;border-bottom-width : 1px;border-left-width : 1px;
	...
	<TR>
	  <TD ALIGN=right nowrap>
          Buscar Causal : 
          <input type="text" name="CAUSE" id="CAUSE" maxlength=4 size=4 
...
<div id="Comments" style="position:absolute; visibility:hidden; left:25px; top:-100px; z-index:3; filter:progid:DXImageTransform.Microsoft.Fade(duration=1.0,overlap=1.0)" onclick="cancelBub()">
 <TABLE id=tbhelp style="border-top-width : 1px;border-right-width : 1px;border-bottom-width : 1px;border-left-width : 1px;
	...
	<TR>
	  <TD ALIGN=Left nowrap>
          Comments :<BR> 
          <input type="text" maxlength=61 size=60 name="txtComments" id="txtComments">
      </TD>
    </TR>
    <TR>
     		...
     		<TR> 
              ...
              <TD ALIGN=Left NOWRAP> 
				<DIV > 
                  ...
                  <img name="IMGREM<%= chkList.getCurrentRow() %>" src="<%=request.getContextPath()%>/images/s/info.gif" title="More Information" border="0" align="middle" onClick="ShowComments(<%= chkList.getCurrentRow() %>)"
...
<%--				<img name="ImgGL_<%= chkList.getCurrentRow() %>" border="0" title="GL. Number: <%= chkList.getRecord(26) %> " src="<%=request.getContextPath()%>/images/s/imgGL.gif" > --%>
				...
				<td><input type="HIDDEN" name="ESTADO_<%= chkList.getCurrentRow() %>" value="<%= chkList.getRecord(4) %>"></td>
