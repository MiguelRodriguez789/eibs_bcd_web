...
function showInqApprovalBatch(batch,bank,branch) {
   ...
   pg = "${pageContext.request.contextPath}/servlet/datapro.eibs.products.JSEGL0035I?SCREEN=200&BATCH=" + batch + "&BANK=" + bank + "&BRANCH=" + branch;
...
<%  
 ...
 if(datapro.eibs.master.SuperServlet.hasPort(request)){
	 if ( session.getAttribute("PORT").toString().toLowerCase().equals("pm") ) {
	 	title = title + " Reproceso Mensual";
	 } else {
		 if ( session.getAttribute("PORT").toString().toLowerCase().equals("py") ) {
		   	title = title + " Reproceso Anual";
		   	}
	 }   
 } 
...
<h3 align="center">
  <%= title %>
<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="approval_list.jsp,EGL6003"></H3>  
<hr size="4"> 
  ...
  <TABLE class="tbenter">
    <TR>
      <TD class="TDBKG" width="33%">
  			<a href="javascript:goAction('A')" id="linkApproval" >Aprobacion</a>
      </TD>
      <TD class="TDBKG" width="34%">
  			<a href="javascript:goExit()" >Salida</a>
      </TD>
    </TR>
  </TABLE>
...
<TABLE  id="mainTable" class="tableinfo" ALIGN=CENTER>
 <TR> 
  ...
  <TR id="trdark">  
      ...
      <TH ALIGN=CENTER nowrap>Debitos</TH>
      <TH ALIGN=CENTER nowrap>Creditos</TH>
