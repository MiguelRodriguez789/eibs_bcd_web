...
<div id="body-wait-div" class="wait-div" style="visibility: hidden;"></div>
...
<SCRIPT> 
	window.onbeforeunload = function (){
		...
		document.getElementById('body-wait-div').style.visibility = 'visible';
	};

	document.forms[0].action="<%= request.getParameter("ServletName")%>";
...
<%
		...
		if (session == null || session.getAttribute("currUser") == null) {
			...
			out.println("</SCRIPT>");
		} else if(Boolean.valueOf(request.getParameter("noCleanSes"))){
			//noCleanSes == true , indicates the request is from Pop-Up Window
			//Pop-Ups do not Clean session objects.
			//do nothing.			
			...
			Object optionMenu = session.getAttribute("option-menu");
			...
			session.setAttribute("option-menu", optionMenu);
...
%>
<table class=tbenter border="0" cellspacing="0" cellpadding="0" style="background-repeat:no-repeat; background-attachement:fixed;" align="center" width="100%" height="100%">
