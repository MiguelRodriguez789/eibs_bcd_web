...
<SCRIPT type="text/javascript">
function callCust() {
var customer = getElement("CUN").value;    
...
function callGuard() {
var customer = getElement("CUN").value;    
var cunam = document.forms[0].getElement("CUNAM").value;    
var PROP = getElement("PROP").value;
...
var page = "<%=request.getContextPath()%>/servlet/datapro.eibs.creditproposal.JSEDP0750?SCREEN=300&PROP="+ prop;
...
var prop = getElement("PROP").value;
var customer = getElement("CUN").value;    
var cunam = getElement("CUNAM").value;    
...
function callImpP() {
	getElement("FMT").value = "L";
    getElement("SCREEN").value="730";
	document.forms[0].submit();		  	       	       
}
...
function getParams(currrow, PROP, CUN, CUNAM, E01YYYFIL) {
    ...
    getElement("PROP").value = PROP;   		   
    getElement("CUN").value = CUN;			   
    getElement("CUNAM").value = CUNAM; 		   
    getElement("E01YYYFIL").value = E01YYYFIL; 
 	getElement("CONTI").value = "0";  
...
function GetCpForm() 
{
	...
	var page = "${pageContext.request.contextPath}/servlet/datapro.eibs.reports.JSEFRM000?SCREEN=7&E01PLTNPR=" + prop;
...
var cunam = getElement("CUNAM").value;    
var PROP = getElement("PROP").value;
//var option = getElement("OPTION").value;
var option = '5';
...
%> 
<h3 align="center">Lista de Propuestas 
	<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="proposals_list.jsp, EDP0720"></h3>
<hr size="4">
<FORM name="form1" METHOD="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.creditproposal.JSEDP0720" >
  <p>  
    ...
    <input type=HIDDEN name="E01CUSNAM" value="<%= userPO.getCusName()%>"> 
...
%> 
  ...
  <table class="tbenter" width=100% align=center>
    <tr> 
		...
		<td class=TDBKG width="16%">
        	<div align="center">
        		<a href="javascript:goAction(5)"><b>Consultar</b></a>
        	</div> 
     	</td>
      ...
      <td NOWRAP valign="top" width="100%"> 
        <table id="headTable" width="100%">
          <tr id="trdark"> 
            ...
            <th align=CENTER nowrap width="20%"> 
              <div align="center">Producto<br>Operacion</div>
            </th>
                                 
           <th align=CENTER nowrap width="10%"> 
              <div align="center">Monto Sol.<BR>Aprobado</div>
           </th>  
		 ...
		 <% indice2 = indice % 2; %>
          <tr id="<% if (indice2==0) {out.print("trdark");} else {out.print("trclear");} %>"> 
        	...
        	<td align=LEFT width="25%"><%= msgList.getE01PLPCUN()%>-<%= msgList.getE01CUSNA1()%></td>
        
        	<td align=LEFT width="25%"><%= msgList.getE01PLTPR0()%>-<%= msgList.getE01PLTDE0()%></td>
        
        	<td NOWRAP  align=RIGHT width="20%"><%= msgList.getE01PLTAMN()%><BR> <%= msgList.getE01PLPAM2()%></td>
			...
			<td NOWRAP  align=CENTER width="10%"><%= msgList.getE01PLPIPD()%>/<%= msgList.getE01PLPIPM()%>/<%= msgList.getE01PLPIPY()%>:<%= msgList.getE01PLPHMS().substring(8,10)%>:<%= msgList.getE01PLPHMS().substring(5,7)%>:<%= msgList.getE01PLPHMS().substring(2,4)%> </td>

        	<td NOWRAP  align=CENTER width="10%"><%= msgList.getE01PLPUDI()%></td>
			
			<td NOWRAP  align=CENTER width="10%"><%= msgList.getE01PLPEJE()%><BR>
			<%= msgList.getE01BTHIDN()%></td>
			
        	        	
            <td NOWRAP  align=LEFT width="25%"><%= msgList.getE01PLPSRU()%>-<%= msgList.getE01DESAC1()%><BR>
