...
<IMG src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="EOF0115_of_chk_apr_confirm.jsp"></H3>
<HR size="4">
<BR>
...
<%			} else {%>
		...
		<a href="javascript:CenterWindow('<%=request.getContextPath()%>/pages/s/error_list_viewer.jsp?ERRORNUM=<%=i%>',400,200,2);"><img src="<%=request.getContextPath()%>/images/1b.gif" title="ayuda" border="0"  ></a>
		
	<%}%>			  
			  
				<TD NOWRAP ALIGN="RIGHT"><%=Util.formatCell(msgPart.getE01OFMNCH())%></TD>
				<TD NOWRAP ALIGN="CENTER"><%=Util.formatCell(msgPart.getE01OFMCCY())%></TD>
				<TD NOWRAP ALIGN="RIGHT"><%=Util.formatCell(msgPart.getE01OFMBRN())%></TD>
				<TD NOWRAP ALIGN="RIGHT"><%=Util.formatCell(msgPart.getE01OFMMCH())%></TD>
				<TD NOWRAP ALIGN="CENTER"><%=Util.formatCell(msgPart.getE01OFMSTS())%></TD>
				<TD NOWRAP ALIGN="CENTER"><%=Util.formatCustomDate(currUser.getE01DTF(),msgPart.getE01OFMIDM(), msgPart.getE01OFMIDD(), msgPart.getE01OFMIDY())%></TD>
				<TD NOWRAP ALIGN="LEFT"><%=Util.formatCell(msgPart.getE01OFMBNF())%></TD>
