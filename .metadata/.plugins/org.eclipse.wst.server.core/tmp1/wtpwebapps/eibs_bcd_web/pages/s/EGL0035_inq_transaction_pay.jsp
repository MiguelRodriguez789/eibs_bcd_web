...
function submitThis(option) {
	...
	if ( currentrow.trim() !== "" ) { setCurrentRow(currentrow);}
...
<%
 ...
 %>
<h3 align="center">Entrada de Multiples Transacciones</h3>
<hr size="4">
<table width="100%" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF"
	bordercolor="#FFFFFF">
	<tr bgcolor="#FFFFFF" bordercolor="#FFFFFF">
		<td align="center"><a href="javascript:submitThis(5)"
			onMouseOut="MM_swapImgRestore()"
			onMouseOver="MM_swapImage('Image4','','<%=request.getContextPath()%>/images/s/imprimir_over.gif',1)"><img
			name="Image4"
			src="<%=request.getContextPath()%>/images/s/imprimir.gif"
			title="Imprimir" align="middle" border="0"></a></td>
	</tr>
</table>
...
<INPUT TYPE=HIDDEN NAME="SCREEN" VALUE="2"> <INPUT TYPE=HIDDEN
	NAME="opt" VALUE="1"> <INPUT TYPE=HIDDEN NAME="CURRENTROW"
	VALUE=""> <input type=HIDDEN name="OPTHELP" value=""> <INPUT
	TYPE=HIDDEN NAME="totalRow" VALUE="<%=total_row%>"> <INPUT
	TYPE=HIDDEN NAME="E01DEAACC" VALUE="<%=transData.getChkNum()%>">
	...
	<tr id="trdark">
		...
		<%
       	if (userPO.getOption().equals("LN")) {
       		...
       		out.println("Cheque No. : ");
       	}
       	//if ( userPO.getHeader19().equals("1")) {out.println("Cheque No. : ");}
       	//else {out.println("");}
       	//}
       	else if (userPO.getOption().equals("TRANSACTION")) {
       		out.println("Lote : ");
       	}
       %>
		</td>
		...
		<%
           ...
           %> <input type="text" name="E01NUMACC" size="12"
			maxlength="11" readonly value="<%=transData.getChkNum()%>"> <%
          	} else {
          %> <input type="text" name="E01NUMACC" size="17"
			maxlength="15" readonly
			value="<%=(request.getParameter("PURPOSE").equals(
							"READONLY") ? transData.getChkNum() : transData
							.getAccNum())%>">
		<%
          	}
          	} else {
          %> <input type="text" name="E01BTHNUM" size="6" maxlength="5"
			readonly value="<%=transData.getBthnum()%>"> <%
          	}
          %>
		</div>
		</td>
		...
		<%
           	if (userPO.getOption().equals("TRANSACTION")) {
           %> <input type="text" name="E01WRKOBK" size="4" maxlength="2"
			readonly value="<%=transData.getBank()%>"> <input type="text"
		...
		<td align="left" nowrap colspan="3"><input type="text"
			...
			value="<%=transData.getDebitAmt()%>"></td>
	</tr>
	<tr id="trdark">
		<td align="right" nowrap width="21%">Total Transacciones :</td>
		<td align="right" nowrap width="21%">
		<div align="left"><input type="text" name="ROW" size="7"
...
<table class="tableinfo">
	<tr id="trclear">
		...
		<td align="left" width="40%" nowrap><input type="text"
			...
			value="<%=transData.getDescription()%>"> <a
			href="javascript:GetInfo('')"><img
		...
		<td align="left" width="29%" nowrap><input type="text"
			...
			value="<%=transData.getDate1()%>"> <input type="text"
			name="E01WRKVD2" size="3" maxlength="2" readonly
			value="<%=transData.getDate2()%>"> <input type="text"
			name="E01WRKVD3" size="5" maxlength="4" readonly
			value="<%=transData.getDate3()%>"></td>
	</tr>
</table>
<BR>
<TABLE id="mainTable" class="tableinfo" ALIGN=CENTER>
	<TR>
     ...
     %>
			<tr id="trclear">
				<td align="center" nowrap><input readonly type="checkbox"
					name="TRANSROW_<%=trans.getCurrentRow()%>" value="checked">
				</td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKBNK_<%=trans.getCurrentRow()%>" size="2" maxlength="2"
					value="<%=trans.getRecord(0)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("class=\"txtchanged\"");%>>
				</td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKBRN_<%=trans.getCurrentRow()%>" size="4" maxlength="4"
				...
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKCCY_<%=trans.getCurrentRow()%>" size="3" maxlength="3"
					value="<%=trans.getRecord(2)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
					out.print("id=\"txtright\"");%>>
				</td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKGLN_<%=trans.getCurrentRow()%>" size="17"
					maxlength="16" value="<%=trans.getRecord(3)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
				...
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKCCN_<%=trans.getCurrentRow()%>" size="8" maxlength="8"
					value="<%=trans.getRecord(4)%>"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
					out.print("id=\"txtright\"");%>>
				...
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKCDE_<%=trans.getCurrentRow()%>" size="5" maxlength="4"
					value="<%=trans.getRecord(6)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
				...
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKDCC_<%=trans.getCurrentRow()%>" size="1" maxlength="1"
					value="<%=trans.getRecord(8)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
				...
				<td align="center" nowrap><input type="text"
					name="E01WRKCOD_<%=trans.getCurrentRow()%>" size="6" maxlength="4"
					value="<%=trans.getRecord(50)%>"
					onFocus="setCurrentRow('<%=trans.getCurrentRow()%>')"
					<%if (trans.getRecord(24).equals("*"))
					out.print("id=\"txtrchanged\"");
				else
				...
				<td align="left" nowrap><a
					...
					readonly type=Hidden name="E01WRKTDS_<%=trans.getCurrentRow()%>"
					size="37" maxlength="300"
					value="<%=trans.getRecord(9) + trans.getRecord(15)
						+ trans.getRecord(16) + trans.getRecord(17)
						+ trans.getRecord(18) + trans.getRecord(19)
						+ trans.getRecord(20) + trans.getRecord(21)
						+ trans.getRecord(22) + trans.getRecord(23)%>">
				<input type=Hidden name="E01WRKVD1_<%=trans.getCurrentRow()%>"
					size="2" maxlength="2" value="<%=trans.getRecord(10)%>"> <input
					type=Hidden name="E01WRKVD2_<%=trans.getCurrentRow()%>" size="2"
					maxlength="2" value="<%=trans.getRecord(11)%>"> <input
					type=Hidden name="E01WRKVD3_<%=trans.getCurrentRow()%>" size="2"
					maxlength="2" value="<%=trans.getRecord(12)%>"> <input
					type=Hidden name="E01WRKTYP_<%=trans.getCurrentRow()%>" size="2"
					maxlength="2" value="<%=trans.getRecord(25)%>"> <input
					type=Hidden name="E01WRKEXR_<%=trans.getCurrentRow()%>" size="12"
					maxlength="11" value="<%=trans.getRecord(13)%>"> <input
					type=Hidden name="E01WRKCUN_<%=trans.getCurrentRow()%>" size="10"
					maxlength="9" value="<%=trans.getRecord(26)%>"> <input
					type=Hidden name="DUPLIC_<%=trans.getCurrentRow()%>" size="2"
					maxlength="2" value="<%=trans.getRecord(24)%>"></td>
			</tr>
			<%
    	}
    ...
    %>
			<tr id="trclear">
				...
				<td align="center" nowrap><input readonly type="text"
					...
					class="txtright" onFocus="setCurrentRow('<%=i%>')"></td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKACC_<%=i%>" size="13" maxlength="11" value=""
					class="txtright" onFocus="setCurrentRow('<%=i%>')"></td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKCDE_<%=i%>" size="5" maxlength="4" value=""
					class="txtright" onFocus="setCurrentRow('<%=i%>')"></td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKAMT_<%=i%>" size="17" maxlength="17" value=""
					class="txtright" onFocus="setCurrentRow('<%=i%>')"></td>
				<td align="center" nowrap><input readonly type="text"
					name="E01WRKDCC_<%=i%>" size="1" maxlength="1" value=""
					class="txtright" onFocus="setCurrentRow('<%=i%>')"></td>
				<td align="center" nowrap><input type="text"
					name="E01WRKCOD_<%=i%>" size="6" maxlength="4" value=""
					class="txtright" onFocus="setCurrentRow('<%=i%>')">
				</td>
				<td align="left" nowrap><a href="javascript:GetInfo('<%=i%>')"><img
					src="<%=request.getContextPath()%>/images/s/info.gif"
					title="Mas Información" align="middle" border="0"></a> <input
					type=Hidden name="E01WRKTDS_<%=i%>" size="37" maxlength="300"
					value=""> <input type=Hidden name="E01WRKVD1_<%=i%>"
					size="2" maxlength="2" value=""> <input type=Hidden
					name="E01WRKVD2_<%=i%>" size="2" maxlength="2" value=""> <input
					type=Hidden name="E01WRKVD3_<%=i%>" size="2" maxlength="2" value="">
				<input type=Hidden name="E01WRKTYP_<%=i%>" size="2" maxlength="2"
					value=" "> <input type=Hidden name="E01WRKEXR_<%=i%>"
					size="12" maxlength="11" value=""> <input type=Hidden
					name="E01WRKCUN_<%=i%>" size="10" maxlength="9" value=""> <input
					type=Hidden name="DUPLIC_<%=i%>" size="2" maxlength="2" value="">
				</td>
			</tr>
			<%
    	}
