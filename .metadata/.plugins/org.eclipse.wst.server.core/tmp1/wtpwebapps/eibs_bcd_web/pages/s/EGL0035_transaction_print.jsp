...
<h3 align="center">Transacciones<img
	src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left"
	name="EIBS_GIF" title="transaction_print,EGL0035"></h3>
<hr size="4">

<form><INPUT TYPE=HIDDEN NAME="SCREEN" VALUE="2"> <INPUT
	TYPE=HIDDEN NAME="opt" VALUE="1"> <INPUT TYPE=HIDDEN
	NAME="CURRENTROW" VALUE=""> <INPUT TYPE=HIDDEN NAME="totalRow"
	VALUE="<%= total_row %>">
...
<table class="tableinfo" id="trdark">
	<tr id="trdark">
		...
		<div align="right">
		<% if (userPO.getOption().equals("TRANSACTION")) { %> <b>Originado :
		</b> <% } %>
		</div>
		</td>
		...
		<td align="left" nowrap colspan="3" width="26%"><%= Util.formatCCY(transData.getDebitAmt())%>
		</td>
	</tr>
	<tr id="trdark">
		<td align="right" nowrap width="21%"><b>Total Transacciones
		:</b></td>
		<td align="right" nowrap width="21%">
		<div align="left"><%= total_row %></div>
		</td>
		...
		<td align="left" nowrap colspan="3" width="26%"><%= Util.formatCCY(transData.getCreditAmt()) %>
		</td>
	</tr>
</table>
...
<table class="tableinfo">
	<tr id="clear">
		<td align="right" width="17%" valign="top"><b> Descripcion :
		</b></td>
		<td align="left" width="40%" valign="top">
		<div align="left" style="width: 350"><%= transData.getDescription() %></div>
		</td>
		<td align="right" nowrap width="14%" valign="top"><b> Fecha
		Valor : </b></td>
		<td align="left" width="29%" nowrap valign="top"><%= Util.formatDate(transData.getDate1(),transData.getDate2(),transData.getDate3())  %>
		</td>
	</tr>
</table>
<BR>
	      ...
	      %>
			<tr id="trclear">
				...
				<td align="center" nowrap
					onclick="GetInfo('<%= trans.getCurrentRow()%>')"
					style="cursor: hand"><%= trans.getRecord(9) %> <input
					type=Hidden name="E01WRKTDS_<%= trans.getCurrentRow()%>" size="37"
					maxlength="300"
				...
				<input type=Hidden name="E01WRKVD1_<%= trans.getCurrentRow()%>"
					size="2" maxlength="2" value="<%= trans.getRecord(10) %>">
				<input type=Hidden name="E01WRKVD2_<%= trans.getCurrentRow()%>"
					size="2" maxlength="2" value="<%= trans.getRecord(11) %>">
				<input type=Hidden name="E01WRKVD3_<%= trans.getCurrentRow()%>"
					size="2" maxlength="2" value="<%= trans.getRecord(12) %>">


				<input type=Hidden name="E01WRKBNK_<%= trans.getCurrentRow() %>"
					size="4" maxlength="4" value="<%= trans.getRecord(0) %>">


				<input type=Hidden name="E01WRKBRN_<%= trans.getCurrentRow() %>"
					size="4" maxlength="4" value="<%= trans.getRecord(1) %>">


				<input type=Hidden name="E01WRKCCY_<%= trans.getCurrentRow() %>"
					size="3" maxlength="4" value="<%= trans.getRecord(2) %>">


				<input type=Hidden name="E01WRKGLN_<%= trans.getCurrentRow() %>"
					size="17" maxlength="17" value="<%= trans.getRecord(3) %>">


				<input type=Hidden name="E01WRKCCN_<%= trans.getCurrentRow() %>"
					size="8" maxlength="8" value="<%= trans.getRecord(4) %>">


				<input type=Hidden name="E01WRKACC_<%= trans.getCurrentRow() %>"
					size="13" maxlength="13" value="<%= trans.getRecord(5) %>">


				<input type=Hidden name="E01WRKCDE_<%= trans.getCurrentRow() %>"
					size="4" maxlength="4" value="<%= trans.getRecord(6) %>">


				<input type=Hidden name="E01WRKAMT_<%= trans.getCurrentRow() %>"
					size="17" maxlength="17" value="<%= trans.getRecord(7) %>">


				<input type=Hidden name="E01WRKDCC_<%= trans.getCurrentRow() %>"
					size="4" maxlength="4" value="<%= trans.getRecord(8) %>">

				<input type=Hidden name="E01WRKMD1_<%= trans.getCurrentRow()%>"
					size="4" maxlength="4" value="<%= trans.getRecord(37) %>">
				<input type=Hidden name="E01WRKMD2_<%= trans.getCurrentRow()%>"
					size="4" maxlength="4" value="<%= trans.getRecord(38) %>">
				<input type=Hidden name="E01WRKMD3_<%= trans.getCurrentRow()%>"
					size="4" maxlength="4" value="<%= trans.getRecord(39) %>">
				<input type=Hidden name="E01WRKCKN_<%= trans.getCurrentRow()%>"
					size="10" maxlength="9" value="<%= trans.getRecord(27) %>">
				...
				<% if ( trans.getRecord(8).equals("D")) {out.print( Util.formatCCY(trans.getRecord(7)) );} else {out.print("");} %>
				</td>
				<td align="right" nowrap>
				<% if ( trans.getRecord(8).equals("C")) {out.print( Util.formatCCY(trans.getRecord(7)) );} else {out.print("");}%>
				</td>
			</tr>

			<tr id="trdark">
				<td nowrap colspan="4"></td>
				<td nowrap colspan="7"><%= trans.getRecord(40).trim()%></td>
			</tr>
...
<table width="80%" class="tbenter" id="OPTDIV">
	<tr align="center">
		...
		<td class="TDBKG"><a href="javascript:goOpenerAction('A')">Aprobar</a>
		</td>
		<% }%>

		<td class="TDBKG"><a href="javascript:doPrint()">Imprimir</a></td>

		<% if (userPO.getPurpose().equals("APPROVAL_INQ") && userPO.getOption().equals("TRANSACTION")) { %>
		<td class="TDBKG"><a href="javascript:goOpenerAction('R')">Rechazar</a>
		</td>
		<% }%>
