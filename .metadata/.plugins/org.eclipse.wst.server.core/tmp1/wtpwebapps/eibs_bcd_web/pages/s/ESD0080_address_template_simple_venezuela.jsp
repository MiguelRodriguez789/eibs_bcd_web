...
function concatCTY(evt){
	...
	<tr id="trclear"> 
        ...
        <td nowrap>
            ...
            <INPUT type="text" name="<%=PaisDsc%>" id="<%=PaisDsc%>" size="55" maxlength="45" value="<%=mr.getField(PaisDsc).getString().equals("")?"Venezuela":mr.getField(PaisDsc).getString()%>" <%=READ_ONLY_TAG%>>
			<% if (!readOnly.toUpperCase().equals("TRUE")){ %>		 
			<a href="javascript:GetCodeDescCNOFC('CTYCode<%=index%>','<%=PaisDsc%>','03')"><img src="<%=request.getContextPath()%>/images/1b.gif" title="ayuda" align="bottom" border="0"></a> 
			<% } %>
	...
	<tr id="trdark">
		...
		<TD nowrap>
			...
			<INPUT type="text" name="<%=EstadoDsc%>" id="address.Estado.Dsc<%=index%>" size="55" maxlength="45" value="<%=estadoDscValue%>" <%=READ_ONLY_TAG%>>
			<% if (!readOnly.toUpperCase().equals("TRUE")){ %>	
			<a href="javascript:clearMunicipio('address.Municipio<%=index%>','address.Municipio.Dsc<%=index%>','address.Parroquia<%=index%>','address.Parroquia.Dsc<%=index%>');GetCodeDescCNOFC('<%=Estado%>','<%=EstadoDsc%>','27')"><img src="<%=request.getContextPath()%>/images/1b.gif" title="ayuda" align="bottom" border="0"></a> 
			<%}%>
        	<!--<img src="<%=request.getContextPath()%>/images/Check.gif" title="mandatory field" align="bottom">-->		
		</td>		
	</tr>
	...
	<tr id="trclear">
    	...
    	<td nowrap>
			...
			<INPUT type="text" name="<%=RegionDsc%>" id="address.Region.Dsc<%=index%>" size="55" maxlength="45" value="<%=regionDscValue%>" <%=READ_ONLY_TAG%>>
			<% if (!readOnly.toUpperCase().equals("TRUE")){ %>		
	...
	<tr id="trdark"> 
        ...
        <td nowrap>
        	...
        	<INPUT type="text" name="<%=MunicipioDsc%>" id="address.Municipio.Dsc<%=index%>" size="55" maxlength="45" value="<%=municipioDscValue%>" <%=READ_ONLY_TAG%>>
        	<% if (!readOnly.toUpperCase().equals("TRUE")){ %>
			<a href="javascript:clearParroquia('address.Parroquia<%=index%>','address.Parroquia.Dsc<%=index%>');GetCNOFCFilteredCodes('address.Municipio<%=index%>','address.Municipio.Dsc<%=index%>',document.getElementById('address.Estado<%=index%>').value,'85')"><img src="<%=request.getContextPath()%>/images/1b.gif" title="ayuda" align="bottom" border="0"></a> 
			<%}%>
        	<!--<img src="<%=request.getContextPath()%>/images/Check.gif" title="mandatory field" align="bottom">-->
		</td>
	</tr>
	...
	<tr id="trclear"> 
        ...
        <td nowrap>
        <input type="text" name="<%=CTY%>" id="<%=CTY%>"size="36" maxlength="26" value="<%=mr.getField(CTY).getString()%>" <%=READ_ONLY_TAG%>>
        ...
        <!--
			...
			<% if (!readOnly.toUpperCase().equals("TRUE")){ %>		
			<a href="javascript:GetCNOFCFilteredCodes('Aux_CTYCode<%=index%>','Aux_CTYDsc<%=index%>',document.getElementById('address.Estado<%=index%>').value,'84')"><img src="<%=request.getContextPath()%>/images/1b.gif" title="ayuda" align="bottom" border="0"></a> 
           	<%}%> 
        	<img src="<%=request.getContextPath()%>/images/Check.gif" title="mandatory field" align="bottom">		!-->
		</td>
	</tr>		
	...
	<tr id="trdark">	
		...
		<TD nowrap>
			...
			<INPUT type="text" name="<%=ParroquiaDsc%>" id="address.Parroquia.Dsc<%=index%>" size="55" maxlength="45" value="<%=parroquiaDscValue%>" <%=READ_ONLY_TAG%>>
			<% if (!readOnly.toUpperCase().equals("TRUE")){ %>		
	...
	<TR id="trclear">
		...
		<TD nowrap>
			<input type="text" name="<%=Referencia%>" id="<%=Referencia%>" size="55" maxlength="45" value="<%=referenciaValue%>" <%=READ_ONLY_TAG%>>
		</TD>
	</TR>
