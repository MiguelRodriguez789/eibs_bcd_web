...
function add(num) {
	...
	if (i == -1) {
		...
		E02CCRCRA.value = acc[i].value;
		var key = acc[i].label;
    	var temp = new Array();
    	temp = key.split('-');
    	E02CCRTYP.value = temp[0];
    	E02CCRSTP.value = temp[1];
		
		if (num == 1) {
			if (E02CCRTYP.value != "CCTE" &&
				E02CCRTYP.value != "CINL" &&
				E02CCRTYP.value != "CTEI") {
		...
		if (num == 4) {
			if (E02CCRTYP.value != "CCTE" &&
				E02CCRTYP.value != "CINL" &&
				E02CCRTYP.value != "CTEI" &&
				E02CCRTYP.value != "CAHO" &&
				E02CCRTYP.value != "CFAL") {
				alert("La cuenta seleccionada no puede ser asignada como cuenta secundaria.");
			}
			else {
				var option = new Option(E02CCRCRA.value + " (" + E02CCRTYP.value + "/" + E02CCRSTP.value + ")", E02CCRCRA.value, false);
				var accs = document.getElementById("AccSList");
				if (accs[0].value == "X") {
					accs.remove(0);
				}
				accs.options[accs.length] = option;
				accs[accs.length-1].label = E02CCRTYP.value + "-" + E02CCRSTP.value;
				acc.remove(i);
			}
		}
		if (acc.length == 0) {
...
function del(num) {
	...
	if (num == 4) {
		if (i == -1) {
			...
			acc.options[acc.length] = option;
			acc[acc.length-1].label = E02CCRTYP.value + "-" + E02CCRSTP.value;
			accs.remove(i);
		}
	}
	if (accs.length == 0) {
...
de Tarjetas de Débito<BR>Mantenimiento de Tarjeta Titular<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" title="assign_accounts_det.jsp,ECC0090"></h3>
<hr size="4">
<% 
