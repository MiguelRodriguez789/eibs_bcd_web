...
<%
  ...
  if( request.getParameter("simple") != null ){
   ...
   if(request.getParameter("showEmail") != null){
	   countryHelp= Boolean.parseBoolean( request.getParameter("showEmail").toString() ) ;
   } else if(basicData && !simple){
   	   showWeb = true;	
   }  
...
<% } %>
             ...
             <td nowrap >  
              ...
              <td nowrap >  
				<input type="hidden" name="<%= ciudad %>" id="<%= ciudad%>" size="8" maxlength="6" value="<%=mr.getField(ciudad).getString()%>" >
				<input type="text" name="<%=ciudadDsc%>" id="<%=ciudadDsc%>" size="55" maxlength="45" value="<%=mr.getField(ciudadDsc).getString()%>" readonly class="direccion"
				<% if(basicData){ out.print((mr.getField(ciudadDscF).getString().equals("Y"))?"class=\"txtchanged\"":""); } %> >
				<% if ( !readOnly ) {
					if(isDomesticAddress){
				%>
						<A href="javascript:GetCodeDescCNOFC('<%= ciudad%>','<%= ciudadDsc%>','84')" >
              ...
              <td nowrap >  
				<input type="hidden" name="<%= estado %>" id="<%= estado%>" size="8" maxlength="6" value="<%=mr.getField(estado).getString()%>" >
				<input type="text" name="<%=estadoDsc%>" id="<%=estadoDsc%>" size="55" maxlength="45" value="<%=mr.getField(estadoDsc).getString()%>" readonly class="direccion"
				<% if(basicData){ out.print((mr.getField(estadoDscF).getString().equals("Y"))?"class=\"txtchanged\"":""); } %> >
				<% if ( !readOnly ) {
					if(isDomesticAddress){
				%>
						<A href="javascript:GetCodeDescCNOFC('<%= estado%>','<%= estadoDsc%>','84')" >
	              			<img src="<%=request.getContextPath()%>/images/1b.gif" title="Ayuda" align="bottom" border="0" style="cursor:hand" >
		          		</A>
		          	<% } %>	
					<img src="<%=request.getContextPath()%>/images/Check.gif" title="mandatory field" align="bottom"	border="0">
              ...
              <td nowrap >  
        			<input type="text" name="<%= apostal%>" id="<%=apostal%>"  size="15" maxlength="10" value='<%= mr.getField(apostal) %>' <%=READ_ONLY_TAG %>  class="direccion"
        			<%= (basicData && mr.getField(apostalF).getString().equals("Y"))?"class=\"txtchanged\"":"" %> > 
                </td>
            </tr>
          	<tr class="<%=getRowColor()%>" > 
              <td nowrap > 
              ...
              <td nowrap >  
        			<input type="text" name="<%=codigoPostal%>" id="<%=codigoPostal%>"  size="15" maxlength="10" value='<%= mr.getField(codigoPostal) %>' <%=READ_ONLY_TAG %>  class="direccion"
        			<%= (basicData && mr.getField(codigoPostalF).getString().equals("Y"))?"class=\"txtchanged\"":"" %> > 
                </td>
            </tr>
         <tr class="<%=getRowColor()%>"  style="display : <%= showEmail ? "table-row" : "none"%>" >
         ...
         <tr class="<%=getRowColor()%>" style="display :<%= showWeb ? "table-row" : "none"%>"  >
