     		...
     		<TR>      			
                <TD ALIGN=CENTER NOWRAP>				
                 <input type="radio" name="RECNUM" value="<%= accList.getCurrentRow() %>" <%=chk%> onClick="showAccInfo('<%= accList.getCurrentRow() %>')">
      			...
      			<TD ALIGN=LEFT NOWRAP><DIV ><a href="javascript:radioClick('RECNUM',<%= accList.getCurrentRow() %>)"><%= accList.getRecord(2) %></a></DIV></TD>      
     		...
     		<TR> 
              ...
              <TD ALIGN=Left NOWRAP width="21"> 
                <input type="radio" name="RECNUM<%=j%>" value="<%= docList.getCurrentRow() %>" <%=chk%> onClick="showDocInfo('<%= docList.getRecord(7)%>','<%= docList.getRecord(1)%>','<%= docList.getRecord(0)%>','<%= docList.getRecord(6)%>','<%=j%>',<%=k%>)"> 
      			...
      			<TD ALIGN=CENTER NOWRAP width="36"><div ><a href="javascript:showDetailApproval('RECNUM<%=j%>',<%= k %>,'<%= docList.getRecord(5) %>')"><%= docList.getRecord(0) %></a></DIV></TD>
      			<TD ALIGN=LEFT NOWRAP><div ><a href="javascript:showDetailApproval('RECNUM<%=j%>',<%= k %>,'<%= docList.getRecord(5) %>')"><%= docList.getRecord(2) %></a></DIV></TD>      
      			<TD ALIGN=RIGHT NOWRAP><div ><a href="javascript:showDetailApproval('RECNUM<%=j%>',<%= k %>,'<%= docList.getRecord(5) %>')"><%= datapro.eibs.master.Util.fcolorCCY(docList.getRecord(3)) %></a></DIV></TD>
      			...
      			<TD ALIGN=CENTER NOWRAP><div ><a href="javascript:showDetailApproval('RECNUM<%=j%>',<%= k %>,'<%= docList.getRecord(5) %>')"><%= docList.getRecord(5) %></a></DIV></TD>
     		</TR>
  		<% 
                    k++;
					chk="";
